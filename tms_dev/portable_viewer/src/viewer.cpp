...
void Viewer::Bezier_Curve(tracker_point before, tracker_point now, tracker_point now2, tracker_point after)
{
    ...
    p1_x = (now.x  - ((pv) * (pv) * (pv)) * p0_x - (3 * (pv) * (vp) * (vp)) * now2.x  -  ((vp) * (vp) * (vp) * p3_x))  / (3 * (pv) * (pv) * (vp));
    p1_y = (now.y  - ((pv) * (pv) * (pv)) * p0_y - (3 * (pv) * (vp) * (vp)) * now2.y  -  ((vp) * (vp) * (vp) * p3_y))  / (3 * (pv) * (pv) * (vp));

    p2_x = (now2.x - ((vp) * (vp) * (vp)) * p0_x - (3 * (vp) * (vp) * (pv)) * now.x   -  ((pv) * (pv) * (pv) * p3_x))  / (3 * (pv) * (pv) * (vp));
    p2_y = (now2.y - ((vp) * (vp) * (vp)) * p0_y - (3 * (vp) * (vp) * (pv)) * now.y   -  ((pv) * (pv) * (pv) * p3_y))  / (3 * (pv) * (pv) * (vp));

    p1_x = (now.x  - ((pv) * (pv) * (pv)) * p0_x - (3 * (pv) * (vp) * (vp)) * p2_x  -  ((vp) * (vp) * (vp) * p3_x))  / (3 * (pv) * (pv) * (vp));
    p1_y = (now.y  - ((pv) * (pv) * (pv)) * p0_y - (3 * (pv) * (vp) * (vp)) * p2_y -  ((vp) * (vp) * (vp) * p3_y))  / (3 * (pv) * (pv) * (vp));
    ...
    while(1)
    {
      p_x = (1.0 - t)*(1.0 - t)*(1.0 - t) * p0_x + 3 * (1.0 - t) * (1.0 - t) * t * p1_x + 3 * (1.0 - t) * t * t * p2_x + t * t * t *p3_x;
      p_y = (1.0 - t)*(1.0 - t)*(1.0 - t) * p0_y + 3 * (1.0 - t) * (1.0 - t) * t * p1_y + 3 * (1.0 - t) * t * t * p2_y + t * t * t *p3_y;
