    ...
    qnode(node)
{
    ...
    fss_Unknown_class_data_subscriber       = nh.subscribe("fss_unknown_class_data",   10, &Viewer::fssUnknownClassDataCallback, this);
    ...
    fss_unknown_object_data_subscriber      = nh.subscribe("fss_unknown_object_data",    10, &Viewer::fssUnknownObjectDataCallback, this);
    ...
    fss_person_trajectory_data_subscriber   = nh.subscribe("fss_person_trajectory_data",    10, &Viewer::fssPersonTrajectoryDataCallback, this);
    ...
    rosclientTmsdbRobotTrajectoryData       = nh.serviceClient<tms_msg_db::tmsdb_robot_trajectory_data>("tmsdb_robot_trajectory_data");
...
void Viewer::display()
{
    ...
    if(m_iViewSmartpal == MODE_SMARTPAL_ON)
    {
        ...
        if(m_msgSmartpal.msgTMSInfo.size() != 0)
            if(m_msgSmartpal.msgTMSInfo[0].iState == STATE_EXIST)
                 drawSmartpal(m_msgSmartpal.msgTMSInfo[0].fX , m_msgSmartpal.msgTMSInfo[0].fY , m_msgSmartpal.msgTMSInfo[0].fTheta * DEG2RAD);
    }
    ...
    if(m_iViewRoomba == MODE_ROOMBA_ON)
    {
        ...
        if(m_msgRoomba.msgTMSInfo.size() != 0)
            if(m_msgRoomba.msgTMSInfo[0].iState == STATE_EXIST)
                drawRoomba(m_msgRoomba.msgTMSInfo[0].fX , m_msgRoomba.msgTMSInfo[0].fY , m_msgRoomba.msgTMSInfo[0].fTheta * DEG2RAD);
    }
    ...
    //CPainter.drawText(10, 10, "Mouse Point : "+QString::number(m_dMouse_point_x) + ", "+ QString::number(m_dMouse_point_y));
    ...
    if(m_iViewCluster == MODE_CLUSTER_ON)
    {
        ...
        if(iLength != 0)
        {
            for(unsigned int i=0; i < iLength; i++)
            {
                ...
                {
                    CPainter.setPen(QPen(Qt::black, iPointSize));
                    CPainter.drawText(MM2Px(m_msgCluster.fCenterX[i]), MM2Py(m_msgCluster.fCenterY[i]+50), "cID = " + QString::number(m_msgCluster.iID[i]));
                }
            }
        }
    }
    ...
    if(m_iViewClass == MODE_CLASS_ON)
    {
        ...
        if(iLength != 0)
        {
            ...
            for(unsigned int i=0; i < iLength; i++)
            {
                ...
                {
                    if(m_msgClass.iID[i]<20) color_id = m_msgClass.iID[i]+1;
                    else	color_id = (m_msgClass.iID[i]+1)%10;
                }
                ...
                {
                    CPainter.setPen(QPen(Qt::black, iPointSize));
                    CPainter.drawText(MM2Px(m_msgClass.fCenterX[i]), MM2Py(m_msgClass.fCenterY[i]+100), "ID = " + QString::number(m_msgClass.iID[i]));
                }
            }
        }
    }
    ...
    if(m_bTagViewIntensity)
    {
        ...
        if(iLength != 0)
        {
            ...
            {
                CPainter.setPen(QPen(Qt::black, iPointSize));
                CPainter.drawText(MM2Px(m_msgClass.fCenterX[i]), MM2Py(m_msgClass.fCenterY[i]+150), QString::number(m_msgClass.fAvgIntrinsicIntensity[i]));
            }
        }
    }
    ...
    if(m_iViewUnknownClass == MODE_UNKNOWN_CLASS_ON)
    {
        ...
        if(iLength != 0)
        {
            ...
            for(unsigned int i=0; i < iLength; i++)
            {
                ...
                {
                    if(m_msgUnknownClass.iID[i]<20) color_id = m_msgUnknownClass.iID[i]+1;
                    else	color_id = (m_msgUnknownClass.iID[i]+1)%10;
                }
                ...
                {
                    CPainter.setPen(QPen(Qt::black, iPointSize));
                    CPainter.drawText(MM2Px(m_msgUnknownClass.fCenterX[i]), MM2Py(m_msgUnknownClass.fCenterY[i]+100), "ID = " + QString::number(m_msgUnknownClass.iID[i]));
                }
            }
        }
    }
    ...
    if(m_bTagViewIntensity)
    {
        ...
        if(iLength != 0)
        {
            ...
            {
                CPainter.setPen(QPen(Qt::black, iPointSize));
                CPainter.drawText(MM2Px(m_msgUnknownClass.fCenterX[i]), MM2Py(m_msgUnknownClass.fCenterY[i]+150), QString::number(m_msgUnknownClass.fAvgIntrinsicIntensity[i]));
            }
        }
    }
    */
...
//------------------------------------------------------------------------------
int Viewer::MM2P(double f_dP)  { return (int)( m_viewer_scale*f_dP); }
int Viewer::MM2Px(double f_dX) { return (int)( m_viewer_scale*(f_dX + m_dOffset_x)); }
int Viewer::MM2Py(double f_dY) { return (int)(-m_viewer_scale*(f_dY + m_dOffset_y)+m_dWindow_size_y); }
int Viewer::MM2Pl(double f_dD) { return (int)( m_viewer_scale*f_dD); }
double Viewer::P2MMx(int f_iX) { return (f_iX)/m_viewer_scale - m_dOffset_x; }
double Viewer::P2MMy(int f_iY) { return (m_dWindow_size_y - f_iY)/m_viewer_scale - m_dOffset_y; }
double Viewer::P2MMl(int f_iD) { return (f_iD)/m_viewer_scale; }
...
void Viewer::load_map()
{
    ...
    e0.dX0 = 0;             e0.dY0 = 0;
    e0.dX1 = MaxMapSizeX;   e0.dY1 = 0;

    e1.dX0 = MaxMapSizeX;   e1.dY0 = 0;
    e1.dX1 = MaxMapSizeX;   e1.dY1 = MaxMapSizeY;

    e2.dX0 = MaxMapSizeX;   e2.dY0 = MaxMapSizeY;
    e2.dX1 = 0;             e2.dY1 = MaxMapSizeY;

    e3.dX0 = 0;             e3.dY0 = MaxMapSizeY;
    e3.dX1 = 0;             e3.dY1 = 0;
...
void Viewer::load_grid()
{
    ...
    for(int n = 0; n<15; n++) {
        point[n].dX0  =   interval*(n+1);   point[n].dY0  = 0;
        point[n].dX1  =   interval*(n+1);   point[n].dY1  = maxMapSizeY;       
    }
    for(int n = 0; n<8; n++) {
        point[n+15].dX0  =   0;             point[n+15].dY0  = interval*(n+1);
        point[n+15].dX1  =   maxMapSizeX;   point[n+15].dY1  = interval*(n+1);       
    }
...
void Viewer::drawPlanPath(void)
{
    ...
    if(m_bPlanPathView == MODE_PLAN_PATH_ON)
    {
		...
		CPainter.drawPoint(MM2Px(m_msgPlanPath.rps_route[m_msgPlanPath.rps_route.size()-1].x), MM2Py(m_msgPlanPath.rps_route[m_msgPlanPath.rps_route.size()-1].y));
    }
}
...
void Viewer::drawRobotPath(void)
{
    ...
    if((m_bRobotPathView == MODE_ROBOT_PATH_ON)&&(m_iViewSmartpal == MODE_SMARTPAL_ON))
    {
		for(unsigned int i=0;i<m_msgRobotPath.rps_route.size();i++){
			...
			int num;
			if (robot_id == 2002) num = 28;
			else if (robot_id == 2006) num = 9;
			else num = 24;
    ...
    if((m_iViewPath == MODE_PATH_ON)&&(m_bRobotPathView == MODE_ROBOT_PATH_ON))
    {
		...
		CPainter.setPen(QPen(Qt::black, iPointSize*(int)(450/(m_msgRPS_MAP.rps_map_x.size()-1))));
		CPainter.drawPoint(MM2Px(m_msgRobotPath.rps_route[m_msgRobotPath.rps_route.size()-1].x), MM2Py(m_msgRobotPath.rps_route[m_msgRobotPath.rps_route.size()-1].y));
    }
			...
			for(unsigned int y=0;y<m_msgRPS_MAP.rps_map_x[x].rps_map_y.size();y++){
				if(m_bCollisionAreaViewRPS_MAP == true){
					if(m_msgRPS_MAP.rps_map_x[x].rps_map_y[y].robot_collision){
						...
						CPainter.drawPoint(MM2Px(x*10*(int)(MaxMapSizeX/(m_msgRPS_MAP.rps_map_x.size()-1))), MM2Py(y*10*(int)(MaxMapSizeY/(m_msgRPS_MAP.rps_map_x[x].rps_map_y.size()-1))));
					}
...
void Viewer::drawSmartpal(float f_dX, float f_dY, float f_dTheta)
{
    ...
    if(m_iViewSmartpal == MODE_SMARTPAL_ON)
    {
        ...
        for (int i=0; i<iRobot_max_point_index; i++)
        {
            int iRotaionX = (CSmartpal_points[i].x()-f_dX)*cos(f_dTheta) - (CSmartpal_points[i].y()-f_dY)*sin(f_dTheta);
            int iRotaionY = (CSmartpal_points[i].x()-f_dX)*sin(f_dTheta) + (CSmartpal_points[i].y()-f_dY)*cos(f_dTheta);
        ...
        if(m_bTagViewSmartpal == true)
        {
            ...
            CPainter.drawText(MM2Px(f_dX-200), MM2Py(f_dY+430), "Theta = " + qsTheta.setNum(f_dTheta * RAD2DEG,'f',0) + "[degree]");
...
void Viewer::drawRoomba(float f_dX, float f_dY, float f_dTheta)
{
    ...
    if(m_bTagViewRoomba == true)
    {
        ...
        CPainter.drawText(MM2Px(f_dX-200), MM2Py(f_dY+220), "Theta = " + qsTheta.setNum(f_dTheta * RAD2DEG,'f',1) + "[degree]");
...
void Viewer::drawWagon(void)
{
    ...
    if(m_iViewWagon== MODE_WAGON_ON)
    {
        ...
        if(m_msgWagon.msgTMSInfo[0].fX != 0 && m_msgWagon.msgTMSInfo[0].fY !=0)
        {
            ...
            //CPainter.drawLine(MM2Px(m_msgWagon.fVirtualPointX[0]), MM2Py(m_msgWagon.fVirtualPointY[0]), MM2Px(m_msgWagon.fVirtualPointX[1]), MM2Py(m_msgWagon.fVirtualPointY[1]));
            //CPainter.drawLine(MM2Px(m_msgWagon.fVirtualPointX[1]), MM2Py(m_msgWagon.fVirtualPointY[1]), MM2Px(m_msgWagon.fVirtualPointX[2]), MM2Py(m_msgWagon.fVirtualPointY[2]));
            //CPainter.drawLine(MM2Px(m_msgWagon.fVirtualPointX[2]), MM2Py(m_msgWagon.fVirtualPointY[2]), MM2Px(m_msgWagon.fVirtualPointX[3]), MM2Py(m_msgWagon.fVirtualPointY[3]));
            //CPainter.drawLine(MM2Px(m_msgWagon.fVirtualPointX[3]), MM2Py(m_msgWagon.fVirtualPointY[3]), MM2Px(m_msgWagon.fVirtualPointX[0]), MM2Py(m_msgWagon.fVirtualPointY[0]));
    ...
    else if(m_iViewLaser==MODE_LASER_POINT)
    {
        if(m_bViewStaticPoint)
        {
            for(int i=0; i < m_dLrf_scan_max_count; i++)
            {
                ...
                {
                    unsigned int iIntensityHueValue = (unsigned int)((m_msgRawdata.fIntensity[i]-m_iMinIntensityView)/(m_iMaxIntensityView-m_iMinIntensityView)*270);

                    if(iIntensityHueValue > 270)        iIntensityHueValue = 270;
                    else if(iIntensityHueValue <= 0)    iIntensityHueValue = 0;
        ...
        if(m_bViewReflectPoint)
        {
            for(int i=0; i < m_dLrf_scan_max_count; i++)
            {
                if(m_msgRawdata.bIsReflect[i]==true && m_msgRawdata.bIsForwardPoint[i]==true)   // Reflect Point
                {
                    ...
                    QLineF line1(MM2Px(m_dLrf_set_x), MM2Py(m_dLrf_set_y), MM2Px(m_msgRawdata.fX1[i]), MM2Py(m_msgRawdata.fY1[i]));
                    CPainter.drawLine(line1);

                    QLineF line2(MM2Px(m_msgRawdata.fX1[i]), MM2Py(m_msgRawdata.fY1[i]), MM2Px(m_msgRawdata.fX2[i]), MM2Py(m_msgRawdata.fY2[i]));
