...
void MainWindow::captureEvent()
{
    ...
    QString format = "png";
    QString fileName = QDir::homePath() + tr("/") + QDate::currentDate().toString("yyyyMMdd") + QTime::currentTime().toString("hhmmss") + tr(".") + format;
...
void MainWindow::viewData()
{
    for(int i=0; i < viewer->m_dLrf_scan_max_count; i++)
    {
        ...
        this->table_model->setData(table_model->index(i,1,QModelIndex()), QString::number( viewer->m_dUtm30LX_raw_data[i].fDistance,'f',0 ) );
        this->table_model->setData(table_model->index(i,2,QModelIndex()), QString::number( viewer->m_dUtm30LX_raw_data[i].fIntensity,'f',0 ) );
        this->table_model->setData(table_model->index(i,3,QModelIndex()), QString::number( viewer->m_dUtm30LX_raw_data[i].fIntrinsicIntensity,'f',0 ) );
        this->table_model->setData(table_model->index(i,4,QModelIndex()), QString::number( viewer->m_dUtm30LX_raw_data[i].fAcuteAngle*RAD2DEG,'f',0 ) );
    }
}
