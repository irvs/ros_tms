...
bool tms_to_find_objects_func(tms_msg_rp::tms_ts_find_objects::Request &req, tms_msg_rp::tms_ts_find_objects::Response &res){
...
//SIN:
		...
		//※(現在のロボットの位置・アームの位置・物品の絶対座標・移動距離 => 「TMS_DBのロボット座標」とKinectで得た情報より変換…ここで出るのは相対座標基準)
		...
		if(client01.call(srv01)){
			ROS_INFO("[ID=%d] x = %f, y =%f, z=%f", srv01.request.furnitures_id, srv01.response.furniture_x, srv01.response.furniture_y, srv01.response.furniture_z);
		}

		srv02.request.robots_id = 1;
		if(client02.call(srv02)){
			ROS_INFO("[ID=%d] x = %f, y =%f, z=%f", srv02.request.robots_id, srv02.response.robots_x, srv02.response.robots_y, srv02.response.robots_theta);
		}
		...
		if(client03.call(srv03)){
			ROS_INFO("[ID=%d] rostime = %lu, pcl_file = %s, get_x = %f, get_y =%f, get_z=%f, get_theta = %f",
					srv03.request.id, srv03.response.rostime, srv03.response.pcd_file.c_str(), srv03.response.get_x, srv03.response.get_y, srv03.response.get_z, srv03.response.get_theta);
		}
		...
		if(client4.call(srv4)){
			for(unsigned int j=0;j<srv4.response.objects.poses.size();j++){
				ROS_INFO("Success![%d] x = %f, y = %f, z = %f", j, srv4.response.objects.poses[j].position.x, srv4.response.objects.poses[j].position.y, srv4.response.objects.poses[j].position.z);
			}
		}
		else{
		...
		if(srv4.response.objects.poses.empty() != true){
			...
			char temp_filename[256];	sprintf(temp_filename, "'/home/pcl_data/%lu_%d_robot%d.pcd'", srv04.request.rostime,srv04.request.id,srv1.request.robot_id);
			srv04.request.filename = std::string(temp_filename);	std::cout << srv04.request.filename << std::endl;
...
//			}
			...
			ROS_INFO("move_x = %lf",L*cos(srv02.response.robots_theta*(3.141592/180) + phi) -
					(sqrt(pow(object_x,2)+pow(object_z,2)))*cos(srv02.response.robots_theta*(3.141592/180)-atan2(object_x, object_z)));
			ROS_INFO("move_y = %lf",L*sin(srv02.response.robots_theta*(3.141592/180) + phi) -
					(sqrt(pow(object_x,2)+pow(object_z,2)))*sin(srv02.response.robots_theta*(3.141592/180)-atan2(object_x, object_z)));

			//移動先x座標
			temp_command.x = srv02.response.robots_x - (L*cos(srv02.response.robots_theta*(3.141592/180) + phi) -
					(sqrt(pow(object_x,2)+pow(object_z,2)))*cos(srv02.response.robots_theta*(3.141592/180)-atan2(object_x, object_z)));
			//移動先y座標
			temp_command.y = srv02.response.robots_y - (L*sin(srv02.response.robots_theta*(3.141592/180) + phi) -
					(sqrt(pow(object_x,2)+pow(object_z,2)))*sin(srv02.response.robots_theta*(3.141592/180)-atan2(object_x, object_z)));
			//移動先θ
			...
			ROS_INFO("([robot_now]%lf,%lf,%lf,[calc_temp]%lf,%lf,[robot_to_object]%lf,%lf,[Robot_move]%lf,%lf,%lf)",
					srv02.response.robots_x,srv02.response.robots_y,srv02.response.robots_theta,
					L,phi,object_x,object_z,temp_command.x,temp_command.y,temp_command.th);
			...
			ROS_INFO("object_x_abs = %f",
					srv02.response.robots_x + object_z*cos(srv02.response.robots_theta*(3.141592/180)) - object_x * sin((-1)*(srv02.response.robots_theta*(3.141592/180))));
			ROS_INFO("object_y_abs = %f",
					srv02.response.robots_y + object_z*sin(srv02.response.robots_theta*(3.141592/180)) - object_x * cos((-1)*(srv02.response.robots_theta*(3.141592/180))));
...
//		}
		...
		ucTagData_sprh.resize(8);ucTagData_sprh[7] = 0;
