...
void calc_goal_pos(double x1, double y1, double x2, double y2, lpose goal_array[]){
	...
	if(fileflag){
		fprintf(fp,
				...
				"%lf,%lf,%lf\n"
				"%lf,%lf,%lf\n\n",
				goal_array[0].x,goal_array[0].y,goal_array[0].th,
				goal_array[1].x,goal_array[1].y,goal_array[1].th,
				goal_array[2].x,goal_array[2].y,goal_array[2].th,
				goal_array[3].x,goal_array[3].y,goal_array[3].th,
				goal_array[4].x,goal_array[4].y,goal_array[4].th,
				goal_array[5].x,goal_array[5].y,goal_array[5].th);
		fclose(fp);
	}

}

bool tms_rs_fall_detection_func(tms_msg_sa::tms_to_activate_service::Request &req, tms_msg_sa::tms_to_activate_service::Response &res){
	...
	if(client01.call(srv01)){
		ROS_INFO("robotid=%u:(%f,%f,%f),state=%u,place=%u",
				srv01.request.robots_id,
				srv01.response.robots_x,srv01.response.robots_y,srv01.response.robots_theta,
				srv01.response.robots_state,srv01.response.robots_place);
	}
	else{
	...
	srv1.request.pangle = ((atan2(cluster_central_y-srv01.response.robots_y, cluster_central_x-srv01.response.robots_x)*180)/M_PI)-srv01.response.robots_theta;
...
//	goto CALC_ROBOT_GOAL;
	...
	if(client4.call(srv4)){
		ROS_INFO("p1(%f,%f,%f), p2(%f,%f,%f)",
				srv4.response.p1_x,srv4.response.p1_y,srv4.response.p1_z,
...
//すなわち、このif文に入るのはodd patternである
	if( sqrt(((pose[0].x-srv01.response.robots_x)*(pose[0].x-srv01.response.robots_x))+((pose[0].y-srv01.response.robots_y)*(pose[0].y-srv01.response.robots_y))) >
	sqrt(((pose[3].x-srv01.response.robots_x)*(pose[3].x-srv01.response.robots_x))+((pose[3].y-srv01.response.robots_y)*(pose[3].y-srv01.response.robots_y))) ){
		ROS_INFO("odd pattern was occer");
		lpose temp;
		temp = pose[0];pose[0]=pose[3];pose[3]=temp;
		temp = pose[1];pose[1]=pose[4];pose[4]=temp;
		temp = pose[2];pose[2]=pose[5];pose[5]=temp;
	}
...
//			goto SKINCOLOR_C;
			if(abs((srv3.response.rps_path.front().rps_route[i+1].th - srv3.response.rps_path.front().rps_route[i].th)) < 0.01){	//回転角0.01度未満であれば直進動作用とみなす
				srv1.request.command = 1;	//直進
				srv1.request.pdist = sqrt((srv3.response.rps_path.front().rps_route[i+1].x-srv3.response.rps_path.front().rps_route[i].x)*(srv3.response.rps_path.front().rps_route[i+1].x-srv3.response.rps_path.front().rps_route[i].x)
						+((srv3.response.rps_path.front().rps_route[i+1].y-srv3.response.rps_path.front().rps_route[i].y)*(srv3.response.rps_path.front().rps_route[i+1].y-srv3.response.rps_path.front().rps_route[i].y)));
...
//				}
				...
				srv1.request.pangle = srv3.response.rps_path.front().rps_route[i+1].th - srv3.response.rps_path.front().rps_route[i].th;
...
//		goto FACE_DETECT;
		...
		if(srv5.response.direction == 2){	//肌色検出が左
			...
			if(directmove == false){
				...
				ROS_INFO("DIRECT MOVE? / 0:NOT MOVE else:MOVE");scanf("%d",&directmove_int);
				if(directmove != 0){
					...
					srv1.request.command = 2;
					srv1.request.pangle = ((atan2(pose[G_i+1].y-pose[G_i].y, pose[G_i+1].x-pose[G_i].x)*180)/M_PI)-srv01.response.robots_theta;
...
//			scanf("%u", &temp);
			...
			if(srv6.response.result == 1){	//顔検出：顔が見つかった場合
				ROS_INFO("face detected.(G1-G2/G4-G5);pose[%d]",G_i+1);
				find_person = true;
				break;
			}
			else{	//顔検出：顔が見つからなかった場合
				...
				if(directmove == false){
					...
					if(directmove_int != 0){
						...
						srv1.request.command = 2;
						srv1.request.pangle = ((atan2(pose[G_i+2].y-pose[G_i+1].y, pose[G_i+2].x-pose[G_i+1].x)*180)/M_PI)-srv01.response.robots_theta;
						...
						srv1.request.command = 1;
						srv1.request.pdist = sqrt( pow(pose[G_i+2].x-pose[G_i+1].x,2) + pow(pose[G_i+2].y-pose[G_i+1].y,2) );
						ROS_INFO("go robot : %f", srv1.request.pdist);
						if(client1.call(srv1)){
						}
						else{
		...
		else{	//肌色検出が右(srv5.response.direction == 1)
			...
			if(directmove == false){
				...
				if(directmove_int != 0){
					...
					srv1.request.command = 2;
					srv1.request.pangle = ((atan2(pose[G_i+2].y-pose[G_i].y, pose[G_i+2].x-pose[G_i].x)*180)/M_PI)-srv01.response.robots_theta;
			...
			if(srv6.response.result == 1){	//顔検出：顔が見つかった場合
				...
				if(directmove == false){
					...
					if(directmove != 0){
						...
						srv1.request.command = 2;
						srv1.request.pangle = ((atan2(pose[G_i+1].y-pose[G_i+2].y, pose[G_i+1].x-pose[G_i+2].x)*180)/M_PI)-srv01.response.robots_theta;
						...
						srv1.request.command = 1;
						srv1.request.pdist = sqrt( pow(pose[G_i+1].x-pose[G_i+2].x,2) + pow(pose[G_i+1].y-pose[G_i+2].y,2) );
						ROS_INFO("go robot : %f", srv1.request.pdist);
						if(client1.call(srv1)){
						}
						else{
