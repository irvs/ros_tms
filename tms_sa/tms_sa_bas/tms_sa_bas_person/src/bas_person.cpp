...
// output stream
time_t now;
std::ofstream ofs1;	char filename1[256]; 	// for NearChairData and chair position
std::ofstream ofs2;	char filename2[256]; 	// for NearBedData and bed position
std::ofstream ofs3;	char filename3[256]; 	// for GaitCycleData
std::ofstream ofs4;	char filename4[256]; 	// for HumanBehaviorData
std::ofstream ofs5;	char filename5[256];		// for FootPrintListData
std::ofstream ofs6;	char filename6[256];		// for FootPrintAllData
std::ofstream ofs7;	char filename7[256];		// for DetectedClusterListData
...
void estimateWalkingBehavior()
{
	...
	double threshold;
	if(WALKINGMODEL_METHOD == 1)		threshold = human.baseGaitCycleByAll + OUTLIER_N*sqrt(human.baseGaitCycleVarByAll);
	else if(WALKINGMODEL_METHOD == 2)	threshold = (double)POPULAR_GAITCYCLE;
		...
		for(unsigned int j=0 ; j<m_msgPersonTrajectory.trajectory[i].fCenterX.size() ; j++){
			...
			if(j==0){
				if(i==0){
					...
					// middle of pos
					float middleX	= (m_msgPersonTrajectory.trajectory[i].fCenterX[j] + m_msgPersonTrajectory.trajectory[i].fCenterX.back())/2.0;
					float middleY	= (m_msgPersonTrajectory.trajectory[i].fCenterY[j] + m_msgPersonTrajectory.trajectory[i].fCenterY.back())/2.0;
...
void estimateSittingBehavior()
{
	...
	for(unsigned int i=0 ; i<nearChairList.size() ; i++){
		...
		if(centerX == 0 && centerY == 0){
			...
			if(1000 <= centerX && centerX <= 1750 &&
			   1500 <= centerY && centerY <= 2250){
...
bool isExit(int t)
{
	...
	for(unsigned int i=0 ; i<walkingHistory.size() ; i++){
		if(	int(walkingHistory[i].sTime) <= t && t <= int(walkingHistory[i].eTime)){
				...
				if(	(0 <= fCenterX && fCenterX <= 2500) &&
					(0 <= fCenterY && fCenterY <= 500)){
					isExit = true;
					break;
				}
		}
	...
	case BEHAVIOR_WALKING:
		...
		case BEHAVIOR_LOST:
			...
			case BEHAVIOR_LOST:
				switch (sleepingBehavior) {
				case BEHAVIOR_LOST:
					if(isExit(t))	mergedBehavior = BEHAVIOR_LOST;
					else			mergedBehavior = BEHAVIOR_WALKING;
					break;
				case BEHAVIOR_SITTING_B:
					if(isExit(t))	mergedBehavior = BEHAVIOR_LOST;
					else			mergedBehavior = BEHAVIOR_WALKING;
	...
	case BEHAVIOR_STAY_F:
		...
		case BEHAVIOR_LOST:
			...
			case BEHAVIOR_LOST:
				...
				case BEHAVIOR_LOST:
					if(isExit(t))	mergedBehavior = BEHAVIOR_LOST;
					else			mergedBehavior = BEHAVIOR_STAY_F;
					break;
				case BEHAVIOR_SITTING_B:
					mergedBehavior = BEHAVIOR_SITTING_B;
					break;
...
bool isNearBed(float bedX, float bedY, float centerX, float centerY)
{
	...
	// Now bed's position is stable.
	if(
		bedX-1500 <= centerX && centerX <= bedX+1500 &&
		bedY-1000 <= centerY && centerY <= bedY+1000
		){
		isNearBed = true;
	}
	else{
...
void displayWalkingHistory()
{
	printf("\n-----------------------------------------\n");
	for(unsigned int i=0 ; i<walkingHistory.size() ; i++){
		printf("%lf\t%lf:\t%d\t%f\t%f\n", walkingHistory[i].sTime, walkingHistory[i].eTime, walkingHistory[i].behavior, walkingHistory[i].fCenterX.back(), walkingHistory[i].fCenterY.back());
	}
}
...
void outputNearChairData()
{
	...
	ofs1 << "sTime" << "\t" << "eTime" << "\t" << "fCenterX" << "\t" << "fCenterY" << std::endl;
	for(unsigned int i=0 ; i<m_msgChairBuffer.tStartTime.size() ; i++){
	...
	ofs1 << "sTime" << "\t" << "eTime" << "\t" << "sensingTime" << "\t" << "fCenterX" << "\t" << "fCenterY" << std::endl;
...
void outputNearBedData()
{
	...
	ofs2 << "sTime" << "\t" << "eTime" << "\t" << "sensingTime" << std::endl;
...
void outputGaitCycleData()
{
	if(WALKINGMODEL_METHOD == 1){
		...
		for(unsigned int i=0 ; i<human.gaitCycleData.size() ; i++){
			...
			ofs3 << "---------------------------------------" << std::endl;
			ofs3 << "gaitCycle" << "\t" << "time[sec]" << "\t" << "isValid" << "\t" << "μ+2σ" << std::endl;
			...
			ofs3 << "----------" << std::endl;
			ofs3 << "gaitCycle Average" << "\t" << human.baseGaitCycle[i] << std::endl;
			ofs3 << "gaitCycle Var" << "\t" << human.baseGaitCycleVar[i] << std::endl;
			ofs3 << "gaitCycle Sigma" << "\t" << sqrt(human.baseGaitCycleVar[i]) << std::endl;
			ofs3 << std::endl;
		}
		...
		ofs3 << "----------" << std::endl;
		ofs3 << "gaitCycle Average" << "\t" << human.baseGaitCycleByAll << std::endl;
		ofs3 << "gaitCycle Var" << "\t" << human.baseGaitCycleVarByAll << std::endl;
		ofs3 << "gaitCycle Sigma" << "\t" << sqrt(human.baseGaitCycleVarByAll) << std::endl;
		ofs3 << std::endl;
	}
}
...
void outputStepCycleData()
{
	if(WALKINGMODEL_METHOD == 1){
		for(unsigned int i=0 ; i<human.stepCycleData.size() ; i++){
			...
			ofs3 << "humanTrajectory[" << i << "]" << std::endl;
			ofs3 << "---------------------------------------" << std::endl;
			ofs3 << "stepCycle" << "\t" << "time[sec]" << "\t" << "isValid" << std::endl;
			...
			ofs3 << "----------" << std::endl;
			ofs3 << "stepCycle Average" << "\t" << human.baseStepCycle[i] << std::endl;
			ofs3 << "stepCycle Var" << "\t" << human.baseStepCycleVar[i] << std::endl;
			ofs3 << "stepCycle Sigma" << "\t" << sqrt(human.baseStepCycleVar[i]) << std::endl;
			ofs3 << std::endl;
		}
		...
		ofs3 << "----------" << std::endl;
		ofs3 << "stepCycle Average" << "\t" << human.baseStepCycleByAll << std::endl;
		ofs3 << "stepCycle Var" << "\t" << human.baseStepCycleVarByAll << std::endl;
		ofs3 << "stepCycle Sigma" << "\t" << sqrt(human.baseStepCycleVarByAll) << std::endl;
		ofs3 << std::endl;
	}
}
...
void outputFootPrintListData()
{
	...
	ofs5 << "no" << "\t" << "sTime" << "\t" << "eTime" << "\t" << "sensingTime" << "\t" << "fCenterX" << "\t" << "fCenterY" << std::endl;
	...
	for(unsigned int i=0 ; i<m_msgPersonTrajectory.trajectory.size() ; i++){
		for(unsigned int j=0 ; j<m_msgPersonTrajectory.trajectory[i].fCenterX.size() ; j++){
			...
			ofs5 << index << "\t" << getTime(sTime) << "\t" << getTime(eTime) << "\t" << sensingTime << "\t" << fCenterX << "\t" << fCenterY << std::endl;
		}
		ofs5 << std::endl;
	}
	ofs5 << std::endl;
...
void outputDetectedClusterListData()
{
	ofs7 << "sTime" << "\t" << "eTime" << "\t" << "sensingTime" << "\t" << "fCenterX" << "\t" << "fCenterY" << std::endl;
	for(unsigned int i=0 ; i<m_msgDetectedClusterList.cluster.size() ; i++){
		...
		ofs7 << getTime(sTime) << "\t" << getTime(eTime) << "\t" << sensingTime << "\t" << fCenterX << "\t" << fCenterY << std::endl;
	}
	ofs7 << std::endl;
}
