...
bool tms_utility_analyze_missing_objects_func(tms_msg_sa::tms_to_activate_service::Request &req, tms_msg_sa::tms_to_activate_service::Response &res){
	...
	if(client1.call(srv1)){
		for(unsigned int i=0;i<srv1.response.id.size();i++){
			ROS_INFO("Success! [%d] rostime=%lu,id=%d, x=%f, y=%f, z=%f, weight=%f, state=%d, place=%d",
					i, srv1.response.rostime[i], srv1.response.id[i],
					srv1.response.x[i], srv1.response.y[i], srv1.response.z[i],
					srv1.response.weight[i], srv1.response.state[i], srv1.response.place[i]);
		}
	}
	else{
	...
	if(client2.call(srv2)){
		for(unsigned int i=0;i<srv2.response.id.size();i++){
			ROS_INFO("Success! [%d] rostime=%lu,id=%d,weight=%f,state=%d,place=%d",
					i, srv2.response.rostime[i], srv2.response.id[i],
					srv2.response.weight[i], srv2.response.state[i], srv2.response.place[i]);
		}
	}
	else{
	...
	for(unsigned int i=0;i<srv2.response.id.size();i++){
		printf("%d\n",i);
		if(srv2.response.place[i] == 0){	//物品の最新状態が0(=紛失)の場合、missing_listへ挿入
	...
	for(unsigned int i=0;i<missing_objects_list.size();i++){
		printf("missing_objects_id=%lu, missing_objects_id=%d\n", missing_objects_list[i].missing_object_time, missing_objects_list[i].missing_object_id);
	}
	...
	for(while_i = 0; while_i < missing_objects_list.size(); while_i++){
		if(missing_objects_list[while_i].missing_object_id_valid == true){	//他の物品探索の結果、ついでに見つかった物品はfalseになる
			ROS_INFO("34");
			...
			if(client3.call(srv3)){
				for(unsigned int i=0;i<srv3.response.id.size();i++){
					ROS_INFO("Success! [%d] rostime=%lu,id=%d,x=%f,y=%f,z=%f,theta=%f,state=%d,place=%d",
							i, srv3.response.rostime[i], srv3.response.id[i],
							srv3.response.x[i], srv3.response.y[i], srv3.response.z[i], srv3.response.theta[i],
							srv3.response.state[i], srv3.response.place[i]);
...
//					}
			...
			for(while_j=0;while_j < missing_objects_list[while_i].person_history.size();while_j++){
				ROS_INFO("1");
//              //bed12に近づいていればセット
//				ROS_INFO("desk12?:%lf",sqrt(pow((missing_objects_list[while_i].person_history[while_j].x - bed12_x),2) + pow((missing_objects_list[while_i].person_history[while_j].y - bed12_y),2)));
//				if(sqrt(pow((missing_objects_list[while_i].person_history[while_j].x - bed12_x),2) + pow((missing_objects_list[while_i].person_history[while_j].y - bed12_y),2))
...
//					else{
//						ROS_INFO("1-2");
//						temp_furniture.left_time = missing_objects_list[while_i].person_history[while_j].rostime+(1*1000*1000*1000);
//					}
//					missing_objects_list[while_i].near_furnitures.push_back(temp_furniture);
//				}

				//desk13に近づいていればセット
				ROS_INFO("desk13?:%lf",sqrt(pow((missing_objects_list[while_i].person_history[while_j].x - desk13_x),2) + pow((missing_objects_list[while_i].person_history[while_j].y - desk13_y),2)));
				if(sqrt(pow((missing_objects_list[while_i].person_history[while_j].x - desk13_x),2) + pow((missing_objects_list[while_i].person_history[while_j].y - desk13_y),2))
						< threshold_furnitures){
					ROS_INFO("2:while_j = %d",while_j);
				...
				//table14に近づいていればセット
				ROS_INFO("table14?:%lf",sqrt(pow((missing_objects_list[while_i].person_history[while_j].x - table14_x),2) + pow((missing_objects_list[while_i].person_history[while_j].y - table14_y),2)));
				if(sqrt(pow((missing_objects_list[while_i].person_history[while_j].x - table14_x),2) + pow((missing_objects_list[while_i].person_history[while_j].y - table14_y),2))
						< threshold_furnitures){
			...
			for(unsigned int i=0;i<missing_objects_list[while_i].near_furnitures.size();i++){
				if(missing_objects_list[while_i].near_furnitures[i].id == 13){
					desk13_sum += missing_objects_list[while_i].near_furnitures[i].left_time - missing_objects_list[while_i].near_furnitures[i].near_time;
				}
				if(missing_objects_list[while_i].near_furnitures[i].id == 14){
					table14_sum += missing_objects_list[while_i].near_furnitures[i].left_time - missing_objects_list[while_i].near_furnitures[i].near_time;
				}
			}
		...
		for(unsigned int j=0;j<missing_objects_list[i].near_furnitures_total.size();j++){
			for(unsigned int k=0;k<furnitures_order.size();k++){
				...
				if(furnitures_order[k].id == missing_objects_list[i].near_furnitures[j].id){
					furnitures_order[k].total_time	+= missing_objects_list[i].near_furnitures[j].left_time - missing_objects_list[i].near_furnitures[j].near_time;
				}
			}
		}
	}
	...
	//srvへ挿入(furnitures_orderのうちtotal_time == 0のものは除く) <- これは近づいていない、すなわち探す対象から省かれるため
	for(unsigned int i=0;i<furnitures_order.size();i++){
...
void callback1(const ros::TimerEvent&)
{
	...
	if(client01.call(srv01)){
		for(unsigned int i=0;i<srv01.response.id.size();i++){
			ROS_INFO("Success! [%d] rostime=%lu,id=%d, x=%f, y=%f, z=%f, weight=%f, state=%d, place=%d",
					i, srv01.response.rostime[i], srv01.response.id[i],
					srv01.response.x[i], srv01.response.y[i], srv01.response.z[i],
					srv01.response.weight[i], srv01.response.state[i], srv01.response.place[i]);
		}
	}
	else{
		ROS_ERROR("Failed to call service tmsdb_get_objects_info_present");
	...
	if(client02.call(srv02)){
		for(unsigned int i=0;i<srv02.response.id.size();i++){
			ROS_INFO("Success! [%d] rostime=%lu,id=%d,weight=%f,state=%d,place=%d",
					i, srv02.response.rostime[i], srv02.response.id[i],
					srv02.response.weight[i], srv02.response.state[i], srv02.response.place[i]);
		}
	}
	else{
		ROS_ERROR("Failed to call service tmsdb_get_objects_info_history");
						...
						for(unsigned int j=0;j<srv03.response.x.size();j++){
							...
							if(   map[srv03.response.x[j]/block_size][srv03.response.y[j]/block_size].near_bed_flag   ){
								...
								if((srv03.response.x.size() - j) != 1){
									missing_table[srv02.response.id[i]-objects_start_number][0] += (srv03.response.rostime[j+1]-srv03.response.rostime[j]);
									//								ROS_INFO("1-1");
								}
							...
							if(   map[srv03.response.x[j]/block_size][srv03.response.y[j]/block_size].near_desk_flag   ){
								//							ROS_INFO("2");
								if((srv03.response.x.size() - j) != 1){
									missing_table[srv02.response.id[i]-objects_start_number][1] += (srv03.response.rostime[j+1]-srv03.response.rostime[j]);
									//								ROS_INFO("2-1");
								}
							...
							if(   map[srv03.response.x[j]/block_size][srv03.response.y[j]/block_size].near_table_flag   ){
								//							ROS_INFO("3");
								if((srv03.response.x.size() - j) != 1){
									missing_table[srv02.response.id[i]-objects_start_number][2] += (srv03.response.rostime[j+1]-srv03.response.rostime[j]);
									//								ROS_INFO("3-1");
								}
								else{
			...
			for(unsigned int i=0;i<srv02.response.id.size();i++){
				...
				if(srv02.response.state[i] == 0){
					...
					if(client03.call(srv03)){
						...
						//								srv03.response.rostime[j], srv03.response.id[j], srv03.response.x[j], srv03.response.y[j]);
						//					}
...
//	client3 = n.serviceClient<tmsdb::tmsdb_modify_missing_objects>("tmsdb_modify_missing_objects");
	...
	for(unsigned int i=1;i<map.size()-1;i++){
		for(unsigned int j=1;j<map[i].size()-1;j++){
			if(map[i][j].bed == false){
				map[i][j].bed_distance =std::min(std::min(map[i-1][j].bed_distance+block_size, map[i][j-1].bed_distance+block_size), map[i][j].bed_distance);
			}
			if(map[i][j].desk == false){
				map[i][j].desk_distance =std::min(std::min(map[i-1][j].desk_distance+block_size, map[i][j-1].desk_distance+block_size), map[i][j].desk_distance);
			}
			if(map[i][j].table == false){
				map[i][j].table_distance =std::min(std::min(map[i-1][j].table_distance+block_size, map[i][j-1].table_distance+block_size), map[i][j].table_distance);
			}
		}
	}
	//距離計算(ラスタ走査"帰り")
	for(unsigned int i=map.size()-2;i>0;i--){
		for(unsigned int j=map[i].size()-2;j>0;j--){
			if(map[i][j].bed == false){
				map[i][j].bed_distance =std::min(std::min(map[i][j].bed_distance, map[i][j+1].bed_distance+block_size), map[i+1][j].bed_distance+block_size);
			}
			if(map[i][j].desk == false){
				map[i][j].desk_distance =std::min(std::min(map[i][j].desk_distance, map[i][j+1].desk_distance+block_size), map[i+1][j].desk_distance+block_size);
			}
			if(map[i][j].table == false){
				map[i][j].table_distance =std::min(std::min(map[i][j].table_distance, map[i][j+1].table_distance+block_size), map[i+1][j].table_distance+block_size);
			}
		}
	}
	...
	for(unsigned int i=1;i<map.size()-1;i++){
		for(unsigned int j=1;j<map[i].size()-1;j++){
			if(!map[i][j].bed){
				map[i][j].bed_distance_d =	std::min( std::min( std::min(map[i-1][j-1].bed_distance_d+sqrt(2.0)*block_size, map[i][j-1].bed_distance_d+1.0*block_size), std::min(map[i-1][j+1].bed_distance_d+sqrt(2.0)*block_size, map[i-1][j].bed_distance_d+1.0*block_size)), map[i][j].bed_distance_d);
			}
			if(!map[i][j].desk){
				map[i][j].desk_distance_d =	std::min( std::min( std::min(map[i-1][j-1].desk_distance_d+sqrt(2.0)*block_size, map[i][j-1].desk_distance_d+1.0*block_size), std::min(map[i-1][j+1].desk_distance_d+sqrt(2.0)*block_size, map[i-1][j].desk_distance_d+1.0*block_size)), map[i][j].desk_distance_d);
			}
			if(!map[i][j].table){
				map[i][j].table_distance_d = std::min( std::min( std::min(map[i-1][j-1].table_distance_d+sqrt(2.0)*block_size, map[i][j-1].table_distance_d+1.0*block_size), std::min(map[i-1][j+1].table_distance_d+sqrt(2.0)*block_size, map[i-1][j].table_distance_d+1.0*block_size)), map[i][j].table_distance_d);
			}
		}
	}
	for(int i=map.size()-2;i>0;i--){
		for(int j=map[i].size()-2;j>0;j--){
			if(!map[i][j].bed){
				map[i][j].bed_distance_d = std::min( std::min( std::min(map[i+1][j].bed_distance_d+1.0*block_size, map[i+1][j-1].bed_distance_d+sqrt(2.0)*block_size), std::min(map[i][j+1].bed_distance_d+1.0*block_size, map[i+1][j+1].bed_distance_d+sqrt(2.0)*block_size)), map[i][j].bed_distance_d);
			}
			if(!map[i][j].desk){
				map[i][j].desk_distance_d = std::min( std::min( std::min(map[i+1][j].desk_distance_d+1.0*block_size, map[i+1][j-1].desk_distance_d+sqrt(2.0)*block_size), std::min(map[i][j+1].desk_distance_d+1.0*block_size, map[i+1][j+1].desk_distance_d+sqrt(2.0)*block_size)), map[i][j].desk_distance_d);
			}
			if(!map[i][j].table){
				map[i][j].table_distance_d = std::min( std::min( std::min(map[i+1][j].table_distance_d+1.0*block_size, map[i+1][j-1].table_distance_d+sqrt(2.0)*block_size), std::min(map[i][j+1].table_distance_d+1.0*block_size, map[i+1][j+1].table_distance_d+sqrt(2.0)*block_size)), map[i][j].table_distance_d);
			}
		}
	}
