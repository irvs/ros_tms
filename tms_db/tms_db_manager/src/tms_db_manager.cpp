...
// @author : Yoonseok Pyo
// @version: Ver0.1.6 (since 2014.05.07)
// @date   : 2014.05.20
//------------------------------------------------------------------------------
//include for ROS
#include <ros/ros.h>
...
private:
  ...
  //----------------------------------------------------------------------------
  bool StoreMasterData(int32_t target_id)
  {
    char select_query[1024];
    char delete_query[1024];
    char insert_query[1024];

    ros::Time standard_time = ros::Time::now() + ros::Duration(9*60*60) - ros::Duration(1*60*60); // GMT +9 -1
    string std_time = boost::posix_time::to_iso_extended_string(standard_time.toBoost());
