...
class TmsDbBackuper():
    ...
    def manageDataCallback(self, event):
        # GMT +9 -1 hour
        nowtime = rospy.Time.now() + rospy.Duration(
            9 * 60 * 60) - rospy.Duration(1 * 60 * 60)
    ...
    def removeForeverDataCallback(self, event):
        # GMT +9hour - 14day
        nowtime = rospy.Time.now() + rospy.Duration(
            9 * 60 * 60) - rospy.Duration(14 * 24 * 60 * 60)
        update_period = str(datetime.utcfromtimestamp(nowtime.to_sec()))
        # print(update_period)
...
# db.getCollection('history').find().sort({'time':-1}) e.g. 1 for
