...
class TmsDbReader():
    ...
    def __init__(self):
        ...
        self.db_reader_srv = rospy.Service(
            'tms_db_reader', TmsdbGetData, self.dbReaderSrvCallback)
    ...
    def dbReaderSrvCallback(self, req):
        ...
        if mode == MODE_ALL:
            ...
            cursor = db.now.find(
                {'name': req.tmsdb.name, 'sensor': req.tmsdb.sensor})
            ...
            cursor = db.now.find(
                {'id': req.tmsdb.id, 'sensor': req.tmsdb.sensor})
        elif mode == MODE_ID_SENSOR_STATE:
            cursor = db.now.find(
                {'id': req.tmsdb.id, 'sensor': req.tmsdb.sensor, 'state': 1})
