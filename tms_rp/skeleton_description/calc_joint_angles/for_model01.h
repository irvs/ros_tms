...
double Teta_SL, Phi_SL, Teta_SL2, Teta_EL, Teta_SR, Phi_SR, Teta_SR2, Teta_ER, Teta_HL, Phi_HL, Teta_HL2, Teta_KL, Teta_HR, Phi_HR, Teta_HR2, Teta_KR;
...
//	ROS_INFO ("SM: %f %f %f\n", temp_vec3(0), temp_vec3(1), temp_vec3(2)); 
	...
	T_WtoSM << 
		rot_WtoSM(0,0), rot_WtoSM(0,1), rot_WtoSM(0,2), temp_vec3(0),
		rot_WtoSM(1,0), rot_WtoSM(1,1), rot_WtoSM(1,2), temp_vec3(1),
		rot_WtoSM(2,0), rot_WtoSM(2,1), rot_WtoSM(2,2), temp_vec3(2),
                	     0,	             0,	             0, 	   1;
...
//	ROS_INFO ("SM: %f %f %f\n", temp_vec(0), temp_vec(1), temp_vec(2)); 
	...
	rot_SMtoSL << 1,0,0,0,1,0,0,0,1;
	temp_vec3 = rot_SMtoSL*rot_WtoSM*(j[SpineMid]-j[ShoulderLeft]);
	
//	checking	
//	ROS_INFO ("SM-SL: %f %f %f\n", temp_vec3(0), temp_vec3(1), temp_vec3(2)); 

	T_SMtoSL << 
		rot_SMtoSL(0,0), rot_SMtoSL(0,1), rot_SMtoSL(0,2), temp_vec3(0),
		rot_SMtoSL(1,0), rot_SMtoSL(1,1), rot_SMtoSL(1,2), temp_vec3(1),
		rot_SMtoSL(2,0), rot_SMtoSL(2,1), rot_SMtoSL(2,2), temp_vec3(2),
                	      0,	       0,	        0, 	      1;
	
	temp_vec3 << rot_SMtoSL*rot_WtoSM*(j[ElbowLeft]-j[ShoulderLeft]);

//	checking
//	ROS_INFO ("EL-SL: %f %f %f\n", temp_vec3(0), temp_vec3(1), temp_vec3(2)); 
	...
	rot_SLtoSL2 <<
		cos(-Teta_SL)*cos(-Phi_SL), -sin(-Teta_SL), cos(-Teta_SL)*sin(-Phi_SL),
		sin(-Teta_SL)*cos(-Phi_SL),  cos(-Teta_SL), sin(-Teta_SL)*sin(-Phi_SL),
		    		         0,              0,               cos(-Phi_SL);

	T_SLtoSL2 <<
	 	rot_SLtoSL2(0,0), rot_SLtoSL2(0,1), rot_SLtoSL2(0,2), 0,
	 	rot_SLtoSL2(1,0), rot_SLtoSL2(1,1), rot_SLtoSL2(1,2), 0,
		rot_SLtoSL2(2,0), rot_SLtoSL2(2,1), rot_SLtoSL2(2,2), 0,
                    	       0,             	 0,                0, 1;
	    ...
	    L_acf(2) != 0)
	{
		...
		Teta_SL2 = M_PI/2-atan2(L_acf(2),L_acf(1));
	...
	// transformation matrix from SL2 to EL (rotate around x_SL2 axis by Teta_SL2 and translate from SL to EL point)
	
	temp_vec3 << rot_SLtoSL2*rot_SMtoSL*rot_WtoSM*(j[ShoulderLeft]-j[ElbowLeft]);

	rot_SL2toEL <<
		1,             0,              0,
		0, cos(Teta_SL2), -sin(Teta_SL2),
	        0, sin(Teta_SL2),  cos(Teta_SL2);

	T_SL2toEL << 
		rot_SL2toEL(0,0), rot_SL2toEL(0,1), rot_SL2toEL(0,2), temp_vec3(0),
		rot_SL2toEL(1,0), rot_SL2toEL(1,1), rot_SL2toEL(1,2), temp_vec3(1),
	        rot_SL2toEL(2,0), rot_SL2toEL(2,1), rot_SL2toEL(2,2), temp_vec3(2),
	      		       0,            	 0,                0, 	 	 1;
	...
	temp_vec <<
		j[SpineMid][0]-j[ShoulderRight][0],
		j[SpineMid][1]-j[ShoulderRight][1],
		j[SpineMid][2]-j[ShoulderRight][2],
						 1;
	t1 = T_WtoSM*temp_vec;
	T_SMtoSR <<
		1, 0, 0, t1[0],
	...
	temp_vec <<
		j[ElbowRight][0]-j[ShoulderRight][0],
		j[ElbowRight][1]-j[ShoulderRight][1],
		j[ElbowRight][2]-j[ShoulderRight][2],
						   1;
