...
void getPushWagonType(tms_msg_rp::rps_position robot_pos, tms_msg_rp::rps_position wagon_pos, int& push_wagon_type, double& rel_th)
{
	push_wagon_type = 0;
	rel_th = 0.0;
...
void cut_unnecessary_path_point(vector<vector<double> > in_wagon_path, vector<vector<double> > passage_point, double threshold, vector<vector<double> >& out_wagon_path)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool start_push_wagon_path_planner(tms_msg_rp::rps_push_wagon_path_planning::Request& req, tms_msg_rp::rps_push_wagon_path_planning::Response& res)
{
	...
	for(unsigned int i=1;i<comp_path.size();i++){
		...
		calcWagonPosOnVoronoi(sub_Map, rob_pos, push_wagon_type, Robot_Control_Wagon_Dist/1000.0, rel_th, change_wagon_rel_th, Push_Wagon_Path_Threshold/1000.0);
	...
	for(unsigned int i=0;i<simple_push_wagon_path.size();i++){
		...
		temp_pos.x = ( (simple_push_wagon_path[i][0] * 1000.0) + Robot_Control_Wagon_Dist*cos(simple_push_wagon_path[i][2]) ) + (wagon_l/2.0)*cos(simple_push_wagon_path[i][2]+simple_push_wagon_path[i][3]);
		temp_pos.y = ( (simple_push_wagon_path[i][1] * 1000.0) + Robot_Control_Wagon_Dist*sin(simple_push_wagon_path[i][2]) ) + (wagon_l/2.0)*sin(simple_push_wagon_path[i][2]+simple_push_wagon_path[i][3]);
...
int main(int argc, char **argv)
{
	...
	ros::ServiceServer service_push_wagon_path = n.advertiseService("rps_push_wagon_path_planning", start_push_wagon_path_planner);
	...
	commander_to_calc_grasp_wagon_pose = n.serviceClient<tms_msg_rp::rps_cnoid_grasp_wagon_planning>("rps_cnoid_calc_grasp_wagon_pose");
