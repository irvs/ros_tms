...
void removeCollisionPos(vector<vector<CollisionMapData> > Map, vector<vector<double> > in_posList, vector<vector<double> >& out_posList){
	out_posList.clear();
...
bool unifyManipulabilityMap(vector<double> human_pos, double human_th, int human_height, char robot_LR, bool robot_useWaist, char human_LR, bool human_useWaist, string& message){	//LR_flg:0=left, 1=right
...
bool set_table_area(vector<vector<CollisionMapData> >& Map, double table_center_x, double table_center_y, double table_th, double table_size_x, double table_size_y, double table_size_z){
...
bool set_table_area_id(vector<vector<CollisionMapData> >& Map, int table_id, string& message){
	...
	if(commander_to_get_furnitures_info.call(srv_get_f_info)){
		ROS_INFO("Success target_x = %lf, y = %lf, z = %lf", srv_get_f_info.response.furniture_x, srv_get_f_info.response.furniture_y, srv_get_f_info.response.furniture_z);
		if(!set_table_area(Map, srv_get_f_info.response.furniture_x, srv_get_f_info.response.furniture_y, deg2rad(table_th), table_size_x, table_size_y, table_size_z)){
...
bool set_person_view(vector<vector<CollisionMapData> >& Map, double person_x, double person_y, double person_th){
	...
	if(person_th>=0)	person_th -= (int)(person_th/PI)*PI;
	else	person_th += (int)(-person_th/PI)*PI;
	cout<<person_th<<endl;
	for(int x=0;x<Map.size();x++){
		for(int y=0;y<Map[x].size();y++){
...
bool set_person_view_id(vector<vector<CollisionMapData> >& Map, int person_id, vector<double>& person_pos, double& person_th, string& message){
	...
	if((srv_get_pb_info.response.behavior==1)||(srv_get_pb_info.response.behavior==2)||(srv_get_pb_info.response.behavior==3)){
		if(commander_to_get_person_info.call(srv_get_p_info)){
			ROS_INFO("Success person_id:%u, x = %f, y = %f, th = %f", srv_get_p_info.response.id[0], srv_get_p_info.response.x[0], srv_get_p_info.response.y[0], srv_get_p_info.response.theta[0]);
			if(!set_person_view(Map, srv_get_p_info.response.x[0], srv_get_p_info.response.y[0], deg2rad(srv_get_p_info.response.theta[0]))){
	...
	if(srv_get_pb_info.response.behavior==4){
		...
		if(commander_to_get_movable_furnitures_info.call(srv_get_f_info)){
			ROS_INFO("Success chair_x = %f, y = %f, th = %f", srv_get_f_info.response.furniture_x, srv_get_f_info.response.furniture_y, srv_get_f_info.response.furnitures_theta);
			if(!set_person_view(Map, srv_get_f_info.response.furniture_x, srv_get_f_info.response.furniture_y, deg2rad(srv_get_f_info.response.furnitures_theta))){
				message = "person position is out of Map";
				ROS_ERROR(message.c_str());
			}
...
bool dividePosList(unsigned int k, vector<vector<double> > in_posList, vector<vector<vector<double> > >& out_posList, string& message){
	if(k<0){
...
bool calcRobotPos_GiveObj(unsigned int rank, vector<vector<double> >& out_posList, vector<double>& object_pos, vector<double>& robot_joint_angle, string& message){
	out_posList.clear();
	object_pos.clear();
	...
	for(double i=0;i<2*M_PI;i+=deg2rad(1.0)){
		...
		PalmPos[0] = BasePos[0] + ( manip_Map[rank].robot_palm_pos_rel[0]*cos(BasePos[2]) - manip_Map[rank].robot_palm_pos_rel[1]*sin(BasePos[2]) );
		PalmPos[1] = BasePos[1] + ( manip_Map[rank].robot_palm_pos_rel[0]*sin(BasePos[2]) + manip_Map[rank].robot_palm_pos_rel[1]*cos(BasePos[2]) );
...
bool start_rps_goal_planner(tms_msg_rp::rps_goal_planning::Request& req, tms_msg_rp::rps_goal_planning::Response& res)
{
	...
	start.clear();start.resize(3);
	target.clear();target.resize(3);
	...
	if(res.goal_pos.size()!=0){
		for(int i=0;i<res.goal_pos.size();i++){
			cout<<"goal_candidate:"<<"	no."<<i+1<<"	x:"<<res.goal_pos[i].x<<" y:"<<res.goal_pos[i].y<<" th:"<<res.goal_pos[i].th<<endl;
		}
	}
	else{
...
bool start_grasp_wagon_pos_planner(tms_msg_rp::rps_goal_planning::Request& req, tms_msg_rp::rps_goal_planning::Response& res)
{
	...
	if(commander_to_get_movable_furnitures_info.call(srv_get_f_info)){
		ROS_INFO("Success wagon_x = %lf, y = %lf, th = %lf", srv_get_f_info.response.furniture_x, srv_get_f_info.response.furniture_y, srv_get_f_info.response.furnitures_theta);
	...
	if(srv_get_f_info.response.furnitures_state==1){	//if wagon exist(stop) : state = 1
	...
	for(int i=0;i<4;i++){
		if(i%2==0){
			temp_pos[0] = wagon_pos.x + ( Wagon_Size_LongSide_Length/2.0 + Robot_Control_Wagon_Dist )*cos( wagon_pos.th + (i/2.0)*M_PI );
			temp_pos[1] = wagon_pos.y + ( Wagon_Size_LongSide_Length/2.0 + Robot_Control_Wagon_Dist )*sin( wagon_pos.th + (i/2.0)*M_PI );
		}
		else{
			temp_pos[0] = wagon_pos.x + ( Wagon_Size_ShortSide_Length/2.0 + Robot_Control_Wagon_Dist )*cos( wagon_pos.th + (i/2.0)*M_PI );
			temp_pos[1] = wagon_pos.y + ( Wagon_Size_ShortSide_Length/2.0 + Robot_Control_Wagon_Dist )*sin( wagon_pos.th + (i/2.0)*M_PI );
		}
...
bool start_give_obj_pos_planner(tms_msg_rp::rps_goal_planning::Request& req, tms_msg_rp::rps_goal_planning::Response& res)
{
	...
	if(!unifyManipulabilityMap(person_pos, person_th, person_height, useRobotHand, useRobotWaist, useHumanHand, useHumanWaist, res.message))
		return false;
	...
	for(unsigned int i=0;i<divide_giveObjPos.size();i++){
		...
		//~ for(unsigned int j=0;j<give_ObjPosData[i].size();j++){
			//~ cout<<give_ObjPosData[i][j].robotPos[0]<<"	"<<give_ObjPosData[i][j].robotPos[1]<<"	"<<give_ObjPosData[i][j].robotPos[2]<<"	"<<give_ObjPosData[i][j].evaluation<<endl;
		//~ }
		//~ cout<<endl;
	}
	...
	for(unsigned int i=0;i<give_ObjPosData.size();i++){
		...
		if(!give_ObjPosData[i].empty()){
			...
			res.goal_pos.push_back(robot_pos);
			//~ cout<<robot_pos.x<<"	"<<robot_pos.y<<"	"<<robot_pos.th<<"	"<<give_ObjPosData[i][0].evaluation<<"	"<<give_ObjPosData[i][0].view<<"	"<<give_ObjPosData[i][0].human_dist<<"	"<<give_ObjPosData[i][0].obj_dist<<endl;
...
bool start_select_path(tms_msg_rp::rps_select_path::Request& req, tms_msg_rp::rps_select_path::Response& res)
{
	...
	for(unsigned int i=0;i<req.in_path.size();i++){
		...
		temp_L = temp_R = 0.0;
		temp_V = sub_Map[(int)round((temp_comp_path[0][0]-x_llimit)/cell_size)][(int)round((temp_comp_path[0][1]-y_llimit)/cell_size)].person_view;
		for(unsigned int j=1;j<temp_comp_path.size();j++){
			temp_L += fabs(getLength(temp_comp_path[j-1][0], temp_comp_path[j-1][1], temp_comp_path[j][0], temp_comp_path[j][1]));
			temp_R += fabs(temp_comp_path[j-1][2] - temp_comp_path[j][2]);
			temp_V += sub_Map[(int)round((temp_comp_path[j][0]-x_llimit)/cell_size)][(int)round((temp_comp_path[j][1]-y_llimit)/cell_size)].person_view;
		}
		L.push_back(temp_L);
		R.push_back(temp_R);
...
int main(int argc, char **argv){
	...
	ros::ServiceServer service_grasp_wagon_pos = n.advertiseService("rps_grasp_wagon_pos_planning", start_grasp_wagon_pos_planner);
	...
	commander_to_get_furnitures_info = n.serviceClient<tms_msg_db::tmsdb_get_furnitures_info>("tmsdb_get_furnitures_info");
	commander_to_get_movable_furnitures_info = n.serviceClient<tms_msg_db::tmsdb_get_movable_furnitures_info>("tmsdb_get_movable_furnitures_info");
	commander_to_get_person_behavior_info = n.serviceClient<tms_msg_db::tmsdb_get_person_behavior_info>("tmsdb_get_person_info_3");
