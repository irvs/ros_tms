...
bool start_give_obj_commander(tms_msg_rp::rps_give_obj_command::Request& req, tms_msg_rp::rps_give_obj_command::Response& res)
{
...
/////////PRM PLANNING///////////	
	...
	for(unsigned int i=0;i<rc_gwp.response.robot_joint_angle.size()-2;i++){
		...
		if(i>2){
			...
			//~ temp_state.robot_joint_angle.joint_angle[9] = temp_state.robot_joint_angle.joint_angle[17] = close_gripper_angle;
		}
...
/////////GRASP OBJECT PLANNING///////////	
	...
	rc_gop.request.pre_robot_joint_angle = req.release_wagon_Robot_joint_angle[req.release_wagon_Robot_joint_angle.size()-1];
...
/////////PRM PLANNING///////////	
	...
	rps_temp_param.rps_pos_param[12] = req.grasp_obj_Robot_joint_angle[req.grasp_obj_Robot_joint_angle.size()-1].joint_angle[9];
	rps_temp_param_array.push_back(rps_temp_param);
...
/////////PRM PLANNING///////////	
	...
	for(unsigned int i=0;i<req.give_obj_Robot_joint_angle.size();i++){
		...
		//~ rps_temp_param.rps_pos_param[12] = rc_gop.response.robot_joint_angle[rc_gop.response.robot_joint_angle.size()-1].joint_angle[9];
		rps_temp_param_array.push_back(rps_temp_param);
	}
	rps_temp_param.rps_pos_param[12] = open_gripper_angle;
	rps_temp_param_array.push_back(rps_temp_param);
