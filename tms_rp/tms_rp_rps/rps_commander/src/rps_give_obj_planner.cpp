...
int main(int argc, char **argv){
	...
	ros::ServiceClient commander_to_get_robots_info = n.serviceClient<tms_msg_db::tmsdb_get_robots_info>("tmsdb_get_robots_info");
	tms_msg_db::tmsdb_get_robots_info srv_get_r_info;
	ros::ServiceClient commander_to_get_movable_furnitures_info = n.serviceClient<tms_msg_db::tmsdb_get_movable_furnitures_info>("tmsdb_get_movable_furnitures_info");
	...
	ros::ServiceClient client_pwpp = n.serviceClient<tms_msg_rp::rps_push_wagon_path_planning>("rps_push_wagon_path_planning");
	...
	ros::ServiceClient client_rc_gop = n.serviceClient<tms_msg_rp::rps_cnoid_grasp_obj_planning>("rps_cnoid_grasp_obj_planning");
	tms_msg_rp::rps_cnoid_grasp_obj_planning rc_gop;
	ros::ServiceClient client_rc_giop = n.serviceClient<tms_msg_rp::rps_cnoid_grasp_obj_planning>("rps_cnoid_give_obj_planning");
	tms_msg_rp::rps_cnoid_grasp_obj_planning rc_giop;
	ros::ServiceClient client_rc_gwp = n.serviceClient<tms_msg_rp::rps_cnoid_grasp_wagon_planning>("rps_cnoid_grasp_wagon_planning");
	tms_msg_rp::rps_cnoid_grasp_wagon_planning rc_gwp;
	ros::ServiceClient client_rc_cgwp = n.serviceClient<tms_msg_rp::rps_cnoid_grasp_wagon_planning>("rps_cnoid_calc_grasp_wagon_pose");
	tms_msg_rp::rps_cnoid_grasp_wagon_planning rc_cgwp;
	ros::ServiceClient client_rc_rwp = n.serviceClient<tms_msg_rp::rps_cnoid_grasp_wagon_planning>("rps_cnoid_release_wagon_planning");
	...
	if(commander_to_get_movable_furnitures_info.call(srv_get_f_info)){
		ROS_INFO("Success wagon_x = %lf, y = %lf, th = %lf, width = %lf, depth = %lf, height = %lf", srv_get_f_info.response.furniture_x,srv_get_f_info.response.furniture_y,srv_get_f_info.response.furnitures_theta,srv_get_f_info.response.furnitures_width,srv_get_f_info.response.furnitures_depth,srv_get_f_info.response.furnitures_height);
	...
	if (client_gwpp.call(rgwp))
	{
		...
		if((rgwp.response.success)&&(rgwp.response.goal_pos.size()!=0)){
			for(int i=0;i<rgwp.response.goal_pos.size();i++){
				cout<< resetiosflags(ios_base::floatfield) <<"GOAL."<<i<<"	x:"<<setw(4)<<setprecision(4)<<rgwp.response.goal_pos[i].x<<"	y:"<<setw(4)<<setprecision(4)<<rgwp.response.goal_pos[i].y<<"	th:"<<fixed<<setprecision(1)<<rgwp.response.goal_pos[i].th<<endl;
			}
		}
	}
	else
		...
		if (client_vpp.call(rvpp))
		{
			...
			if((rvpp.response.success)&&(rvpp.response.VoronoiPath.size()!=0)){
				...
				for(int i=0;i<rvpp.response.VoronoiPath.size();i++){
					...
					//~ cout<< resetiosflags(ios_base::floatfield) <<"Point."<<setw(3)<<i<<"	x:"<<setw(4)<<setprecision(4)<<rvpp.response.VoronoiPath[i].x<<"	y:"<<setw(4)<<setprecision(4)<<rvpp.response.VoronoiPath[i].y<<"	th:"<<fixed<<setprecision(1)<<rvpp.response.VoronoiPath[i].th<<endl;
				}
			}
		}
		else
	...
	if (client_gopp.call(rgop))
	{
		...
		if((rgop.response.success)&&(rgop.response.goal_pos.size()!=0)){
			for(int i=0;i<rgop.response.goal_pos.size();i++){
				cout<< resetiosflags(ios_base::floatfield) <<"GOAL."<<i<<"	x:"<<setw(4)<<setprecision(4)<<rgop.response.goal_pos[i].x<<"	y:"<<setw(4)<<setprecision(4)<<rgop.response.goal_pos[i].y<<"	th:"<<fixed<<setprecision(1)<<rgop.response.goal_pos[i].th<<endl;
			}
		}
	}
	else
			...
			if (client_pwpp.call(rpwp))
			{
				...
				if((rpwp.response.success)&&(rpwp.response.RobotPath.size()!=0)){
					...
					for(int i=0;i<rpwp.response.RobotPath.size();i++){
						...
						//~ std::cout<< resetiosflags(ios_base::floatfield) <<"Point."<<setw(3)<<i<<"	x:"<<setw(4)<<setprecision(4)<<rpwp.response.RobotPath[i].x<<"	y:"<<setw(4)<<setprecision(4)<<rpwp.response.RobotPath[i].y<<"	th:"<<fixed<<setprecision(1)<<rpwp.response.RobotPath[i].th<<std::endl;
					}
					k++;
				}
			}
	...
	if(client_sp.call(rsp)){
		//~ for(unsigned int n=0;n<rsp.response.out_path.size();n++){
			//~ for(unsigned int m=0;m<rsp.response.out_path[n].rps_route.size();m++){
				//~ std::cout<< resetiosflags(ios_base::floatfield) <<"Point."<<setw(3)<<m<<"	x:"<<setw(4)<<setprecision(4)<<rsp.response.out_path[n].rps_route[m].x<<"	y:"<<setw(4)<<setprecision(4)<<rsp.response.out_path[n].rps_route[m].y<<"	th:"<<fixed<<setprecision(1)<<rsp.response.out_path[n].rps_route[m].th<<std::endl;
			//~ }
		//~ }
	}
	else{
...
/////////RELEASE WAGON PLANNING///////////	
	...
	rc_rwp.request.pre_robot_joint_angle = execute_path.push_wagon_Robot_joint_angle[execute_path.push_wagon_Robot_joint_angle.size()-1];
...
/////////GRASP OBJECT PLANNING///////////	
	...
	rc_gop.request.pre_robot_joint_angle = execute_path.release_wagon_Robot_joint_angle[execute_path.release_wagon_Robot_joint_angle.size()-1];
...
/////////GIVE OBJECT PLANNING///////////	
	...
	rc_giop.request.pre_robot_joint_angle = execute_path.grasp_obj_Robot_joint_angle[execute_path.grasp_obj_Robot_joint_angle.size()-1];
...
/////////PRM PLANNING///////////	
	...
	temp_state.robot_joint_angle.joint_angle[9] = rc_gop.response.robot_joint_angle[rc_gop.response.robot_joint_angle.size()-1].joint_angle[9];
	rc_PRM.request.in_motion_state.push_back(temp_state);
