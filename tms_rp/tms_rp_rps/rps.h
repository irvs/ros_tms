...
class GivePosData
{
	public:
		...
		void set_Evaluation(vector<vector<CollisionMapData> > Map, vector<double> human_pos){
			view = Map[(int)round((robotPos[0]-x_llimit)/cell_size)][(int)round((robotPos[1]-y_llimit)/cell_size)].person_view;
			obj_dist = Map[(int)round((robotPos[0]-x_llimit)/cell_size)][(int)round((robotPos[1]-y_llimit)/cell_size)].dist_from_obj;
...
class ExecutePushWagonPath
{
	public:
		...
		tms_msg_rp::rps_position	start_Robot_pos, start_Wagon_pos, give_obj_Obj_pos, grasp_wagon_Robot_pos, give_obj_Robot_pos;
		vector<tms_msg_rp::rps_position> grasp_wagon_Robot_path, push_wagon_Robot_path, push_wagon_Wagon_path;
		vector<tms_msg_rp::rps_joint_angle> grasp_wagon_Robot_joint_angle, push_wagon_Robot_joint_angle, release_wagon_Robot_joint_angle, grasp_obj_Robot_joint_angle, give_obj_Robot_joint_angle; 
		
};

//Function///////////////////////////////////////
bool convertPos(double in_x, double in_y, int& out_x, int& out_y){	//in_pos:(m)
