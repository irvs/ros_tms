...
void path_comp(vector<vector<double> > in_path, vector<vector<double> >& out_path){
	...
	temp_pos.resize(3);next_pos.resize(3);d_vector.resize(3);
...
bool set_robot_path(tms_msg_rp::rps_path_checking::Request& req, tms_msg_rp::rps_path_checking::Response& res)
{
	...
	smoothing_path.clear();complement_path.clear();
	temp_pos.resize(3);
	...
	if(req.rps_robot_path.rps_route.size()==0){
		...
		for(unsigned int i=0;i<complement_path.size();i++){
			if(!MAP[(int)round(complement_path[i][0]/(10*cell_resolution))][(int)round(complement_path[i][1]/(10*cell_resolution))].robot_collision)
				MAP[(int)round(complement_path[i][0]/(10*cell_resolution))][(int)round(complement_path[i][1]/(10*cell_resolution))].route = 1;
		}
...
void MAP_time_init(vector<vector<map_data> >& Map){
	for(unsigned int x=0;x<Map.size();x++){ //初期化
		for(unsigned int y=0;y<Map[x].size();y++){
			Map[x][y].object_detect_time=0.0;
		}
	}
