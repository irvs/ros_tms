...
bool sp5_control_caller(int unit, int cmd, int arg_size, const double* arg) {
	...
	if (motion_client2.call(sp_control_srv)) ROS_INFO("result: %d", sp_control_srv.response.result);
	else ROS_ERROR("Failed to call service sp5_control");	return true;
}
...
int main(int argc, char **argv) {
	...
	while(ros::ok()) {
		...
		case 5:
		{
			...
			th_motion42.join();
			boost::thread th_motion43(boost::bind(&sp5_control_caller, UNIT_ARM_L, CMD_MOVE_ABS, 8, aArmIni));
			ROS_INFO("Successflly finish demonstration^^");
		}
		default:
		{
			break;
