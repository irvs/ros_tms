...
//	sp5_tts(s_sentence_list, timing_list, size);
	...
	while(ros::ok()) {
		...
		while(1) {
			...
			if(speech_num == 10000) goto QUIT;
			if (!std::cin.fail() && (1<=speech_num && speech_num<=6)) break;
		...
		switch (speech_num) {
		case 1:
			if (sp5_tts(hello)) ROS_INFO("succeed to call service speech1");
			break;
		case 2:
			if (sp5_tts(thank_you)) ROS_INFO("succeed to call service speech2");
			break;
		case 3:
			if (sp5_tts(enjoy)) ROS_INFO("succeed to call service speech3");
			break;
		case 4:
			if (sp5_tts(do_my_best)) ROS_INFO("succeed to call service speech4");
			break;
		case 5:
			if (sp5_tts(open)) ROS_INFO("succeed to call service speech5");
			break;
		case 6:
			if (sp5_tts(close)) ROS_INFO("succeed to call service speech6");
			break;
		default:
			break;
		}
