    ...
    , qnode(argc,argv)
{
    ...
    curve2->setPen( QPen( Qt::darkGreen, 1 ) ),
...
void MainWindow::plot_graph()
{
    ...
    for(int i=0; i < 721; i++)
    {
        if(bViewOnlyCluster == true)
        {
            ...
            {
                vDistanceRangeData[i] = QwtIntervalSample( double(i)*0.25,
                    QwtInterval(qnode.m_stMinErrorData[i].fDistance, qnode.m_stMaxErrorData[i].fDistance));

                vIntensityRangeData[i] = QwtIntervalSample( double(i)*0.25,
                    QwtInterval(qnode.m_stMinErrorData[i].fIntensity, qnode.m_stMaxErrorData[i].fIntensity));

                vIntrinsicIntensityRangeData[i] = QwtIntervalSample( double(i)*0.25,
                    QwtInterval(qnode.m_stMinErrorData[i].fIntrinsicIntensity, qnode.m_stMaxErrorData[i].fIntrinsicIntensity));
