...
void Cluster::initialize(const tms_msg_ss::fss_class_data::ConstPtr& msg, int index){
	...
	tempRectangleMinX = FLT_MAX;	tempRectangleMinY = FLT_MAX;
	tempRectangleMaxX = FLT_MIN;	tempRectangleMaxY = FLT_MIN;
...
void Cluster::copy(std::list<Cluster>::iterator it, int startIndex, int size, bool isFull){
	...
	if(isFull){
		...
		iType			= it->iType;
		fLikelihood	= it->fLikelihood;
		sensingTime	= (double)(it->tMeasuredTime[size-1].toNSec()/it->tMeasuredTime[startIndex].toNSec())/(double)1.0e9;
		inOcclusionArea = it->inOcclusionArea;
