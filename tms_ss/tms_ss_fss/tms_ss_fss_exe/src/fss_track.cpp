...
bool smartpalTracking(CLASS_DATA stClassData)
{
    ...
    if(iBoundaryPointNum >= 2)
    {
        ...
        for(i=0; i < iBoundaryPointNum-1; i++)
        {
            ...
            {
                fLength = get_length(vstBoundaryPonit[i].fPointX, vstBoundaryPonit[i].fPointY, vstBoundaryPonit[j].fPointX, vstBoundaryPonit[j].fPointY);
        ...
        float fDifferenceAngle = acos( (fModelPA_fMeasuredPB_D*fModelPA_fMeasuredPB_D+fModelPA_ModelPB_D*fModelPA_ModelPB_D-fMeasuredPB_ModelPB_D*fMeasuredPB_ModelPB_D)/(2*fModelPA_fMeasuredPB_D*fModelPA_ModelPB_D) );

        float dir = (fModelPB.fX-fModelPA.fX)*(fMeasuredPB.fY-fModelPA.fY)-(fMeasuredPB.fX-fModelPA.fX)*(fModelPB.fY-fModelPA.fY);

        if ( dir == 0.0f )      fDifferenceAngle = 0.0f;
        else if ( dir < 0.0f )  fDifferenceAngle = fDifferenceAngle *-1;
        else                    fDifferenceAngle = fDifferenceAngle * 1;
...
bool wagonTracking(CLASS_DATA stClassData)
{
    ...
    for(unsigned int i=0; i < stClassData.vfClusterCenterX.size(); i++)
    {
        ...
        for(unsigned int j=0; j < stClassData.vfClusterCenterX.size(); j++)
        {
            ...
            fDistance = get_length(stClassData.vfClusterCenterX[i],stClassData.vfClusterCenterY[i],stClassData.vfClusterCenterX[j],stClassData.vfClusterCenterY[j]);
    ...
    else if(iWheelNum == 2)
    {
        ...
        else if(fLength_max > fMinSideLength && fLength_max < fMaxSideLength)
        {
            ...
            fTheta = (vfClusterCenterX[iNumi]-vfClusterCenterX[iNumj])*(vfClusterCenterY[iNumi]-vfClusterCenterY[iNumj]) > 0 ? fTheta : M_PI - fTheta;
    ...
    else if(iWheelNum == 3 && fObjectCenterX != 0 && fObjectCenterY != 0)
    {
        for(unsigned int i=0; i < 3; i++)
        {
            if(i!=iNumi && i!=iNumj)
            {
                fDx = (fObjectCenterX > vfClusterCenterX[i] ? fObjectCenterX - vfClusterCenterX[i] : vfClusterCenterX[i] - fObjectCenterX);
                fDy = (fObjectCenterY > vfClusterCenterY[i] ? fObjectCenterY - vfClusterCenterY[i] : vfClusterCenterY[i] - fObjectCenterY);
...
void trackCallback(const tms_msg_ss::fss_class_data::ConstPtr& msg)
{
    ...
    if(bIsExistSmartpal)
    {
        ...
        for(unsigned int i=0; i < stFssClassData.viID.size(); i++)
        {
            float fRadius = sqrt((stFssClassData.vfClusterCenterX[i]-stSmartpalCenter.fX)*(stFssClassData.vfClusterCenterX[i]-stSmartpalCenter.fX)+(stFssClassData.vfClusterCenterY[i]-stSmartpalCenter.fY)*(stFssClassData.vfClusterCenterY[i]-stSmartpalCenter.fY));
    ...
    if(bIsExistRoomba)
    {
        ...
        for(unsigned int i=0; i < stFssClassData.viID.size(); i++)
        {
            float fRadius = sqrt((stFssClassData.vfClusterCenterX[i]-stRoombaCenter.fX)*(stFssClassData.vfClusterCenterX[i]-stRoombaCenter.fX)+(stFssClassData.vfClusterCenterY[i]-stRoombaCenter.fY)*(stFssClassData.vfClusterCenterY[i]-stRoombaCenter.fY));
    ...
    if(bIsExistWagon)
    {
        ...
        for(unsigned int i=0; i < stFssClassData.viID.size(); i++)
        {
            float fRadius = sqrt((stFssClassData.vfClusterCenterX[i]-stWagonCenter.fX)*(stFssClassData.vfClusterCenterX[i]-stWagonCenter.fX)+(stFssClassData.vfClusterCenterY[i]-stWagonCenter.fY)*(stFssClassData.vfClusterCenterY[i]-stWagonCenter.fY));
    ...
    if(bIsExistChair)
    {
        ...
        for(unsigned int i=0; i < stFssClassData.viID.size(); i++)
        {
            float fRadius = sqrt((stFssClassData.vfClusterCenterX[i]-stChairCenter.fX)*(stFssClassData.vfClusterCenterX[i]-stChairCenter.fX)+(stFssClassData.vfClusterCenterY[i]-stChairCenter.fY)*(stFssClassData.vfClusterCenterY[i]-stChairCenter.fY));
    ...
    if(stFssClassData.viID.size() != 0)
    {
        ...
        for(unsigned int i=0; i < stOcclusionGroup.size(); i++)
        {
            ...
            if(iRestorationNum!=0)
            {
                for(unsigned int j=0; j < stCurrentGroup.size(); j++)
                {
                    ...
                    float fLength1 = get_length(stOcclusionGroup[i].vfX[0],
                                                stOcclusionGroup[i].vfY[0],
                                                stCurrentGroup[j].vfX[0],
                                                stCurrentGroup[j].vfY[0]);
                    float fLength2 = get_length(stOcclusionGroup[i].vfX[0],
                                                stOcclusionGroup[i].vfY[0],
                                                stCurrentGroup[j].vfX[iTagetNum-1],
                                                stCurrentGroup[j].vfY[iTagetNum-1]);
                    float fLength3 = get_length(stOcclusionGroup[i].vfX[iRestorationNum-1],
                                                stOcclusionGroup[i].vfY[iRestorationNum-1],
                                                stCurrentGroup[j].vfX[0],
                                                stCurrentGroup[j].vfY[0]);
                    float fLength4 = get_length(stOcclusionGroup[i].vfX[iRestorationNum-1],
                                                stOcclusionGroup[i].vfY[iRestorationNum-1],
                                                stCurrentGroup[j].vfX[iTagetNum-1],
                                                stCurrentGroup[j].vfY[iTagetNum-1]);
        ...
        for(uint32_t i=0; i < stReclusterDataTemp.viID.size(); i++)
        {
            for(uint32_t j=i+1; j < stReclusterDataTemp.viID.size(); j++)
            {
                ...
                fLength = get_length(stReclusterDataTemp.vfClusterCenterX[i],
                                     stReclusterDataTemp.vfClusterCenterY[i],
                                     stReclusterDataTemp.vfClusterCenterX[j],
                                     stReclusterDataTemp.vfClusterCenterY[j]);

                //--------------------------------------------------------------
                if(fLength < 200)
                {
                    fCenterX =  (stReclusterDataTemp.vfClusterCenterX[i] +
                                 stReclusterDataTemp.vfClusterCenterX[j] ) / 2;

                    fCenterY =  (stReclusterDataTemp.vfClusterCenterY[i] +
                                 stReclusterDataTemp.vfClusterCenterY[j] ) / 2;
                    ...
                    for(uint32_t k=0; k < stReclusterDataTemp.vstDatas[j].fX2.size(); k++)
                    {
                        ...
                        stReclusterDataTemp.vstDatas[i].bIsForwardPoint.push_back(stReclusterDataTemp.vstDatas[j].bIsForwardPoint[k]);
                        ...
                        stReclusterDataTemp.vstDatas[i].fIntrinsicIntensity.push_back(stReclusterDataTemp.vstDatas[j].fIntrinsicIntensity[k]);
                    ...
                    for(unsigned l=0; l < stReclusterDataTemp.vstDatas[i].fX2.size(); l++)
                    {
                        ...
                        {
                            fLength = get_length(stReclusterDataTemp.vstDatas[i].fX2[l],
                                                 stReclusterDataTemp.vstDatas[i].fY2[l],
                                                 stReclusterDataTemp.vstDatas[i].fX2[m],
                                                 stReclusterDataTemp.vstDatas[i].fY2[m]);
    ...
    if(vstBefore_fss_unknown_cluster_data.size() != 0)
    {
        ...
        for(uint32_t i=0; i < stReclusterDataTemp.viID.size(); i++)
        {
            ...
            for(uint32_t j=0; j < msgIdInformation.iID.size(); j++)
            {
                fLength = get_length(stReclusterDataTemp.vfClusterCenterX[i],
                                     stReclusterDataTemp.vfClusterCenterY[i],
                                     msgIdInformation.fCenterX[j],
                                     msgIdInformation.fCenterY[j]);
