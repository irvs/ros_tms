...
// output stream
time_t now;
std::ofstream ofs1;	char filename1[256];		// for trajectoryTree
...
void makeTrajectoryTree()
{
	...
	for(detected_it=detectedClusterList.begin() ; detected_it!=detectedClusterList.end() ; detected_it++){
		...
		if(!isCreateNode){
			...
			for(sensing_it = sensingClusterList.begin();
				sensing_it != sensingClusterList.end(); sensing_it++){
				// center, sensing time
	...
	for(this_it=trajectoryTree.begin() ; this_it!=trajectoryTree.end() ; this_it++){
		...
		for(; child_it!=trajectoryTree.end() ; child_it++){
			...
			if(!isLinked){
				...
				if( (getTime(child_sTime)-getTime(this_sTime))>= 0 && (getTime(this_eTime)+(double)OVERLAPTIME_ROOM-getTime(child_sTime))>0 ){
					if(stepWidth < HUMAN_STEP_WIDTH){
						...
						for(s_it=sensingClusterList.begin() ; s_it!=sensingClusterList.end() ; s_it++){
							...
							// 計測時間、中心間距離をチェック
							if( (getTime(s_sTime)-getTime(this_sTime))>= 0 && (getTime(this_eTime)+(double)OVERLAPTIME_ROOM-getTime(s_sTime))>0 ){
...
float getMaxCost(std::list<Node>::iterator it)
{
	...
	if(it->cost != -1){
		...
		if(it->child_it.size() == 0){	// if node is leaf
			float cost = 0.0;
			it->cost = cost;
			it->isLeaf = true;
			return cost;
