...
void lrfCallback(const tms_msg_ss::fss_pre_data::ConstPtr& msg)
{
    ...
    for(unsigned int i=0 ; i<vstLrfDataTemp.size() ; i++)
    {
        ...
        else
        {
            ...
            float lengthB = get_length(vstLrfDataTemp[i-1].stCoordinate.fX2, vstLrfDataTemp[i-1].stCoordinate.fY2, vstLrfDataTemp[i+1].stCoordinate.fX2, vstLrfDataTemp[i+1].stCoordinate.fY2)/2;
