...
//	memset( fileName2, 0x00, sizeof(fileName2) );
	...
	/* 計測結果出力用ファイル名の設定 */
	sprintf(fileNamePosition, "%4d%02d%02d%02d%02d%02dp.csv",
		pnow->tm_year+1900,
		pnow->tm_mon+1,
		pnow->tm_mday,
		pnow->tm_hour,
...
//	if( preDetectedClusterList.size() > 0 ){
		...
		for( sensorNo = 0; sensorNo < MAX_SENSOR_NO; sensorNo++ ){
			for( dataNo = 0; dataNo < MAX_SENSOR_DATA_NUM; dataNo++ ){
				printf( "dataNo:%d\nlostTimeS:%llu\nlostTimeE:%llu\n difftime:%llu\n", dataNo, lostTimeS, lostTimeE, diffAccZ[sensorNo].rtime[dataNo] );
			}
		}
		...
		for( sensorNo = 0; sensorNo < MAX_SENSOR_NO; sensorNo++ ){
			...
			for( dataNo = (MAX_SENSOR_DATA_NUM-1); dataNo >= 0; dataNo-- ){
				if( (lostTimeE > diffAccZ[sensorNo].rtime[dataNo]) ){
					lostTimeNo = dataNo;
					printf( "lostTimeE:%llu diffTime:%llu lostTimeNo:%d\n", lostTimeE, diffAccZ[sensorNo].rtime[dataNo] ,lostTimeNo );
...
//						 || (diffAccZ[sensorNo].data[dataNo-4]>THRESHOLD_STOP) ){
			...
			for( dataNo = (MAX_SENSOR_DATA_NUM-1); dataNo >= 0; dataNo-- ){
				if( lostTimeS > diffAccZ[sensorNo].rtime[dataNo] ){
					detectTimeNo = dataNo;
					printf( "lostTimeS:%llu diffTime:%llu detectTimeNo:%d\n", lostTimeS, diffAccZ[sensorNo].rtime[dataNo] ,detectTimeNo );
//					if(		(diffAccZ[sensorNo].data[dataNo]>THRESHOLD_STOP)
//						 || (diffAccZ[sensorNo].data[dataNo+1]>THRESHOLD_STOP)
//						 || (diffAccZ[sensorNo].data[dataNo+2]>THRESHOLD_STOP)
//						 || (diffAccZ[sensorNo].data[dataNo+3]>THRESHOLD_STOP)
//						 || (diffAccZ[sensorNo].data[dataNo+4]>THRESHOLD_STOP) ){
		...
		fprintf( fpresultPosition, "%llu,%llu,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
				 lostTimeS, lostTimeE, lostPositionX, lostPositionY, maxEvalValueNo(),
				 diffAccFssDetect[0], diffAccFssDetect[1], diffAccFssDetect[2], diffAccFssDetect[3],
				 diffAccFssLost[0], diffAccFssLost[1], diffAccFssLost[2], diffAccFssLost[3] );
		fclose( fpresultPosition );
	}
...
float getScore(std::list<Cluster>::iterator it, const tms_msg_ss::fss_class_data::ConstPtr& msg, int gt_index)
{
	...
	if(SCORE_METHOD == 1){
		...
		float distanceCenters = getLength(it->fCenterX.back(), it->fCenterY.back(), msg->fCenterX[gt_index], msg->fCenterY[gt_index]);
		...
		RectangleMinX = FLT_MAX;	RectangleMinY = FLT_MAX;
		RectangleMaxX = FLT_MIN;	RectangleMaxY = FLT_MIN;
		...
		if(RectangleMinX < it->fRectangleMaxX.back() && it->fRectangleMinX.back() < RectangleMaxX){
			if(RectangleMinY < it->fRectangleMaxY.back() && it->fRectangleMinY.back() < RectangleMaxY){
				...
				//printf("score: %f, [%f, %f][%f, %f] - [%f, %f][%f, %f]\n", score, it->fRectangleMinX.back(), it->fRectangleMinY.back(), it->fRectangleMaxX.back(), it->fRectangleMaxY.back(), RectangleMinX, RectangleMinY, RectangleMaxX, RectangleMaxY);
			}
		}
	}
	return score;
...
int rcv9axis()
{
	...
	/* 計測結果出力用ファイル名の設定 */
	sprintf(fileName9axis, "%4d%02d%02d%02d%02d%02dn.csv",
		pnow->tm_year+1900,
		pnow->tm_mon+1,
		pnow->tm_mday,
		pnow->tm_hour,
	...
	*********************************************************************/
	...
	/* sensor0 */
	while( true ){
		if( com_ReadPacket( fdSensor0, buff, BUFF_SIZE ) ){		// パケットを１つ読み込む
	...
	/* sensor1 */
	while( true ){
		if( com_ReadPacket( fdSensor1, buff, BUFF_SIZE ) ){		// パケットを１つ読み込む
	...
	/* sensor2 */
	while( true ){
		if( com_ReadPacket( fdSensor2, buff, BUFF_SIZE ) ){		// パケットを１つ読み込む
	...
	/* sensor3 */
	while( true ){
		if( com_ReadPacket( fdSensor3, buff, BUFF_SIZE ) ){		// パケットを１つ読み込む
...
VOID on_BinaryMessage(PBYTE data, char fileName[], int sensorNo)
{
	...
	static int		cntStop[3] = {0, 0, 0};		// 歩行状態がどのくらい続いているかを計数（実際にはセンサが固定された方の足が動いている時間を計数）
	static int		cntWalk[3] = {0, 0, 0};		// 停止状態がどのくらい続いているかを計数（実際にはセンサが固定された方の足が止まっている時間を計数）
	...
	if( data[0] == 'B' ){
		...
		if( first == true ){		// 計測開始時のtime_stampを保持
			time_stamp_first[node_no] = ((UINT)data[1] << 12) & 0x7000 | ((UINT)data[2] << 4) & 0x0ff0 | ((UINT)data[3] >> 4) & 0x000f;
		}else{						// ２回目以降は計測開始時のtime_stampを引く（各９軸センサのスイッチを入れるタイミングがずれてもtime_stampは揃う）
...
//			fprintf( fpresult, "%d,%lf,%lf,%lf,", time_stamp, sensor[node_no-1].acc[0], sensor[node_no-1].acc[1], sensor[node_no-1].acc[2] );
			fclose( fpresult9axis );
		...
		if( (data[1]=='E') && (data[2]=='C') ){
			...
			sprintf( prof.bd_addr, "%02x%02x%02x%02x%02x%02x",
...
//		fclose( fpresult2 );
	...
	if( dataNum[sensorNo] > MAX_SENSOR_DATA_NUM ){
		...
		diffAccZ[sensorNo].data[MAX_SENSOR_DATA_NUM-1] = fabs( accZ[sensorNo].data[MAX_SENSOR_DATA_NUM-1] - accZ[sensorNo].data[MAX_SENSOR_DATA_NUM-2] );
	}else{
		...
		if( dataNum[sensorNo] >= 2 ){		// ２個以上データが格納されていたら前回との差分をとる
			diffAccZ[sensorNo].rtime[ dataNum[sensorNo]-1 ] = rtime;
			diffAccZ[sensorNo].data[ dataNum[sensorNo]-1 ] = fabs( accZ[sensorNo].data[ dataNum[sensorNo]-1 ] - accZ[sensorNo].data[ dataNum[sensorNo]-2 ] );
		}
	}
//	for( dataNo = 0; dataNo < dataNum[sensorNo]; dataNo++ ){
//		printf( "%d:%llu,%f,%llu,%f\n", sensorNo, accZ[sensorNo].rtime[dataNo], accZ[sensorNo].data[dataNo], diffAccZ[sensorNo].rtime[dataNo], diffAccZ[sensorNo].data[dataNo] );
//	}
...
//			accZ[sensorNo].time[dataNo], accZ[sensorNo].data[dataNo], diffAccZ[sensorNo].time[dataNo], diffAccZ[sensorNo].data[dataNo]
//		);
//	}
//	fclose( fpresult2 );
}
