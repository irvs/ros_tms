...
def main():
    ...
    while not rospy.is_shutdown():
        dev.update()
                   # must update() many times in a loop because most of buffer
                   # data is stuff data
        if dev.is_updated:
            r.sleep()
...
class MindWaveMobile(object):
    ...
    def __init__(self, port='/dev/rfcomm1'):
        ...
        self.dongle = serial.Serial(
            port, baudrate=115200, timeout=0.001)  # serialのポート
    ...
    def parse_payload(self, payload):
        while len(payload) > 0:
            ...
            if code >= 0x80:    # multi-bytes value
                vlength = payload.pop(
                    0)  # dont use because vlength can assumed by [code] variable
                if code == 0x80:
