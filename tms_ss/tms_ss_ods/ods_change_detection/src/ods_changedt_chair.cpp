...
//     std::cout << "chair position : " << chair.x << " " << chair.y << " " << chair.z << " " << chair.theta << std::endl;
...
//     t1(0,0) = sin(robot.theta);    t1(0,1) = -sin(camera_yaw)*cos(robot.theta);    t1(0,2) = cos(camera_yaw)*cos(robot.theta);    t1(0,3) = camera_x;
//     t1(1,0) = -cos(robot.theta);    t1(1,1) = -sin(camera_yaw)*sin(robot.theta);     t1(1,2) = cos(camera_yaw)*sin(robot.theta);     t1(1,3) = camera_y;
//     t1(2,0) = 0;                    t1(2,1) = -cos(camera_yaw);                      t1(2,2) = -sin(camera_yaw);                     t1(2,3) = camera_z;
...
// //クラスタリング
// void clustering(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& chair, double th1, double th2, int id)
// {
...
// // レジストレーション
// void registration(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& dsp_cloud, pcl::PointCloud<pcl::PointXYZRGB>& model, Eigen::Matrix4f& m)
// {
...
//             if(dis < 0.05){
//                 if((abs(Object_Map[j].size_x - Object_Map[i].size_x) < 0.05) && (abs(Object_Map[j].size_y - Object_Map[i].size_y) < 0.05) && (abs(Object_Map[j].size_z - Object_Map[i].size_z) < 0.02)){
//                     std::cout << "同じ物品" << i << " " << j << std::endl;
//                     std::cout << "size : " << abs(Object_Map[j].size_x - Object_Map[i].size_x) << " " << abs(Object_Map[j].size_y - Object_Map[i].size_y) << " " << abs(Object_Map[j].size_z - Object_Map[i].size_z) << std::endl;
//                     Object_Map[i].tf = 0;
//                     Object_Map[j].tf = 0;
//                     c = 1;
//                     break;
//                 }
//                 else std::cout << i << " " << j << " 大きさが違う" << abs(Object_Map[j].size_x - Object_Map[i].size_x) << " " << abs(Object_Map[j].size_y - Object_Map[i].size_y) << " " << abs(Object_Map[j].size_z - Object_Map[i].size_z) << std::endl;
//             }
...
//             fprintf(fp, "%d %lf %lf, %lf\n", Object_Map[i].tf, Object_Map[i].g.x, Object_Map[i].g.y, Object_Map[i].g.z);
...
{
    for(int i=0;i<cloud.points.size();i++){
        if(cloud.points[i].z <= 0.43) continue;
