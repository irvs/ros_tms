...
void transformation(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& model)
{
    ...
    t1(0,0) = cos(shelf.theta);     t1(0,1) = sin(shelf.theta);     t1(0,2) = 0;    t1(0,3) = shelf.x;
    t1(1,0) = -sin(shelf.theta);    t1(1,1) = cos(shelf.theta);     t1(1,2) = 0;    t1(1,3) = shelf.y;
    t1(2,0) = 0;                    t1(2,1) = 0;                    t1(2,2) = 1;    t1(2,3) = shelf.z;
    t1(3,0) = 0;                    t1(3,1) = 0;                    t1(3,2) = 0;    t1(3,3) = 1;
...
void filtering(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& model)
{
    ...
    pass.setFilterLimits (m_size.min_z-0.1, m_size.max_z+0.2);
    pass.filter (cloud);
...
void make_shelfvoxel(pcl::PointCloud<pcl::PointXYZRGB>& model)
{
...
void remove_shelf(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& rmc)
{
...
int make_elevation(object_map& m)
{
    ...
    *tmp_cloud = m.cloud_rgb;
    ...
    if((m.size_x > MAP_X) || (m.size_y > MAP_Y)) return 0;
...
void segmentation(pcl::PointCloud<pcl::PointXYZRGB>& cloud, double th, int c)
{
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it){
        ...
        if(1){
            ...
            map.g = g;
            map.tf = c;
            if(!make_elevation(map)) continue;
...
bool change_detection (tms_msg_ss::ods_furniture::Request &req, tms_msg_ss::ods_furniture::Response &res)
{
    ...
    std::cout << mkdir("/tmp/tms_ss_ods/ods_change_detection/shelf",  S_IRUSR | S_IWUSR | S_IXUSR) << std::endl;
   ...
   pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/input.pcd", *cloud1);

//    pcl::fromROSMsg (req.model, *model);

    //pcl::io::loadPCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/input.pcd", *cloud1);
    if(SHELF == 1)
        pcl::io::loadPCDFile ("/tmp/tms_ss_ods/ods_change_detection/shelf/model_shelf.pcd", *model);
    else
        pcl::io::loadPCDFile ("/tmp/tms_ss_ods/ods_change_detection/shelf/model_bigshelf.pcd", *model);

    //***************************
    // Fill in the cloud data
    //***************************
    // shelf.x = req.furniture.position.x/1000.0;
    // shelf.y = req.furniture.position.y/1000.0;
    // shelf.z = req.furniture.position.z/1000.0;
    ...
    *tfm_cloud = *model + *cloud1;
    ...
    if(!(tfm_cloud->points.size())){
        ...
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/tfm_cloud1.pcd", *tfm_cloud);
    ...
    if(!(cloud1->points.size())){
        ...
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/filter.pcd", *cloud1);
    ...
    if(SHELF == 1){
        ...
        downsampling(*model, 0.01);
    }
    else if(SHELF == 2){
        downsampling(*dsp_cloud, 0.03);
        downsampling(*model, 0.01);
    }

    //*******************************
    //registration new Info's shelf to memory Info's shelf
    //*******************************
    std::cout << "registration shelfs" << std::endl;
    ...
    if(!(rgs_cloud->size() != 0)){
        ...
        pcl::io::savePCDFile ("/tmp/tms_ss_ods/ods_change_detection/shelf/rgs_cloud.pcd", *rgs_cloud, true);

    //***************************
    // init s_voxel
    //***************************
    std::cout << "init shelf_voxel" << std::endl;

    make_shelfvoxel(*model);

    //***************************
    // remove shelf
    //***************************
    std::cout << "remove shelf" << std::endl;
    ...
    pcl::io::loadPCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/memory.pcd", *rms_cloud2);

    remove_shelf(*cloud1, *rms_cloud1);

    if(!(rms_cloud1->size() != 0)){
        std::cout << "removed shelf cloud has no point" << std::endl;
        return false;
    }

    else
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/remove_shelf1.pcd", *rms_cloud1, false);

//    //*******************************
//    //delete shelf
//    //*******************************
//    pcl::PointCloud<pcl::PointXYZRGB>::Ptr rms1_cloud (new pcl::PointCloud<pcl::PointXYZRGB>);
//    pcl::PointCloud<pcl::PointXYZRGB>::Ptr rms2_cloud (new pcl::PointCloud<pcl::PointXYZRGB>);

//    pcl::fromROSMsg (req.cloud, *rms2_cloud);

//    pcl::PointCloud<pcl::PointXYZRGB>::Ptr rgb5_cloud (new pcl::PointCloud<pcl::PointXYZRGB>);

//    remove_shelf(*src_cloud, *rms1_cloud);

//    pcl::io::savePCDFile ("/tmp/tms_ss_ods/ods_change_detection/shelf/rms_cloud.pcd", *rms1_cloud, true);
    ...
    if(rms_cloud1->size() != 0)
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/memory.pcd", *rms_cloud1, true);
    if(rms_cloud2->size() != 0)
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/obj_cloud2.pcd", *rms_cloud2, true);
    ...
    for(int i=0;i<Object_Map.size();i++){
        ...
        if((Object_Map[i].tf == 1)||(Object_Map[i].tf == 2)) {
            std::stringstream ss;
            ss << "/tmp/tms_ss_ods/ods_change_detection/shelf/result_rgb" << i << ".pcd";
    ...
    if((tmp_rgb1->size() == 0) && (tmp_rgb2->size() == 0)){
        std::cout << "No change on shelf!!" << std::endl;
        return true;
    }

    if(tmp_rgb1->size() != 0)
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/result1.pcd", *tmp_rgb1, true);
    if(tmp_rgb2->size() != 0)
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/result2.pcd", *tmp_rgb2, true);

    *view_cloud += *tmp_rgb1;
    pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/shelf/view_result.pcd", *view_cloud, true);
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "ods_changedt_shelf");
    ros::NodeHandle n;

    service = n.advertiseService("chg_dt_shelf", change_detection);
