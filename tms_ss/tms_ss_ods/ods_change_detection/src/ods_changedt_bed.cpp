...
void transformation(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& model)
{
    ...
    t1(0,0) = sin(robot.theta);     t1(0,1) = -sin(camera_pitch)*cos(robot.theta);  t1(0,2) = cos(camera_pitch)*cos(robot.theta);   t1(0,3) = camera_x;
    t1(1,0) = -cos(robot.theta);    t1(1,1) = -sin(camera_pitch)*sin(robot.theta);  t1(1,2) = cos(camera_pitch)*sin(robot.theta);   t1(1,3) = camera_y;
    t1(2,0) = 0;                    t1(2,1) = -cos(camera_pitch);                   t1(2,2) = -sin(camera_pitch);                   t1(2,3) = camera_z;
    t1(3,0) = 0;                    t1(3,1) = 0;                                    t1(3,2) = 0;                                    t1(3,3) = 1;

    pcl::transformPointCloud(cloud, cloud, t1);

    t1(0,0) = cos(bed.theta);     t1(0,1) = sin(bed.theta);     t1(0,2) = 0;    t1(0,3) = bed.x;
    t1(1,0) = -sin(bed.theta);    t1(1,1) = cos(bed.theta);     t1(1,2) = 0;    t1(1,3) = bed.y;
    t1(2,0) = 0;                    t1(2,1) = 0;                    t1(2,2) = 1;    t1(2,3) = bed.z;
    t1(3,0) = 0;                    t1(3,1) = 0;                    t1(3,2) = 0;    t1(3,3) = 1;
...
void registration(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& model, pcl::PointCloud<pcl::PointXYZRGB>& rmb, Eigen::Matrix4f& m)
{
    ...
    pcl::transformPointCloud (rmb, rmb, m);

    return;
}

//void make_bedvoxel(pcl::PointCloud<pcl::PointXYZRGB>& model)
//{
...
//    for(int i=0;i<model.points.size();i++){
//        int X = 50*(model.points[i].x-m_size.min_x);
//        int Y = 50*(model.points[i].y-m_size.min_y);
...
//void remove_bed(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& rmc)
//{
//    std::cout << m_size.min_x << " " << m_size.min_y << std::endl;
//    for(int i=0;i<cloud.points.size();i++){
//        int X = 50*(cloud.points[i].x-m_size.min_x);
//        int Y = 50*(cloud.points[i].y-m_size.min_y);
//        double Z = cloud.points[i].z;

//        if(((X < 0) || (SIZE_X <= X)) || ((Y < 0) || (SIZE_Y <= Y)) || (Z < 0)){
//            continue;
//        }

//        if(C_voxel[X][Y]<Z){
//            //std::cout << "b " << i << " " << C_voxel[X][Y] << " " << Z << std::endl;
...
void classifying(pcl::PointCloud<pcl::PointXYZRGB>& f_cloud, pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& rmb, double th1, int th2)
{
...
int compare_histogram(int a, int b)
{
    ...
    for(int i=0;i<MAP_H;i++){
        for(int j=0;j<MAP_S;j++){
            if(Bhattacharyya == 1) distance += sqrt(object1.histogram[i][j] * object2.histogram[i][j]);
            else distance += abs(object1.histogram[i][j] - object2.histogram[i][j]);
...
void segment_matching()
{
    ...
    for(int i=0;i<Object_Map.size();i++){
        ...
        for(int j=0;j<Object_Map.size();j++){
            ...
            if(dis < 0.07){
                ...
                if((abs(Object_Map[j].size_x - Object_Map[i].size_x) < 5) && (abs(Object_Map[j].size_y - Object_Map[i].size_y) < 5)){
                    ...
                    if(abs(avg1 - avg2) < 1.5){
                        ...
                        else std::cout << i << " " << j << " 高さの分散が違う " << abs(sqrt((double)variance(Object_Map[j], avg1)) - sqrt((double)variance(Object_Map[i], avg2))) << std::endl;
                    }
            ...
            else std::cout << i << " " << j << " 近くに物品なし" << dis << std::endl;
        }
...
////////////////////////////////////////////////////////////////////////
//main function
// to detect change of objects belong to bed
////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
bool change_detection (tms_msg_ss::ods_furniture::Request &req, tms_msg_ss::ods_furniture::Response &res)
{
    ...
    std::cout << mkdir("/tmp/tms_ss_ods/ods_change_detection/bed",  S_IRUSR | S_IWUSR | S_IXUSR) << std::endl;
   ...
   pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/input.pcd", *cloud1);

//    pcl::fromROSMsg (req.model, *model);

    //pcl::io::loadPCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/input.pcd", *cloud1);
    pcl::io::loadPCDFile ("/tmp/tms_ss_ods/ods_change_detection/bed/model_bed.pcd", *model);

    //*******************************
    //input PointCloud data
    //*******************************
    bed.x = req.furniture.position.x/1000.0;
    bed.y = req.furniture.position.y/1000.0;
    bed.z = req.furniture.position.z/1000.0;
    bed.theta = req.furniture.orientation.z;
    ...
    pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/downsampling1.pcd", *dsp_cloud1);
    ...
    *tfm_cloud = *model + *dsp_cloud1;
    ...
    if(!(tfm_cloud->points.size())){
        ...
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/tfm_cloud1.pcd", *tfm_cloud);
    ...
    if(!(cloud1->points.size())){
        ...
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/filter.pcd", *dsp_cloud1);
    ...
    for(int i=0;i<dsp_cloud1->points.size();i++){
        pcl::PointXYZRGB tmp;
        tmp.x = dsp_cloud1->points[i].r/100.0;
        tmp.y = dsp_cloud1->points[i].g/100.0;
        tmp.z = dsp_cloud1->points[i].b/100.0;
        tmp.r = dsp_cloud1->points[i].r;
        tmp.g = dsp_cloud1->points[i].g;
        tmp.b = dsp_cloud1->points[i].b;
    ...
    pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/rgb.pcd", *rgb_cloud, false);

    classifying(*rgb_cloud, *dsp_cloud1, *rmb_cloud1, 0.04, 5000);

    pcl::io::savePCDFile ("/tmp/tms_ss_ods/ods_change_detection/bed/branket1.pcd", *dsp_cloud1, false);
    pcl::io::savePCDFile ("/tmp/tms_ss_ods/ods_change_detection/bed/remove_branket1.pcd", *rmb_cloud1, false);

    //*******************************
    // register captured data to model using icp algorithm
    //*******************************
    ...
    //while (1){
        ...
        registration(*dsp_cloud1, *model, *rmb_cloud1, m);

        /*if((double)(m(0,0)+m(1,1)+m(2,2)+m(3,3)) >= 4){
            if(n > 5) break;
        }*/

        pcl::copyPointCloud(*dsp_cloud1, *view_cloud);

        *rgs_cloud = *dsp_cloud1 + *rmb_cloud1;

        n++;
    //}

    pcl::io::savePCDFile ("/tmp/tms_ss_ods/ods_change_detection/bed/registration1.pcd", *rgs_cloud, false);

    //*******************************
    // segmentation
    //*******************************
    segmentation(*rmb_cloud1, 0.02, 1);
    if(rmb_cloud2->points.size() != 0)
        segmentation(*rmb_cloud2, 0.015, 2);

    if(rmb_cloud1->size() != 0)
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/memory.pcd", *rmb_cloud1, true);
    if(rmb_cloud2->size() != 0)
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/obj_cloud2.pcd", *rmb_cloud2, true);
    ...
    for(int i=0;i<Object_Map.size();i++){
        ...
        if((Object_Map[i].tf == 1)||(Object_Map[i].tf == 2)) {
            std::stringstream ss;
            ss << "/tmp/tms_ss_ods/ods_change_detection/bed/result_rgb" << i << ".pcd";
    ...
    if((tmp_rgb1->size() == 0) && (tmp_rgb2->size() == 0)){
        std::cout << "No change on bed!!" << std::endl;
        return true;
    }

    if(tmp_rgb1->size() != 0)
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/result1.pcd", *tmp_rgb1, true);
    if(tmp_rgb2->size() != 0)
        pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/result2.pcd", *tmp_rgb2, true);

    *view_cloud += *tmp_rgb1;
    pcl::io::savePCDFile("/tmp/tms_ss_ods/ods_change_detection/bed/view_result.pcd", *view_cloud, true);
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "ods_changedt_bed");
    ros::NodeHandle n;
