...
void transformation(pcl::PointCloud<pcl::PointXYZRGB>& cloud)
{
    ...
    t1(0,0) = sin(Robot.theta);    t1(0,1) = -sin(Camera.pitch)*cos(Robot.theta);    t1(0,2) = cos(Camera.pitch)*cos(Robot.theta);    t1(0,3) = Camera.x;
    t1(1,0) = -cos(Robot.theta);    t1(1,1) = -sin(Camera.pitch)*sin(Robot.theta);     t1(1,2) = cos(Camera.pitch)*sin(Robot.theta);     t1(1,3) = Camera.y;
    t1(2,0) = 0;                    t1(2,1) = -cos(Camera.pitch);                      t1(2,2) = -sin(Camera.pitch);                     t1(2,3) = Camera.z;
...
void registration(pcl::PointCloud<pcl::PointXYZRGB>& src_cloud, pcl::PointCloud<pcl::PointXYZRGB>& mem_cloud, Eigen::Matrix4f& m)
{
...
int main ()
{
    ...
    float colors[8][3] ={{125,125,125},{255, 0, 0}, {0,255,0}, {0,0,255}, {255,255,0}, {0,255,255}, {255,0,255}, {255,255,255}};

    Eigen::Matrix4f m[7];

    for(int i=0;i<7;i++){
        m[i](0,0) = 1;    m[i](0,1) = 0;    m[i](0,2) = 0;    m[i](0,3) = 0;
        m[i](1,0) = 0;    m[i](1,1) = 1;    m[i](1,2) = 0;    m[i](1,3) = 0;
        m[i](2,0) = 0;    m[i](2,1) = 0;    m[i](2,2) = 1;    m[i](2,3) = 0;
    ...
    for(int j=1;j<LOOP;j++){
        ...
        for(int i=1;i<=8;i++){
            ...
            if(j == 1){
                if(i > 1){
                    Eigen::Matrix4f m1;
                    std::cout << "registration "  << " " << i  << " -> " << i-1 << std::endl;
    ...
    t1(0,0) = 1;    t1(0,1) = 0;    t1(0,2) = 0;    t1(0,3) = -Bed.width/2.0-0.1;
    t1(1,0) = 0;    t1(1,1) = 1;    t1(1,2) = 0;    t1(1,3) = -Bed.depth/2.0-0.1;
    t1(2,0) = 0;    t1(2,1) = 0;    t1(2,2) = 1;    t1(2,3) = -0.1;
    t1(3,0) = 0;    t1(3,1) = 0;    t1(3,2) = 0;    t1(3,3) = 1;
