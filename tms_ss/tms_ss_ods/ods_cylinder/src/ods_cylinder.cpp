...
//平面検出を行う
void segmentate(pcl::PointCloud<PointType>& cloud, pcl::PointCloud<PointType>& p_cloud, pcl::ModelCoefficients& coefficients)
{
...
void transformation(pcl::PointCloud<PointType>& cloud, pcl::PointCloud<PointType>& p_cloud, pcl::ModelCoefficientsPtr coefficients)
{
    ...
    //平行移動
    g = g_pos(p_cloud);
    r1(0,0) = 1;  r1(0,1) = 0;  r1(0,2) = 0;  r1(0,3) = -g.x;
    r1(1,0) = 0;  r1(1,1) = 1;  r1(1,2) = 0;  r1(1,3) = -g.y;
    r1(2,0) = 0;  r1(2,1) = 0;  r1(2,2) = 1;  r1(2,3) = -g.z;
    ...
    r2(0,0) = 1;  r2(0,1) = 0;        r2(0,2) = 0;          r2(0,3) = 0;
    r2(1,0) = 0;  r2(1,1) = cos(rad); r2(1,2) = -sin(rad);  r2(1,3) = 0;
    r2(2,0) = 0;  r2(2,1) = sin(rad); r2(2,2) = cos(rad);   r2(2,3) = 0;
    ...
    r3(0,0) = cos(rad);   r3(0,1) = 0;  r3(0,2) = sin(rad); r3(0,3) = 0;
    r3(1,0) = 0;          r3(1,1) = 1;  r3(1,2) = 0;        r3(1,3) = 0;
    r3(2,0) = -sin(rad);  r3(2,1) = 0;  r3(2,2) = cos(rad); r3(2,3) = 0;
...
void elevation_map(object& m)
{
    ...
    *tmp_cloud = m.cloud;
    ...
    m.max_x = max_x;  m.min_x = min_x;  m.max_y = max_y;  m.min_y = min_y;
    ...
    for(int i=0;i<tmp_cloud->size();i++){
        if(-tmp_cloud->points[i].z > m.map[(int)(100*(tmp_cloud->points[i].x-min_x))][(int)(100*(tmp_cloud->points[i].y-min_y))]){
            m.map[(int)(100*(tmp_cloud->points[i].x-min_x))][(int)(100*(tmp_cloud->points[i].y-min_y))] = -tmp_cloud->points[i].z;
        }
    }
...
bool cylinder_detection (tms_msg_ss::ods_cylinder::Request &req, tms_msg_ss::ods_cylinder::Response &res)
{
    ...
    for(int i=0;i<Object_Map.size();i++){
        ...
        pose.position.x = g.x; pose.position.y = g.y; pose.position.z = g.z;
