...
void check(int* array1, int x, int y, double z,int c)
{
    ...
    if(((x < 0) || (IMAGE_WIDTH <= x)) || ((y <0) || (IMAGE_HEIGHT <= y))) return;
    ...
    if((array1[cn] == 0)||(array1[cn] == 3)){
        ...
        if((z-50 < d) && (d < z+50)){
            if(array1[cn] == 0) array1[cn] = 4;
            else if(array1[cn] == 3) {
                array1[cn] = 2;
            }
            //さらに隣接するピクセルを調べる
            switch(c){
            case 1 : check(array1, x-1, y, d, 1);
            case 2 : check(array1, x+1, y, d, 2);
            case 3 : check(array1, x, y-1, d, 3);
            case 4 : check(array1, x, y+1, d, 4);
            }
        }
...
void extraction(IplImage& src, cv::Mat& mask)
{
    ...
    CV_INIT_PIXEL_POS( pos_src, (unsigned char *) src.imageData,
                           src.widthStep,cvGetSize(&src),0, 0, src.origin);
...
//**コールバック関数
//*******************************
bool skin_extraction(tms_msg_ss::ods_skincolor_extraction::Request &req, tms_msg_ss::ods_skincolor_extraction::Response &res)
{
    ...
    //顔方向の決定
    if(count >= 0) res.direction = 2; //right
    else res.direction = 2; //left
