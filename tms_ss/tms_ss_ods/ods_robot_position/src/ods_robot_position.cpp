...
void transformation(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZ>& model)
{
    ...
    t1(0,0) = 1;    t1(0,1) = 0;    t1(0,2) = 0;    t1(0,3) = table.x;
    t1(1,0) = 0;    t1(1,1) = 1;    t1(1,2) = 0;    t1(1,3) = table.y;
    t1(2,0) = 0;    t1(2,1) = 0;    t1(2,2) = 1;    t1(2,3) = table.z;
    t1(3,0) = 0;    t1(3,1) = 0;    t1(3,2) = 0;    t1(3,3) = 1;
...
void filtering(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZ>& model, pcl::PointIndices& inliers)
{
...
//マッチング
void registration(pcl::PointCloud<pcl::PointXYZ>& cloud, pcl::PointCloud<pcl::PointXYZ>& model, pcl::PointCloud<pcl::PointXYZ>& cloud_out, pcl::PointCloud<pcl::PointXYZRGB>& tmp_rgb, Eigen::Matrix4f& m)
{
...
bool robot_position (tms_msg_ss::ods_get_robots_pos::Request &req, tms_msg_ss::ods_get_robots_pos::Response &res)
{
    ...
    viewpoint.resize(4);
    viewpoint[0].x = 0.0;  viewpoint[0].y = 0.0; viewpoint[0].z = 0.0;
    viewpoint[1].x = 1.0;  viewpoint[1].y = 0.0; viewpoint[1].z = 0.0;
    viewpoint[2].x = 0.0;  viewpoint[2].y = 1.0; viewpoint[2].z = 0.0;
    viewpoint[3].x = 0.0;  viewpoint[3].y = 0.0; viewpoint[3].z = 1.0;
    ...
    *tfm_cloud += *cloud1;
    ...
    res.m_robot.x = 1000.0*(viewpoint[0].x + C_OFFSET_X*cos((res.m_robot.theta-90.0)*PI/180) - C_OFFSET_Y*cos(res.m_robot.theta*PI/180));
    res.m_robot.y = 1000.0*(viewpoint[0].y + C_OFFSET_X*sin((res.m_robot.theta-90.0)*PI/180) - C_OFFSET_Y*sin(res.m_robot.theta*PI/180));


    std::cout << res.m_robot.x << " " << res.m_robot.y << " " << viewpoint[0].z - C_OFFSET_Z << " " << res.m_robot.theta << std::endl;
