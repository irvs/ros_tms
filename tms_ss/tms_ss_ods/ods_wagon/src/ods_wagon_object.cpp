...
//平面検出を行う
void plane_detect(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& cloud_out, pcl::ModelCoefficients& coefficients)
{
    ...
    while(1){
        ...
        if((atan(-coefficients.values[2]/coefficients.values[3])<(-1.0*PI/4.0)) ||((1.0*PI/4.0) < atan(-coefficients.values[2]/coefficients.values[3]))){
            extract.setInputCloud(cloud.makeShared());
...
{
    double min_x = 100.0;   double max_x = -100.0;
    double min_z = 100.0;   double max_z = -100.0;

    for(int i=0;i<cloud.points.size();i++){
        if(cloud.points[i].x < min_x) min_x = cloud.points[i].x;
        else if(cloud.points[i].x > max_x) max_x = cloud.points[i].x;
        if(cloud.points[i].z < min_z) min_z = cloud.points[i].z;
        else if(cloud.points[i].z > max_z) max_z = cloud.points[i].z;
    }
...
//クラスタリング
void clustering(pcl::PointCloud<pcl::PointXYZRGB>& cloud, std::vector<pcl::PointXYZRGB>& points, pcl::ModelCoefficients& coefficients)
{
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it){
        ...
        if((0.02 < abs(coefficients.values[0]+g1.y*coefficients.values[1]+g1.z*coefficients.values[2]+coefficients.values[3])) &&
                (abs(coefficients.values[0]+g1.y*coefficients.values[1]+g1.z*coefficients.values[2]+coefficients.values[3]) < 0.14)){
            if(coefficients.values[3]*(g1.x*coefficients.values[0]+g1.y*coefficients.values[1]+g1.z*coefficients.values[2]+coefficients.values[3]) > 0){
...
bool wagon_object(tms_msg_ss::ods_wagon_object::Request &req, tms_msg_ss::ods_wagon_object::Response &res)
{
    ...
    *tmp_cloud += *cloud;
    ...
    for(int i=0;i<g_cloud.size();i++){
        ...
        pose.position.x = g_cloud[i].x; pose.position.y = g_cloud[i].y; pose.position.z = g_cloud[i].z;
