...
void plane_detect(pcl::PointCloud<pcl::PointXYZRGB>& cloud)
{
    ...
    while(1){
        ...
        if((atan(-coefficients->values[2]/coefficients->values[3])<(-1.0*PI/4.0)) ||((1.0*PI/4.0) < atan(-coefficients->values[2]/coefficients->values[3]))){ // x軸まわりに45度以上回転なら棚板（※カメラ座標空間）
            extract.setInputCloud(cloud.makeShared());
...
pcl::PointXYZ transformation(pcl::PointXYZ point)
{
    ...
    // 変換行列の設定
    t(0,0) =  sin(robot.theta);   t(0,1) = -sin(camera_pitch)*cos(robot.theta);   t(0,2) =  cos(camera_pitch)*cos(robot.theta);
    t(1,0) = -cos(robot.theta);   t(1,1) = -sin(camera_pitch)*sin(robot.theta);   t(1,2) =  cos(camera_pitch)*sin(robot.theta);
...
void clustering(pcl::PointCloud<pcl::PointXYZRGB>& cloud, std::vector<pcl::PointXYZ>& points)
{
    ...
    // クラスタリング後の処理
    int m = 0;
    float colors[6][3] ={{255, 0, 0}, {0,255,0}, {0,0,255}, {255,255,0}, {0,255,255}, {255,0,255}};	// クラスタの色
    ...
    // クラスタ（グリッパ）が2つ以上ない場合
    if(m<2) std::cout << "Not find two clusters!!" << std::endl;
...
void quick_sort(std::vector<pcl::PointXYZ>& points, int left, int right)
{
	...
	// 並び替えるべき要素を見つける
    while(1){
        while(distance[i] < pivot) i++;

        while(pivot < distance[j]) j--;
    ...
    if(left < i-1) quick_sort(points, left, i-1);
    if(j+1 < right) quick_sort(points, j+1, right);
...
void wagon_position(std::vector<pcl::PointXYZ>& points)
{
    ...
    if(points.size() == 4){
        ...
        double dis1, dis2;
        dis1 = sqrt((points[1].x-points[0].x)*(points[1].x-points[0].x) + (points[1].y-points[0].y)*(points[1].y-points[0].y));
        dis2 = sqrt((points[2].x-points[0].x)*(points[2].x-points[0].x) + (points[2].y-points[0].y)*(points[2].y-points[0].y));

        std::cout << dis1 << " " << dis2 << std::endl;

        if(dis1 < dis2) wagon.theta = atan2(points[2].y-points[0].y, points[2].x-points[0].x)*180.0/PI;
        else wagon.theta = atan2(points[1].y-points[0].y, points[1].x-points[0].x)*180.0/PI;
    }
        ...
        dis1 = sqrt((points[1].x-points[0].x)*(points[1].x-points[0].x) + (points[1].y-points[0].y)*(points[1].y-points[0].y));
        dis2 = sqrt((points[2].x-points[0].x)*(points[2].x-points[0].x) + (points[2].y-points[0].y)*(points[2].y-points[0].y));
        dis3 = sqrt((points[2].x-points[1].x)*(points[2].x-points[1].x) + (points[2].y-points[1].y)*(points[2].y-points[1].y));
        ...
        else std::cout << "No match wagon" << std::endl;

        if((0.38 < dis1) && (dis1 < 0.46)) wagon.theta = atan2(points[1].y-points[0].y, points[1].x-points[0].x)*180.0/PI;
        else if((0.38 < dis2) && (dis2 < 0.46)) wagon.theta = atan2(points[2].y-points[0].y, points[2].x-points[0].x)*180.0/PI;
        else if((0.38 < dis3) && (dis3 < 0.46)) wagon.theta = atan2(points[2].y-points[1].y, points[2].x-points[1].x)*180.0/PI;
        else std::cout << "No match wagon" << std::endl;
    }
        ...
        if(0.46 <= dis1){
            ...
            if(points[0].x != points[1].x) theta = (points[1].y - points[0].y) / (points[1].x - points[0].x);
            else theta = 90.0;
    ...
    else std::cout << "No find wagon gripper " << points.size() << std::endl;
}
...
bool ods_wagon(tms_msg_ss::ods_wagon::Request &req, tms_msg_ss::ods_wagon::Response &res)
{
    ...
    srv.request.id = 3;
    if(commander_to_kinect_capture.call(srv)) pcl::fromROSMsg (srv.response.cloud, *cloud);
    ...
    *tmp_cloud += *cloud;
    ...
    while(1){
        if(wagon.theta < -90.0) wagon.theta += 180.0;
        else if(wagon.theta >= 90.0) wagon.theta -= 180.0;
        else break;
    }
