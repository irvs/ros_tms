...
void passfilter(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointCloud<pcl::PointXYZRGB>& cloud_out, std::vector<int>& index)
{
...
int check_occlusion(pcl::PointCloud<pcl::PointXYZRGB>& cloud, pcl::PointXYZRGB p1, pcl::PointXYZRGB p2, int inlier1, int inlier2)
{
    double a, b;
    int x1, y1, x2, y2;
    int count = 0, check = 0;
...
std::vector<Endpoints> group_endpoints(std::vector<Endpoints> endpoints_vec, pcl::PointCloud<pcl::PointXYZRGB>& cloud)
{
    ...
    for(int i=0;i<endpoints_vec.size();i++){
        for(int j=0;j<endpoints_vec.size();j++){
            if(i>=j) continue;
            double dis1, dis2;
            dis1 = sqrt((tmp[group_number[i]].p1.x - tmp[group_number[j]].p2.x)*(tmp[group_number[i]].p1.x - tmp[group_number[j]].p2.x) +
                        (tmp[group_number[i]].p1.z - tmp[group_number[j]].p2.z)*(tmp[group_number[i]].p1.z - tmp[group_number[j]].p2.z));

            dis2 = sqrt((tmp[group_number[i]].p2.x - tmp[group_number[j]].p1.x)*(tmp[group_number[i]].p2.x - tmp[group_number[j]].p1.x) +
                        (tmp[group_number[i]].p2.z - tmp[group_number[j]].p1.z)*(tmp[group_number[i]].p2.z - tmp[group_number[j]].p1.z));

            //std::cout << "dis1 : " << dis1 << " dis2 : " << dis2 << std::endl;
            if((dis1 < 0.5) || (dis2 < 0.5)){
                if((dis1 <= dis2)){
                    if(check_occlusion(cloud, tmp[group_number[j]].p2, tmp[group_number[i]].p1, tmp[group_number[j]].inlier2, tmp[group_number[i]].inlier1)){
                    ...
                    if(check_occlusion(cloud, tmp[group_number[i]].p2, tmp[group_number[j]].p1, tmp[group_number[i]].inlier2, tmp[group_number[j]].inlier1)){
                        tmp[group_number[i]].cloud += tmp[group_number[j]].cloud;
...
//クラスタリング
Endpoints clustering(pcl::PointCloud<pcl::PointXYZRGB>& f_cloud, pcl::PointCloud<pcl::PointXYZRGB>& cloud, std::vector<int>& index)
{
    ...
    for(int i=0;i<fin_endpoints_vec.size();i++){
        ...
        *tmp_cloud += fin_endpoints_vec[i].cloud;
        ...
        double l = sqrt((fin_endpoints_vec[i].p1.x - fin_endpoints_vec[i].p2.x) * (fin_endpoints_vec[i].p1.x - fin_endpoints_vec[i].p2.x) +
                        (fin_endpoints_vec[i].p1.y - fin_endpoints_vec[i].p2.y) * (fin_endpoints_vec[i].p1.y - fin_endpoints_vec[i].p2.y) +
                        (fin_endpoints_vec[i].p1.z - fin_endpoints_vec[i].p2.z) * (fin_endpoints_vec[i].p1.z - fin_endpoints_vec[i].p2.z));
        if(l > len){
            len = l;
            id = i;
        }
        ...
        //std::cout << "\ngroup " << i << std::endl;
        //std::cout << "p1 " << fin_endpoints_vec[i].p1.x << " " << fin_endpoints_vec[i].p1.y << " " << fin_endpoints_vec[i].p1.z << std::endl;
	...
	std::string file_name2 = path + "/catkin_ws/src/ros_tms/tms_ss/tms_ss_ods/ods_person_detection/data/cluster_cloud.pcd";
