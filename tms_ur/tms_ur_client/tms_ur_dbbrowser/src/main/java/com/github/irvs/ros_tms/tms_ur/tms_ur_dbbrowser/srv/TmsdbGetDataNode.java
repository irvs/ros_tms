package com.github.irvs.ros_tms.tms_ur.tms_ur_dbbrowser.srv;
...
public class TmsdbGetDataNode extends AbstractNodeMain {
	...
	//単品情報の格納領域
	private TmsdbObject obj;

	//上位オブジェクトの格納領域(placeにあたる)
	private TmsdbObject src;

	//家具列の格納領域
	private ArrayList<TmsdbObject> furnitures;

	//物品列の格納領域
	private ArrayList<TmsdbObject> objects;
	...
	//家具情報を取得(AR用)
	//これを実行すると家具情報+ロボット情報がfurnituresに入る
	public int sendFurnitures(){
	...
	public int sendInfo(TmsdbObject data){
		...
		srvClient.call(request, new ServiceResponseListener<TmsdbGetDataResponse>(){
			...
			public void onSuccess(TmsdbGetDataResponse response) {
				...
				isCalled = 1;
				obj = null;
				obj = new TmsdbObject(response.getTmsdb().get(0));
				result = response.getTmsdb().size();
			}
			@Override
			public void onFailure(RemoteException e) {
			...
			public void onSuccess(TmsdbGetDataResponse response) {
				...
				if(response.getTmsdb().size()!=0){
					for(Tmsdb tmsdb:response.getTmsdb()){
						if(tmsdb.getType().equals(TmsdbObject.TYPE._FURNITURE)||tmsdb.getType().equals(TmsdbObject.TYPE._ROBOT)){
							objects.add(new TmsdbObject(tmsdb));
						}
						else{
							if(tmsdb.getState()==1) objects.add(new TmsdbObject(tmsdb)); 
						}
					}
				}
				result += objects.size();
