...
void control_manipulator(unsigned int iPos0, unsigned int iPos1,
		unsigned int iPos2, unsigned int iPos3, SendIJogPacket *stSendIJOGPacket) {
	...
	//CheckSum
	stSendIJOGPacket->ucCheckSum1 = stSendIJOGPacket->ucPacketSize
			^ stSendIJOGPacket->ucChipID ^ stSendIJOGPacket->ucCmd;
...
bool callback(tms_msg_rc::kobuki_control_1::Request  &req,
	...
	case 2:
		...
		if (req.arg.size() != 4 || req.arg[0] < 308 || req.arg[0] > 715
				|| req.arg[1] < 120 || req.arg[1] > 900 || req.arg[2] < 235 || req.arg[2] > 788) {
...
int main(int argc, char **argv) {
	...
	//CheckSum
	stSendRWPacket.ucCheckSum1 = stSendRWPacket.ucPacketSize
			^ stSendRWPacket.ucChipID ^ stSendRWPacket.ucCmd
