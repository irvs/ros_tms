...
int user_angle_rad2enc(CLMBase* robot, int index, double radian){
	...
	c = (double)param->encoderOffset + (double)param->rotationDirection * ((param->angleOffset-radian)/(2*M_PI)) * (double)param->encodersPerCycle;
	d = user_round(c);
...
//	katana->getCoordinatesFromEncoders(pose_init,enc_init);
	...
	ROS_INFO("Encoder Value init Motor 0 : %d", enc_init[0]);	ROS_INFO("Encoder Value init Motor 1 : %d", enc_init[1]);
	ROS_INFO("Encoder Value init Motor 2 : %d", enc_init[2]);	ROS_INFO("Encoder Value init Motor 3 : %d", enc_init[3]);
	ROS_INFO("Encoder Value init Motor 4 : %d", enc_init[4]);	ROS_INFO("Encoder Value init Motor 5 : %d", enc_init[5]);

	ROS_INFO("Encoder degree init Motor 0 : %lf", pose_init[0]);	ROS_INFO("Encoder degree init Motor 1 : %lf", pose_init[1]);
	ROS_INFO("Encoder degree init Motor 2 : %lf", pose_init[2]);	ROS_INFO("Encoder degree init Motor 3 : %lf", pose_init[3]);
	ROS_INFO("Encoder degree init Motor 4 : %lf", pose_init[4]);	ROS_INFO("Encoder degree init Motor 5 : %lf", pose_init[5]);
...
//	katana->moveMotorToEnc(0, user_angle_rad2enc(katana.get(),0, user_angle_deg2rad(270)));	//モーター0を角度45度へEnc値で動かすテスト
