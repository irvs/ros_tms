...
bool Kinematics6M90G::enc2rad(std::vector<double> &aAngles, const std::vector<int> aEncoders)
{
  ...
  {
    aAngles[i] =
        MHF::enc2rad(aEncoders[i], mAngleOffset[i], mEncodersPerCycle[i], mEncoderOffset[i], mRotationDirection[i]);
  }
  return true;
}
//////////////////////////////////////////////////////////////////////////
bool Kinematics6M90G::rad2enc(std::vector<int> &aEncoders, const std::vector<double> aAngles)
{
  ...
  {
    aEncoders[i] =
        MHF::rad2enc(aAngles[i], mAngleOffset[i], mEncodersPerCycle[i], mEncoderOffset[i], mRotationDirection[i]);
  }
  return true;
}
...
void Kinematics6M90G::IK_theta234theta5(angles_calc &angle, const position &p_gr) const
{
  angle.theta234 = -MHF::acotan(((p_gr.x * p_gr.z * cos(angle.theta1)) -
                                 sqrt((-MHF::pow2(p_gr.z)) *
                                      (-MHF::pow2(mSegmentLength[3]) + MHF::pow2(p_gr.x) + MHF::pow2(p_gr.z)) *
                                      MHF::pow2(sin(angle.theta1)))) /
                                MHF::pow2(p_gr.z));
...
void Kinematics6M90G::IK_b1b2costh3_6MS(angles_calc &angle, const position &p) const
{
  ...
  angle.costh3 =
      -(MHF::pow2(angle.b1) + MHF::pow2(angle.b2) - MHF::pow2(mSegmentLength[0]) - MHF::pow2(mSegmentLength[1])) /
      (2.0 * mSegmentLength[0] * mSegmentLength[1]);
}
//////////////////////////////////////////////////////////////////////////
void Kinematics6M90G::thetacomp(angles_calc &angle, const position &p_m) const
{
  angle.theta2 = -MHF_PI / 2.0 -
                 (MHF::atan0(angle.b1, angle.b2) + MHF::atan0(mSegmentLength[0] + mSegmentLength[1] * cos(angle.theta3),
                                                              mSegmentLength[1] * sin(angle.theta3)));
...
bool Kinematics6M90G::PositionTest6MS(const angles_calc &a, const position &p) const
{
  ...
  temp = mSegmentLength[0] * sin(a.theta2) + mSegmentLength[1] * sin(a.theta2 + a.theta3) +
         mSegmentLength[2] * sin(a.theta234);
  xm2 = cos(a.theta1) * temp;
  ym2 = sin(a.theta1) * temp;
  zm2 = mSegmentLength[0] * cos(a.theta2) + mSegmentLength[1] * cos(a.theta2 + a.theta3) +
        mSegmentLength[2] * cos(a.theta234);
