...
void trymat3()
{
...

  {
    ...
    SymmetricMatrix S2 = D;
    Matrix M2 = S2;
    {
      M2 = -D + M2;
      Print(M2);
    }
    UpperTriangularMatrix U2 = D;
    {
      M2 = U2;
      M2 = D - M2;
      Print(M2);
    }
    LowerTriangularMatrix L2 = D;
    {
      M2 = L2;
      M2 = D - M2;
      Print(M2);
...

  {
    ...
    for (i = 1; i <= 6; i++)
      D(i, i) = i * 3.0 + i * i + 2.0;
    UpperTriangularMatrix U2(7);
    LowerTriangularMatrix L2(7);
    for (i = 1; i <= 7; i++)
      for (j = 1; j <= i; j++)
        L2(i, j) = 2.0 - i * i + j;
    {
    ...
    for (i = 1; i <= 6; i++)
      for (j = 1; j <= 7; j++)
        M2(i, j) = 2.0 + i * j + i * i + 2.0 * j * j;
    Matrix MD = D;
    SymmetricMatrix MD1(1);
    MD1 = D1;
    Matrix MX = MD * M2 * MD1 - D * (M2 * D1);
    Print(MX);
    MX = MD * M2 * MD1 - (D * M2) * D1;
    Print(MX);
    {
      D.ReSize(7);
      for (i = 1; i <= 7; i++)
        D(i, i) = i * 3.0 + i * i + 2.0;
      LowerTriangularMatrix LD(1);
      LD = D;
      UpperTriangularMatrix UD(1);
      UD = D;
      M2 = U2;
      M2 = LD * M2 * MD1 - D * (U2 * D1);
      Print(M2);
      M2 = U2;
      M2 = UD * M2 * MD1 - (D * U2) * D1;
      Print(M2);
      M2 = L2;
      M2 = LD * M2 * MD1 - D * (L2 * D1);
      Print(M2);
      M2 = L2;
      M2 = UD * M2 * MD1 - (D * L2) * D1;
      Print(M2);
    }
  }
  ...
  }
  {
    ...
    ColumnVector X = CV - RCV.Reverse();
    Print(X);
    RowVector Y = CV.t() - RCV.t().Reverse();
    Print(Y);
    DiagonalMatrix D = CV.AsDiagonal() - RCV.AsDiagonal().Reverse();
    Print(D);
    X = CV & CV.Rows(1, 9).Reverse();
    ColumnVector Z(19);
    Z.Rows(1, 10) = RCV.Reverse();
    Z.Rows(11, 19) = RCV.Rows(2, 10);
    X -= Z;
    Print(X);
    Z -= Z.Reverse();
    Print(Z);
  ...
  }
  {
    ...
    Matrix Y(3, 3);
    Y << 0.0 << 1.0 << 1.0 << 5.0 << 0.0 << 5.0 << 3.0 << 3.0 << 0.0;
