  ...
  for (;;)
  {
    switch (TheState)
    {
      case Start:
        ...
        if (oorg)
          Throw(ProgramException("invalid starting value\n"));
...
void NonLinearLeastSquares::Value(const ColumnVector& Parameters, bool, Real& v, bool& oorg)
{
...
bool NonLinearLeastSquares::NextPoint(ColumnVector& Adj, Real& test)
{
  ...
  test = M.SumSquare();
  cout << " " << setw(15) << setprecision(10) << test << " " << Y.SumSquare() / (n_obs - n_param);
  Adj = U.i() * M;
  if (test < errorvar * criterion)
    return true;
  else
    return false;
}
...
void NonLinearLeastSquares::Fit(const ColumnVector& Data, ColumnVector& Parameters)
{
...
void MLE_D_FI::Value(const ColumnVector& Parameters, bool wg, Real& v, bool& oorg)
{
