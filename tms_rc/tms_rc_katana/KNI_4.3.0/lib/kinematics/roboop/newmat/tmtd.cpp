...
ReturnMatrix LU3(const Matrix& A)
{
  ...
  CroutMatrix* X = new CroutMatrix(A);
...
ReturnMatrix LU3(const BandMatrix& A)
{
  ...
  BandLUMatrix* X = new BandLUMatrix(A);
  MatrixErrorNoSpace(X);
  X->release_and_delete();
  return X->for_return();
}
...
void CircularShift(const Matrix& X1, int first, int last)
{
  ...
  X = X1.Columns(1, first - 1) | X1.Column(last) | X1.Columns(first, last - 1) | X1.Columns(last + 1, n);
  QRZ(X, U2);
  X = U1 - U2;
  Clean(X, 0.000000001);
  Print(X);
  ...
  X = X1.Columns(1, first - 1) | X1.Columns(first + 1, last) | X1.Column(first) | X1.Columns(last + 1, n);
  QRZ(X, U2);
  X = U1 - U2;
  Clean(X, 0.000000001);
  Print(X);
...
public:
  void Reset();
  TestUpdateQRZ(int mx, int n1x, int n2x = 0, int n3x = 0) : m(mx), n1(n1x), n2(n2x), n3(n3x)
  {
    Reset();
  }
  void DoTest();
...
void trymatd()
{
...

  {
    ...
    A << 5 << 1 << 4 << 2 << 1 << 6 << 1 << 0 << 1 << 7;
    B << 8 << 1 << 5 << 1 << 0 << 9 << 2 << 1 << 0 << 6;
...

  {
    ...
    B = LU1(A);
    B1 = LU2(A);
    CroutMatrix B2 = LU3(A);
    C = A * B.i() - IdentityMatrix(7);
    Clean(C, 0.000000001);
    Print(C);
    ...
    D(25) = (d != d1 ? 0 : 1) + (d1 == d2 ? 0 : 1) + (i != i1 ? 0 : 1) + (i1 == i2 ? 0 : 1);
...

  {
    ...
    // check release again - see if memory moves
    const Real* d = B.const_data();
    const Real* dd = B.const_data();
    const int* i = B.const_data_indx();
    B1 = B;
    const Real* d1 = B1.const_data();
    const Real* dd1 = B1.const_data();
    const int* i1 = B1.const_data_indx();
    B1.release();
    B2 = B1;
    const Real* d2 = B2.const_data();
    const Real* dd2 = B2.const_data();
    const int* i2 = B2.const_data_indx();
    D(25) = (d != d1 ? 0 : 1) + (d1 == d2 ? 0 : 1) + (dd != dd1 ? 0 : 1) + (dd1 == dd2 ? 0 : 1) + (i != i1 ? 0 : 1) +
            (i1 == i2 ? 0 : 1);
...

  {
    ...
    X = X1.Rows(1, 19) & X1.Rows(21, 34) & X1.Rows(36, 100) & NewRow;
    QRZ(X, U1);
    Diff = U1 - U2;
    Clean(Diff, 0.000000001);
    Print(Diff);
...

  {
    ...
    TestUpdateQRZ tuqrz1(10, 100, 50, 25);
    tuqrz1.DoTest();
    tuqrz1.Reset();
    tuqrz1.ClearRow(1);
    tuqrz1.DoTest();
    ...
    tuqrz1.ClearRow(10);
    tuqrz1.DoTest();
    TestUpdateQRZ tuqrz2(15, 100, 0, 0);
    tuqrz2.DoTest();
    tuqrz2.Reset();
    tuqrz2.ClearRow(1);
    tuqrz2.DoTest();
