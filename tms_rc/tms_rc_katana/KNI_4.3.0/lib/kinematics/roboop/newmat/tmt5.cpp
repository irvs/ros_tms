...
void trymat5()
{
  ...
  RV = Returner1(RV) - 1;
  Print(RowVector(RV - RV1));
  CV1 = Returner1(RV.t()) - 1;
  Print(ColumnVector(RV.t() - CV1));
#ifndef DONT_DO_NRIC
  nricMatrix AA = A;
  X = Returner1(AA) - A - 1;
  Print(X);
  ...
  for (i = 1; i <= 31; i++)
    for (j = i; j <= 31; j++)
      UT(i, j) = i + j + (i - j) * (i - 2 * j);
  UpperBandMatrix UB(31, 5);
  UB.Inject(UT);
  LowerTriangularMatrix LT = UT.t();
  LowerBandMatrix LB(31, 5);
  LB.Inject(LT);
  A = Returner0(UB) - LB.t();
  Print(A);
  A = Returner2(UB).t() - LB * 2;
  Print(A);
  A = Returner3(LB).t() - UB * 3;
  Print(A);
  SymmetricMatrix SM;
  SM << (UT + LT);
  A = Returner4(SM) - UT - LT - 4;
  Print(A);
  SymmetricBandMatrix SB(31, 5);
  SB.Inject(SM);
  A = Returner5(SB) / 5 - UB - LB;
  Print(A);
  BandMatrix B = UB + LB * LB;
  A = LB;
  A = Returner6(B) / 6 - UB - A * A;
  Print(A);
  DiagonalMatrix D;
  D << UT;
  D << (Returner7(D) / 7 - UT);
