...
public:
  NonLinearLeastSquares(R1_Col_I_D& pred, int lim = 1000, Real crit = 0.0001) : criterion(crit), Pred(pred), Lim(lim)
  {
  }
...
public:
  MLE_D_FI(LL_D_FI& ll, int lim = 1000, Real criterion = 0.0001) : LL(ll), Lim(lim), Criterion(criterion)
  {
  }
