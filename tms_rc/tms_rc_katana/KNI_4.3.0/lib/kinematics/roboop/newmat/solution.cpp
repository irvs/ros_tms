...
{
  if (!xSet)
    Throw(SolutionException("Value of X not set"));
  Real y = operator()();
  return y;
}
...
void OneDimSolve::LookAt(int V)
{
  lim--;
  if (!lim)
    Throw(SolutionException("Does not converge"));
  Last = V;
...
void OneDimSolve::Quadratic(int I, int J, int K)
{
  ...
  if (square(YJK / YIK) > (x[K] - x[J]) / XKI || square(YIJ / YIK) > (x[J] - x[I]) / XKI)
  {
    x[I] = XKJ;
...
Real OneDimSolve::Solve(Real Y, Real X, Real Dev, int Lim)
{
  ...
  Captured = false;
  if (Dev == 0.0)
    Throw(SolutionException("Dev is zero"));
  L = 0;
  C = 1;
  U = 2;
  vpol = 1;
