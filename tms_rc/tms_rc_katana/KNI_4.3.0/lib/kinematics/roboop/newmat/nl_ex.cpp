...
public:
  Model_3pe(const ColumnVector& X_Values) : x_values(X_Values)
  {
    deriv.resize(3);
  }
  // load X data
...
{
  {
    ...
    cout << "\nResidual s.d. = " << setw(10) << setprecision(2) << ResidualSD << endl;
    ...
    cout << "\nX, Y, Residual, Hat\n" << setw(10) << setprecision(2) << (X | Y | Residuals | Hat.as_column()) << endl;
