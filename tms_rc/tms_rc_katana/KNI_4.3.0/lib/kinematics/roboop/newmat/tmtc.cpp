...
void trymatc()
{
...

  {
    Tracer et1("Stage 6");
    B = A.AsColumn();
    ColumnVector B1 = (A + 10000).AsColumn() - 10000;
    Print(ColumnVector(B1 - B));
  }

  {
    Tracer et1("Stage 7");
    Matrix X = B.AsMatrix(3, 5);
    Print(Matrix(X - A));
    for (i = 1; i <= 3; i++)
      for (j = 1; j <= 5; j++)
        B(5 * (i - 1) + j) -= i + 100 * j;
...

  {
    ...
    Print(Y);
    Y = 0.0;
    Y = ((const Matrix &)X) - ((const Matrix &)A);
    Print(Y);
  }

  {
    ...
    U2.SubMatrix(4, 8, 5, 9) -= U1;
    Print(U2);
    U1 -= (777 * V);
    Print(U1);

    U1 = U;
    U1.SubMatrix(4, 8, 5, 9) -= U.SymSubMatrix(1, 5);
    U1 -= U;
    U2 = U1;
    U1 << U1.SubMatrix(4, 8, 5, 9);
    U2.SubMatrix(4, 8, 5, 9) -= U1;
    ...
    U1 -= U;
    Matrix X = U1.SubMatrix(3, 10, 15, 19);
    X -= 29;
    Print(X);
    U1.SubMatrix(3, 10, 15, 19) *= 0;
    Print(U1);
    ...
    L2.SubMatrix(5, 9, 4, 8) -= L1;
    Print(L2);
    L1 -= (777 * M);
    Print(L1);

    L1 = L;
    L1.SubMatrix(5, 9, 4, 8) -= L.SymSubMatrix(1, 5);
    L1 -= L;
    L2 = L1;
    L1 << L1.SubMatrix(5, 9, 4, 8);
    L2.SubMatrix(5, 9, 4, 8) -= L1;
    ...
    L1 -= L;
    X = L1.SubMatrix(15, 19, 3, 10);
    X += 29;
    Print(X);
    L1.SubMatrix(15, 19, 3, 10) *= 0;
    Print(L1);
  }

  {
    ...
    for (j = 1; j <= 30; j++)
    {
      ColumnVector CV = 3 * M1.Column(j);
      M.Column(j) += CV;
    }
    for (i = 1; i <= 20; i++)
    {
      RowVector RV = 5 * M1.Row(i);
      M.Row(i) -= RV;
    }
...

  {
    ...
    Clean(X, 0.000000001);
    Print(X);
    X = RV - A.t().sum_square_rows().t();
    Clean(X, 0.000000001);
    Print(X);
    X = CV - A.t().sum_square_columns().t();
    Clean(X, 0.000000001);
    Print(X);
    ...
    UT << A;
    Matrix A1 = UT;
    X = UT.sum_square_rows() - A1.sum_square_rows();
    Print(X);
    X = UT.sum_square_columns() - A1.sum_square_columns();
    Print(X);
    // LowerTriangularMatrix
    LowerTriangularMatrix LT;
    LT << A;
    A1 = LT;
    X = LT.sum_square_rows() - A1.sum_square_rows();
    Print(X);
    X = LT.sum_square_columns() - A1.sum_square_columns();
    Print(X);
    // SymmetricMatrix
    SymmetricMatrix SM;
    SM << A;
    A1 = SM;
    X = SM.sum_square_rows() - A1.sum_square_rows();
    Print(X);
    X = SM.sum_square_columns() - A1.sum_square_columns();
    Print(X);
    // DiagonalMatrix
    DM << A;
    A1 = DM;
    X = DM.sum_square_rows() - A1.sum_square_rows();
    Print(X);
    X = DM.sum_square_columns() - A1.sum_square_columns();
    Print(X);
    // BandMatrix
    BandMatrix BM(17, 3, 5);
    BM.Inject(A);
    A1 = BM;
    X = BM.sum_square_rows() - A1.sum_square_rows();
    Print(X);
    X = BM.sum_square_columns() - A1.sum_square_columns();
    Print(X);
    // SymmetricBandMatrix
    SymmetricBandMatrix SBM(17, 4);
    SBM.Inject(A);
    A1 = SBM;
    X = SBM.sum_square_rows() - A1.sum_square_rows();
    Print(X);
    X = SBM.sum_square_columns() - A1.sum_square_columns();
    Print(X);
    // IdentityMatrix
    IdentityMatrix IM(29);
    X = IM.sum_square_rows() - 1;
    Print(X);
    X = IM.sum_square_columns() - 1;
    Print(X);
    // Matrix with zero rows
    A1.ReSize(0, 10);
    X.ReSize(1, 10);
    X = 0;
    X -= A1.sum_square_columns();
    Print(X);
    X.ReSize(0, 1);
    X -= A1.sum_square_rows();
    Print(X);
...

  {
    ...
    UT << A;
    Matrix A1 = UT;
    X = UT.sum_rows() - A1.sum_rows();
    Print(X);
    X = UT.sum_columns() - A1.sum_columns();
    Print(X);
    // LowerTriangularMatrix
    LowerTriangularMatrix LT;
    LT << A;
    A1 = LT;
    X = LT.sum_rows() - A1.sum_rows();
    Print(X);
    X = LT.sum_columns() - A1.sum_columns();
    Print(X);
    // SymmetricMatrix
    SymmetricMatrix SM;
    SM << A;
    A1 = SM;
    X = SM.sum_rows() - A1.sum_rows();
    Print(X);
    X = SM.sum_columns() - A1.sum_columns();
    Print(X);
    // DiagonalMatrix
    DiagonalMatrix DM;
    DM << A;
    A1 = DM;
    X = DM.sum_rows() - A1.sum_rows();
    Print(X);
    X = DM.sum_columns() - A1.sum_columns();
    Print(X);
    // BandMatrix
    BandMatrix BM(17, 3, 5);
    BM.Inject(A);
    A1 = BM;
    X = BM.sum_rows() - A1.sum_rows();
    Print(X);
    X = BM.sum_columns() - A1.sum_columns();
    Print(X);
    // SymmetricBandMatrix
    SymmetricBandMatrix SBM(17, 4);
    SBM.Inject(A);
    A1 = SBM;
    X = SBM.sum_rows() - A1.sum_rows();
    Print(X);
    X = SBM.sum_columns() - A1.sum_columns();
    Print(X);
    // IdentityMatrix
    IdentityMatrix IM(29);
    X = IM.sum_rows() - 1;
