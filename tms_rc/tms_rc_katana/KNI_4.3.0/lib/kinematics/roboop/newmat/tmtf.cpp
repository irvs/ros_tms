  ...
  for (int i = 0; i < m; ++i)
    for (int j = 0; j < n; ++j)
    {
      ...
      for (int k = 0; k < m; ++k)
        for (int l = 0; l < n; ++l)
        {
          Real a = -pi2 * ((Real)k * (Real)i / (Real)m + (Real)j * (Real)l / (Real)n);
          Real cs = cos(a);
          Real sn = sin(a);
...
static void test(int n)
{
  ...
  for (int i = 0; i < n; i++)
  {
    A.element(i) = mwc.Next();
    B.element(i) = mwc.Next();
  }
  FFT(A, B, X, Y);
  FFTI(X, Y, X, Y);
  X = X - A;
...
static void test1(int n)
{
  ...
  X2 = X1 - X.Rows(1, n2);
  Y2 = Y1 - Y.Rows(1, n2);
  Clean(X2, 0.000000001);
  Clean(Y2, 0.000000001);
  Print(X2);
  Print(Y2);
  X2.CleanUp();
  Y2.CleanUp();  // release some more space
  RealFFTI(X1, Y1, B);
  B = A - B;
  Clean(B, 0.000000001);
  Print(B);
...
static void test4(int n)
{
  ...
  A1 -= A2;
  B1 -= B2;
  Clean(A1, 0.000000001);
  Clean(B1, 0.000000001);
  Print(A1);
  Print(B1);
}
...
static void test9(int m, int n)
{
  ...
  for (int i = 0; i < m; i++)
    for (int j = 0; j < n; j++)
    {
      A.element(i, j) = mwc.Next();
      B.element(i, j) = mwc.Next();
    }
  FFT2(A, B, X, Y);
  SlowFT2(A, B, X1, Y1);
  X = X - X1;
...
void trymatf()
{
  ...
  {
    Real i1 = i - 1;
    A(i) = .7 + .2 * cos(6.2831853071795864769 * 4.0 * i1 / 12) + .3 * sin(6.2831853071795864769 * 3.0 * i1 / 12);
    B(i) = .9 + .5 * sin(6.2831853071795864769 * 2.0 * i1 / 12) + .4 * cos(6.2831853071795864769 * 1.0 * i1 / 12);
  }
  FFT(A, B, A, B);
  A(1) -= 8.4;
  A(3) -= 3.0;
