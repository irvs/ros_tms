...
static void GR_1D_FT(int N, int N_FACTOR, const SimpleIntArray& FACTOR, Real* X, Real* Y)
{
  ...
  for (int i = 0; i < N_FACTOR; i++)
  {
    ...
    switch (P)
    {
      ...
      case 8:
        REPORT
        R_8_FTK(N, M, X, Y, X + M, Y + M, X + 2 * M, Y + 2 * M, X + 3 * M, Y + 3 * M, X + 4 * M, Y + 4 * M, X + 5 * M,
                Y + 5 * M, X + 6 * M, Y + 6 * M, X + 7 * M, Y + 7 * M);
        break;
      case 16:
        REPORT
        R_16_FTK(N, M, X, Y, X + M, Y + M, X + 2 * M, Y + 2 * M, X + 3 * M, Y + 3 * M, X + 4 * M, Y + 4 * M, X + 5 * M,
                 Y + 5 * M, X + 6 * M, Y + 6 * M, X + 7 * M, Y + 7 * M, X + 8 * M, Y + 8 * M, X + 9 * M, Y + 9 * M,
                 X + 10 * M, Y + 10 * M, X + 11 * M, Y + 11 * M, X + 12 * M, Y + 12 * M, X + 13 * M, Y + 13 * M,
                 X + 14 * M, Y + 14 * M, X + 15 * M, Y + 15 * M);
        break;
      default:
