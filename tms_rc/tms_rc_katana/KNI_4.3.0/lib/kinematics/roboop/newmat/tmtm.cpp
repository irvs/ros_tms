...
void trymatm()
{
...

  {
    ...
    IdentityMatrix I1(10);
    IdentityMatrix I2(15);
    I2 *= 2;
...

  {
    ...
    UpperTriangularMatrix AB = KP(A, B) - C;
    Print(AB);

    LowerTriangularMatrix BT = B.t();
    Matrix N(12, 12);
...

  {
    ...
    UT << KP(BMA, BMB);
    UpperTriangularMatrix UT1;
    UT1 << (C - UT);
    Print(UT1);
    LowerTriangularMatrix LT;
    LT << KP(BMA, BMB);
    LowerTriangularMatrix LT1;
    LT1 << (C - LT);
    Print(LT1);
  }
...

  {
    ...
    Matrix M = KP(C * A, B * D) - KP(C, B) * KP(A, D);
    Print(M);
  }

  {
    ...
    A << 5 << 2 << 4;
    B << 3 << 2 << 0 << 1 << 6;
    C << 15 << 10 << 0 << 5 << 30 << 6 << 4 << 0 << 2 << 12 << 12 << 8 << 0 << 4 << 24;
    Matrix N = KP(A, B) - C;
    Print(N);
    N = KP(A.t(), B.t()) - C.t();
    Print(N);
    N = KP(A.AsDiagonal(), B.AsDiagonal()) - C.AsDiagonal();
    Print(N);
  }
...

  {
    ...
    AB.ReSize(5, 5);
    AB = 0;
    AB.SubMatrix(1, 2, 1, 3) = A;
    AB.SubMatrix(4, 5, 3, 5) = A;
    AB.SubMatrix(1, 3, 4, 5) = B;
    AB.SubMatrix(3, 5, 1, 2) = B;
    SquareMatrix C(5);
    C.Row(1) << 2 << 6 << 7 << 1 << 3;
    C.Row(2) << 4 << 3 << 9 << 4 << 8;
    C.Row(3) << 1 << 3 << 0 << 0 << 6;
