...
void trymatj()
{
...

  {
    ...
    X = SP(A - 1, B) + B - C;
    Print(X);
    X = SP(A - 1, B + 1) + B - A - C + 1;
    Print(X);
    // Where row-wise routine will be used
    A = A.Rows(7, 13);
    B = B.Rows(7, 13);
    C = C.Rows(7, 13);
    LowerTriangularMatrix LTA;
    LTA << A;
    UpperTriangularMatrix UTB;
    UTB << B;
    DiagonalMatrix DC;
    DC << C;
    X = SP(LTA, UTB) - DC;
  ...
  }
  {
    ...
    X = SP(BA, SB) - SP(A, S);
    Print(X);
    X = SP(BA * 2, SB) - SP(A, S * 2);
    Print(X);
    X = SP(BA, SB / 4) - SP(A / 4, S);
    Print(X);
    X = SP(BA * 4, SB / 4) - SP(A, S);
    Print(X);
  }
}
