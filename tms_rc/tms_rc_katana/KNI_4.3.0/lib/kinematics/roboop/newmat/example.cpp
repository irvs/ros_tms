...
void test1(Real* y, Real* x1, Real* x2, int nobs, int npred)
{
  ...
  cout << setw(9) << setprecision(3) << (X.columns(2, 3) | Y | Fitted | Residual | Hat.as_column());
  cout << "\n\n";
}
...
void test2(Real* y, Real* x1, Real* x2, int nobs, int npred)
{
  ...
  // Get diagonals of Hat matrix (an expensive way of doing this)
  Matrix X1(nobs, npred1);
  X1.column(1) << Ones;
  X1.columns(2, npred1) << X;
  DiagonalMatrix Hat;
  Hat << X1*(X1.t() * X1).i() * X1.t();
  ...
  cout << setw(11) << setprecision(5) << (A | SE) << endl;
  cout << "\nObservations, fitted value, residual value, hat value\n";
  cout << setw(9) << setprecision(3) << (X | Y | Fitted | Residual | Hat.as_column());
  cout << "\n\n";
}
...
void test4(Real* y, Real* x1, Real* x2, int nobs, int npred)
{
  ...
  ColumnVector M;
  QRZ(X1, U);
  QRZ(X1, Y1, M);  // Y1 now contains resids
  ...
  cout << setw(11) << setprecision(5) << (A | SE) << endl;
  cout << "\nObservations, fitted value, residual value, hat value\n";
  cout << setw(9) << setprecision(3) << (X.columns(2, 3) | Y | Fitted | Y1 | Hat.as_column());
  cout << "\n\n";
}
...
int main()
{
  ...
  cout << "Checking for lost memory: " << (unsigned long)s1 << " " << (unsigned long)s2 << " ";
  if (s1 != s2)
    cout << " - see section 2.8\n";
  else
    cout << " - ok\n";
