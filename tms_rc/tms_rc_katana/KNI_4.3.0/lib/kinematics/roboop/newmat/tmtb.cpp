...
ReturnMatrix TestClass::Sum()
{
  return Matrix(A + B).ForReturn();
}
...
void trymatb()
{
  ...
  }
  {
    ...
    for (i = 1; i <= 5; i++)
      for (j = 1; j <= 10; j++)
        M4(i, j) = (i * i * i + 10) * (j * j - 3);
    Print(Matrix(M1 - M4));
    M1 = L.t();
    M2 = U.t();
    M3 = L + U;
    Print(Matrix(M1 - M3.t() + M2));
  }
  ...
  UpperTriangularMatrix U2((UpperTriangularMatrix &)U);
  {
    ...
    for (i = 1; i <= 10; i++)
      for (j = 1; j <= 10; j++)
        M2(i, j) = (i * i * i + 10) * (j * j - 3);
    D << M2;
    L << M2;
    U << M2;  // check copy into
  ...
  int a1, a2, a3, a4;
  {
    ...
    M3.ReSize(a2 - a1 + 1, a4 - a3 + 1);
    M3 = 0.0;
    for (i = a1; i <= a2; i++)
      for (j = (i > a3) ? i : a3; j <= a4; j++)
