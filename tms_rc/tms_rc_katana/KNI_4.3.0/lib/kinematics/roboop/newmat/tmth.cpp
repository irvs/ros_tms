...
void BandFunctions(int l1, int u1, int l2, int u2)
{
  ...
  BandMatrix BMA = BM1 + BM2, BMS = BM1 - BM2, BMSP = SP(BM1, BM2), BMM = BM1 * BM2, BMN = -BM1;
  ...
  BandMatrix BM1E(20, l1, u1);
  BM1E = 999999.0;
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= 20; ++j)
      if (i - j <= l1 && i - j >= -u1)
        BM1E.element(i - 1, j - 1) = 100 * i + j;
  BandMatrix BM2E = BM1E;
  BM2E.ReSize(BM2);
  BM2E = 777777.0;
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= 20; ++j)
...
void LowerBandFunctions(int l1, int l2)
{
  ...
  LowerBandMatrix BMA = BM1 + BM2, BMS = BM1 - BM2, BMSP = SP(BM1, BM2), BMM = BM1 * BM2, BMN = -BM1;
  ...
  // test element function
  LowerBandMatrix BM1E(20, l1);
  BM1E = 999999.0;
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
      if (i - j <= l1)
        BM1E.element(i - 1, j - 1) = 100 * i + j;
  LowerBandMatrix BM2E = BM1E;
  BM2E.ReSize(BM2);
  BM2E = 777777.0;
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
...
void UpperBandFunctions(int u1, int u2)
{
  ...
  UpperBandMatrix BMA = BM1 + BM2, BMS = BM1 - BM2, BMSP = SP(BM1, BM2), BMM = BM1 * BM2, BMN = -BM1;
  ...
  // test element function
  UpperBandMatrix BM1E(20, u1);
  BM1E = 999999.0;
  for (i = 1; i <= 20; ++i)
    for (j = i; j <= 20; ++j)
      if (i - j >= -u1)
        BM1E.element(i - 1, j - 1) = 100 * i + j;
  UpperBandMatrix BM2E = BM1E;
  BM2E.ReSize(BM2);
  BM2E = 777777.0;
  for (i = 1; i <= 20; ++i)
    for (j = i; j <= 20; ++j)
...
void SymmetricBandFunctions(int l1, int l2)
{
  ...
  SymmetricBandMatrix BMA = BM1 + BM2, BMS = BM1 - BM2, BMSP = SP(BM1, BM2), BMN = -BM1;
  BandMatrix BMM = BM1 * BM2;

  SymmetricMatrix M1(20);
  M1 = 0;
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
      if (i - j <= l1)
  ...
  // test element function
  SymmetricBandMatrix BM1E(20, l1);
  BM1E = 999999.0;
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
      if (i - j <= l1)
        BM1E.element(i - 1, j - 1) = 100 * i + j;
  SymmetricBandMatrix BM2E = BM1E;
  BM2E.ReSize(BM2);
  BM2E = 777777.0;
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
  ...
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
      if (i - j <= l1)
        BM1E.element(j - 1, i - 1) = 100 * i + j;
  BM2E = 777777.0;
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
      if (i - j <= l2)
        BM2E.element(j - 1, i - 1) = (100 * i + j) * 11;
  M1 = BM1E - BM1;
  Print(M1);
  M2 = BM2E - BM2;
  Print(M2);
  ...
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
      if (i - j <= l1)
        BM1E.element(j - 1, i - 1) = BM1C.element(j - 1, i - 1);
  for (i = 1; i <= 20; ++i)
    for (j = 1; j <= i; ++j)
      if (i - j <= l2)
        BM2E.element(j - 1, i - 1) = BM2C.element(j - 1, i - 1);
  M1 = BM1E - BM1;
  Print(M1);
  M2 = BM2E - BM2;
  Print(M2);
...
void trymath()
{
  ...
  }
  {
    ...
    for (i = 1; i <= 80; i++)
      CX(i) = i * 100.0;
    Matrix MP = P;
    ColumnVector V1 = P.i() * CX;
    ColumnVector V2 = MP.i() * CX;
    V = V1 - V2;
    Clean(V, 0.000000001);
    Print(V);
...

  {
    ...
    UpperBandMatrix UB(8, 3);
    LowerBandMatrix LB(8, 4);
    D << SA;
    UB << SA;
    LB << SA;
    SA = SA * 5.0;
    ...
    LB = LB * 5.0;
    UB = UB * 5.0;
    BandMatrix B = LB - D + UB - SA;
    Print(Matrix(B));

    SymmetricBandMatrix A(7, 2);
    A = 100.0;
    for (i = 1; i <= 7; i++)
      for (j = 1; j <= 7; j++)
      {
    ...
    Clean(M, 0.000000001);
    Print(M);
    M = LB.i() * UB - LB.i() * Matrix(UB);
    Clean(M, 0.000000001);
    Print(M);
    M = UB.i() * LB - UB.i() * Matrix(LB);
    Clean(M, 0.000000001);
    Print(M);
  }

  {
    ...
    Print(X);
    Matrix Test(9, 2);
    Test(1, 1) = BD.BandWidth().Lower();
    Test(1, 2) = BD.BandWidth().Upper();
    Test(2, 1) = UBD.BandWidth().Lower();
    Test(2, 2) = UBD.BandWidth().Upper();
    Test(3, 1) = LBD.BandWidth().Lower();
    Test(3, 2) = LBD.BandWidth().Upper();
    Test(4, 1) = SBD.BandWidth().Lower();
    Test(4, 2) = SBD.BandWidth().Upper();
    ...
    X = SBD - I;
    Print(X);
    Test(5, 1) = BD.BandWidth().Lower();
    Test(5, 2) = BD.BandWidth().Upper();
    Test(6, 1) = UBD.BandWidth().Lower();
    Test(6, 2) = UBD.BandWidth().Upper();
    Test(7, 1) = LBD.BandWidth().Lower();
    Test(7, 2) = LBD.BandWidth().Upper();
    Test(8, 1) = SBD.BandWidth().Lower();
    Test(8, 2) = SBD.BandWidth().Upper();

    RowVector RV = D.AsRow();
    I.ReSize(6);
    BandMatrix BI = I;
    I = 1;
    BD = RV.AsDiagonal() + BI;
    X = BD - D - I;
    Print(X);
    Test(9, 1) = BD.BandWidth().Lower();
    Test(9, 2) = BD.BandWidth().Upper();
...

  {
    ...
    for (i = 1; i <= 20; ++i)
      for (j = 1; j <= 30; ++j)
      {
        if (M(i, j) != CM(i, j))
          ++Count(1, 1);
        if (M(i, j) != CM.element(i - 1, j - 1))
          ++Count(1, 1);
        if (M(i, j) != M.element(i - 1, j - 1))
          ++Count(1, 1);
      }
    ...
    for (i = 1; i <= 20; ++i)
      for (j = i; j <= 20; ++j)
      {
        if (U(i, j) != CU(i, j))
          ++Count(1, 1);
        if (U(i, j) != CU.element(i - 1, j - 1))
          ++Count(1, 1);
        if (U(i, j) != U.element(i - 1, j - 1))
          ++Count(1, 1);
      }
    ...
    for (i = 1; i <= 20; ++i)
      for (j = 1; j <= i; ++j)
      {
        if (L(i, j) != CL(i, j))
          ++Count(1, 1);
        if (L(i, j) != CL.element(i - 1, j - 1))
          ++Count(1, 1);
        if (L(i, j) != L.element(i - 1, j - 1))
          ++Count(1, 1);
      }
    ...
    for (i = 1; i <= 20; ++i)
      for (j = 1; j <= 20; ++j)
      {
        if (S(i, j) != CS(i, j))
          ++Count(1, 1);
        if (S(i, j) != CS.element(i - 1, j - 1))
          ++Count(1, 1);
        if (S(i, j) != S.element(i - 1, j - 1))
          ++Count(1, 1);
        if (S(i, j) != S(j, i))
          ++Count(1, 1);
        if (S(i, j) != CS(i, j))
    ...
    for (i = 1; i <= 20; ++i)
    {
      if (D(i, i) != CD(i, i))
        ++Count(1, 1);
      if (D(i, i) != CD.element(i - 1, i - 1))
        ++Count(1, 1);
      if (D(i, i) != D.element(i - 1, i - 1))
        ++Count(1, 1);
      if (D(i, i) != D(i))
        ++Count(1, 1);
      if (D(i) != CD(i))
        ++Count(1, 1);
      if (D(i) != CD.element(i - 1))
        ++Count(1, 1);
      if (D(i) != D.element(i - 1))
        ++Count(1, 1);
    }
    ...
    for (i = 1; i <= 20; ++i)
    {
      if (R(i) != CR(i))
        ++Count(1, 1);
      if (R(i) != CR.element(i - 1))
        ++Count(1, 1);
      if (R(i) != R.element(i - 1))
        ++Count(1, 1);
    }
    ...
    for (i = 1; i <= 20; ++i)
    {
      if (C(i) != CC(i))
        ++Count(1, 1);
      if (C(i) != CC.element(i - 1))
        ++Count(1, 1);
      if (C(i) != C.element(i - 1))
        ++Count(1, 1);
    }
...

  {
    ...
    UT << 3 << 7 << 3 << 9 << 5 << 2 << 6 << 8 << 0 << 4;
    LowerTriangularMatrix LT;
    LT.ReSize(UT);
    LT << 2 << 7 << 9 << 2 << 8 << 6 << 1 << 0 << 3 << 5;
...

  {
    ...
    A.ReSize(8, 9);
    for (i = 1; i <= 9; ++i)
      A.Column(i) = Helmert(9, i);
    A -= Y;
    Clean(A, 0.000000001);
    Print(A);

    ColumnVector Twos(100);
    Twos = 2;
    ColumnVector CV = Helmert(Twos);
    Clean(CV, 0.000000001);
    Print(CV);
