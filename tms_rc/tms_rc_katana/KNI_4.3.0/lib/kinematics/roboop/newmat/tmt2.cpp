...
void trymat2()
{
  ...
  // test growing and shrinking a vector
  {
    ...
    M = V;
    M = 100;  // to make sure V will hold its values
    for (i = 1; i <= 50; i++)
      V(i) -= i * i + i;
    Print(V);
  ...
  // test copying of vectors and matrices with no elements
  {
    ...
    dims(1) = M2.Nrows();
    dims(2) = M2.Ncols();
    dims(3) = (Real)(unsigned long)M2.Store();
    dims(4) = M2.Storage();
    M2 = M1;
    dims(5) = M2.Nrows();
    dims(6) = M2.Ncols();
    dims(7) = (Real)(unsigned long)M2.Store();
    dims(8) = M2.Storage();
    M2.ReSize(10, 20);
    M2.CleanUp();
    dims(9) = M2.Nrows();
    dims(10) = M2.Ncols();
    dims(11) = (Real)(unsigned long)M2.Store();
    dims(12) = M2.Storage();
    M2.ReSize(20, 10);
    M2.ReSize(0, 0);
    dims(13) = M2.Nrows();
    dims(14) = M2.Ncols();
    dims(15) = (Real)(unsigned long)M2.Store();
    dims(16) = M2.Storage();
    Print(dims);
  }
  ...
  // test identity matrix
  {
    ...
    N = SP(I, J) - SP(D, E);
    Print(N);
    N = D.SubMatrix(2, 5, 3, 8) - I.SubMatrix(2, 5, 3, 8);
    Print(N);
  ...
  {
    Matrix X(2, 3);
    X << 5.25 << 7.75 << 1.25 << 9.00 << 1.00 << 2.50;
    Matrix Y = X;
    X = 10 + X;
    X += (-10);
    X -= Y;
    ...
    // also test f suffix
    X << 5.25f << 7.75f << 1.25f << 9.00f << 1.00f << 2.50f;
    X -= Y;
    Print(X);
  }
