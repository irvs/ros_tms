...
void trymatk()
{
  ...
  for (i = 0; i < 55; i++)
    for (j = my_max(0, i - 10); j <= my_min(54, i + 5); j++)
    {
      A7[i][j] = i + 100 * j;
      B7(i + 1, j + 1) = i + 100 * j;
    }
  ...
  Real a[] = {1.2, 5.6, 7.9, 3.8, 4.5, 1.3, 5.2, 9.9, 2.1, 4.7, 0.0, 1.6};
  ColumnVector CV0(12);
  CV0 << a;
  ColumnVector CV1(a, 12);
  CV1 -= CV0;
  Print(CV1);
  RowVector RV1(a, 12);
  RV1 -= CV0.AsRow();
  Print(RV1);
  Matrix RM1(a, 3, 4);
  RM1 -= CV0.AsMatrix(3, 4);
  Print(RM1);
  ColumnVector CV2(5.75, 15);
  CV2 -= 5.75;
  Print(CV2);
  RowVector RV2(2.75, 15);
  RV2 -= 2.75;
  Print(RV2);
  Matrix RM2(-3.75, 6, 4);
  RM2 += 3.75;
