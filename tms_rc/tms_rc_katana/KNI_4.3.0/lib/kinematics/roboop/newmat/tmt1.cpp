...
void trymat1()
{
  ...
  }
  {
    Tracer et1("Stage 5");
    int i, j;
    BandMatrix BM1(10, 2, 3), BM2(10, 4, 1);
    Matrix M1(10, 10), M2(10, 10);
    for (i = 1; i <= 10; i++)
      for (j = 1; j <= 10; j++)
      {
        M1(i, j) = 0.5 * i + j * j - 50;
        M2(i, j) = (i * 101 + j * 103) % 13;
      }
    BM1.Inject(M1);
    BM2.Inject(M2);
    BandMatrix BM = BM1;
    ...
    MX -= M2X;
    Print(MX);
    SymmetricBandMatrix SM1;
    SM1 << BM1 *BM1.t();
    SymmetricBandMatrix SM2;
    SM2 << BM2 *BM2.t();
    SM1 *= 5.5;
    M1X *= M1X.t();
    M1X *= 5.5;
...

  {
    ...
    k << 0 << 0 << 1;
    ColumnVector X;
    X = CrossProduct(i, j) - k;
    Print(X);
    X = CrossProduct(j, k) - i;
    Print(X);
    X = CrossProduct(k, i) - j;
    Print(X);
    X = CrossProduct(j, i) + k;
    Print(X);
    X = CrossProduct(k, j) + i;
    Print(X);
    X = CrossProduct(i, k) + j;
    Print(X);
    X = CrossProduct(i, i);
    Print(X);
    X = CrossProduct(j, j);
    ...
    X = CrossProduct(A, CrossProduct(B, C)) + CrossProduct(B, CrossProduct(C, A)) + CrossProduct(C, CrossProduct(A, B));
    Print(X);
