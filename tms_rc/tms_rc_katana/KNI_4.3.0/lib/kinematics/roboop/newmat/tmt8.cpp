...
void trymat8()
{
  ...
  DiagonalMatrix DX = D;
  {
    Tracer et1("Stage 1");
    DX = (DX + D1) * DX;
    Print(Matrix(DX - (MD + MD1) * MD));
    DX = D;
    DX = -DX * DX + (DX - (-D1)) * ((-D1) + DX);
    // Matrix MX = Matrix(MD1);
    // MD1=DX+(MX.t())*(MX.t()); Print(MD1);
    MD1 = DX + (Matrix(MD1).t()) * (Matrix(MD1).t());
    Print(MD1);
    DX = D;
    DX = DX;
    DX = D2 - DX;
    Print(DiagonalMatrix(DX));
    DX = D;
  }
  {
    Tracer et1("Stage 2");
  ...
  }
  {
    ...
    LowerTriangularMatrix LT1 = GM1 - LT;
    Print(LT1);
    GenericMatrix GM2 = GM1;
    LT1 = GM2;
    LT1 = LT1 - LT;
    Print(LT1);
    GM2 = GM1;
  ...
  }
  {
    ...
    Matrix M(12, 12);
    M = 0;
    M(1, 1) = M(2, 2) = M(4, 4) = M(6, 6) = M(7, 7) = M(8, 8) = M(10, 10) = M(12, 12) = -1;
...
#endif
  {
    ...
    UT << 3 << 5 << 2 << 1 << 7 << 1 << 1 << 8 << 2 << 7 << 0 << 1 << 3 << 5 << 6;
    LowerTriangularMatrix LT(5);
    LT << 5 << 2 << 3 << 1 << 0 << 7 << 9 << 8 << 1 << 2 << 0 << 2 << 1 << 9 << 2;
...

  {
    ...
    GBM2 = BM2;
    GRM1 *= GBM2;
    GBM1 *= GRM2;  // Bs and Rs swapped on LHS
    X = GRM1 - BM1 * BM2;
    Print(X);
    X = RM1 * RM2 - GBM1;
    Print(X);
    ...
    SymmetricBandMatrix SBM;
    SBM << SP(BM1, BM1.t());
    SBM << IM.i() * SBM;
    X = 2 * SBM - SP(RM1, RM1.t());
    Print(X);
