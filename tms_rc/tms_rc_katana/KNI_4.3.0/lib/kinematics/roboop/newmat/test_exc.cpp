...
int main()
{
  ...
  Try
  {
    Throw(BaseException("Just a dummy\n"));
  }
  CatchAll{};
  {
    Matrix A1(40, 200);
  ...
  cout << "\nChecking for lost memory (large block): " << (unsigned long)s1 << " " << (unsigned long)s2 << " ";
  if (s1 != s2)
  ...
  cout << "\nChecking for lost memory (small block): " << (unsigned long)s3 << " " << (unsigned long)s4 << " ";
  if (s3 != s4)
    cout << " - see section 2.8\n\n";
  else
    cout << " - ok\n\n";
