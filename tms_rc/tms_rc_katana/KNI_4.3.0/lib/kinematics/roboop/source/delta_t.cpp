...
static const char rcsid[] = "$Id: delta_t.cpp,v 1.17 2005/07/01 16:11:45 gourdeau Exp $";
...
void Robot::delta_torque(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &qpp, const ColumnVector &dq,
                         const ColumnVector &dqp, const ColumnVector &dqpp, ColumnVector &ltorque,
...
*/
{
  int i;
  Matrix Rt, temp;
  if (q.Ncols() != 1 || q.Nrows() != dof)
    error("q has wrong dimension");
  if (qp.Ncols() != 1 || qp.Nrows() != dof)
    error("qp has wrong dimension");
  if (qpp.Ncols() != 1 || qpp.Nrows() != dof)
    error("qpp has wrong dimension");
  if (dq.Ncols() != 1 || dq.Nrows() != dof)
    error("dq has wrong dimension");
  if (dqp.Ncols() != 1 || dqp.Nrows() != dof)
    error("dqp has wrong dimension");
  if (dqpp.Ncols() != 1 || dqpp.Nrows() != dof)
    error("dqpp has wrong dimension");
  ...
  for (i = 1; i <= dof; i++)
  {
    ...
    if (links[i].get_joint_type() == 0)
    {
      ...
      dw[i] = Rt * (dw[i - 1] + z0 * dqp(i) - Q * (w[i - 1] + z0 * qp(i)) * dq(i));
      wp[i] = Rt * (wp[i - 1] + z0 * qpp(i) + CrossProduct(w[i - 1], z0 * qp(i)));
      dwp[i] =
          Rt * (dwp[i - 1] + z0 * dqpp(i) + CrossProduct(dw[i - 1], z0 * qp(i)) + CrossProduct(w[i - 1], z0 * dqp(i)) -
                Q * (wp[i - 1] + z0 * qpp(i) + CrossProduct(w[i - 1], z0 * qp(i))) * dq(i));
      vp[i] = CrossProduct(wp[i], p[i]) + CrossProduct(w[i], CrossProduct(w[i], p[i])) + Rt * (vp[i - 1]);
      dvp[i] = CrossProduct(dwp[i], p[i]) + CrossProduct(dw[i], CrossProduct(w[i], p[i])) +
               CrossProduct(w[i], CrossProduct(dw[i], p[i])) + Rt * (dvp[i - 1] - Q * vp[i - 1] * dq(i));
    }
    else
    {
      w[i] = Rt * w[i - 1];
      ...
      wp[i] = Rt * wp[i - 1];
      dwp[i] = Rt * dwp[i - 1];
      vp[i] = Rt * (vp[i - 1] + z0 * qpp(i) + 2.0 * CrossProduct(w[i - 1], z0 * qp(i))) + CrossProduct(wp[i], p[i]) +
              CrossProduct(w[i], CrossProduct(w[i], p[i]));
      dvp[i] = Rt * (dvp[i - 1] + z0 * dqpp(i) +
                     2.0 * (CrossProduct(dw[i - 1], z0 * qp(i)) + CrossProduct(w[i - 1], z0 * dqp(i)))) +
               CrossProduct(dwp[i], p[i]) + CrossProduct(dw[i], CrossProduct(w[i], p[i])) +
               CrossProduct(w[i], CrossProduct(dw[i], p[i])) +
               (CrossProduct(wp[i], dp[i]) + CrossProduct(w[i], CrossProduct(w[i], dp[i]))) * dq(i);
    }
    a[i] = CrossProduct(wp[i], links[i].r) + CrossProduct(w[i], CrossProduct(w[i], links[i].r)) + vp[i];
    da[i] = CrossProduct(dwp[i], links[i].r) + CrossProduct(dw[i], CrossProduct(w[i], links[i].r)) +
            CrossProduct(w[i], CrossProduct(dw[i], links[i].r)) + dvp[i];
  }
  ...
  for (i = dof; i >= 1; i--)
  {
    ...
    dN[i] = links[i].I * dwp[i] + CrossProduct(dw[i], links[i].I * w[i]) + CrossProduct(w[i], links[i].I * dw[i]);
    if (i == dof)
    {
      f[i] = F[i];
      df[i] = dF[i];
      n[i] = CrossProduct(p[i], f[i]) + CrossProduct(links[i].r, F[i]) + N[i];
      dn[i] = CrossProduct(p[i], df[i]) + CrossProduct(links[i].r, dF[i]) + dN[i];
      if (links[i].get_joint_type() != 0)
    ...
    else
    {
      ...
      n[i] = links[i + 1].R * n[i + 1] + CrossProduct(p[i], f[i]) + CrossProduct(links[i].r, F[i]) + N[i];
      dn[i] = links[i + 1].R * dn[i + 1] + CrossProduct(p[i], df[i]) + CrossProduct(links[i].r, dF[i]) + dN[i];
      if (links[i].get_joint_type() == 0)
...
void mRobot::delta_torque(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &qpp,
                          const ColumnVector &dq, const ColumnVector &dqp, const ColumnVector &dqpp,
...
*/
{
  ...
  for (i = 1; i <= dof; i++)
  {
    ...
    {
      w[i] = Rt * w[i - 1] + z0 * qp(i);
      dw[i] = Rt * dw[i - 1] + z0 * dqp(i) - Q * Rt * w[i - 1] * dq(i);
      wp[i] = Rt * wp[i - 1] + CrossProduct(Rt * w[i - 1], z0 * qp(i)) + z0 * qpp(i);
      dwp[i] = Rt * dwp[i - 1] + CrossProduct(Rt * dw[i - 1], z0 * qp(i)) + CrossProduct(Rt * w[i - 1], z0 * dqp(i)) -
               (Q * Rt * wp[i - 1] + CrossProduct(Q * Rt * w[i - 1], z0 * qp(i))) * dq(i) + z0 * dqpp(i);
      vp[i] = Rt * (CrossProduct(wp[i - 1], p[i]) + CrossProduct(w[i - 1], CrossProduct(w[i - 1], p[i])) + vp[i - 1]);
      dvp[i] = Rt * (CrossProduct(dwp[i - 1], p[i]) + CrossProduct(dw[i - 1], CrossProduct(w[i - 1], p[i])) +
                     CrossProduct(w[i - 1], CrossProduct(dw[i - 1], p[i])) + dvp[i - 1]) -
               Q * Rt *
                   (CrossProduct(wp[i - 1], p[i]) + CrossProduct(w[i - 1], CrossProduct(w[i - 1], p[i])) + vp[i - 1]) *
                   dq(i);
    }
    else
    {
      ...
      wp[i] = Rt * wp[i - 1];
      dwp[i] = Rt * dwp[i - 1];
      vp[i] = Rt * (vp[i - 1] + CrossProduct(wp[i - 1], p[i]) + CrossProduct(w[i - 1], CrossProduct(w[i - 1], p[i]))) +
              z0 * qpp(i) + 2.0 * CrossProduct(w[i], z0 * qp(i));

      dvp[i] = Rt * (CrossProduct(dwp[i - 1], p[i]) + CrossProduct(dw[i - 1], CrossProduct(w[i - 1], p[i])) +
                     CrossProduct(w[i - 1], CrossProduct(dw[i - 1], p[i])) + dvp[i - 1] +
                     (CrossProduct(wp[i - 1], dp[i]) + CrossProduct(w[i - 1], CrossProduct(w[i - 1], dp[i]))) * dq(i)) +
               2 * (CrossProduct(dw[i], z0 * qp(i)) + CrossProduct(w[i], z0 * dqp(i))) + z0 * dqpp(i);
    }
    a[i] = CrossProduct(wp[i], links[i].r) + CrossProduct(w[i], CrossProduct(w[i], links[i].r)) + vp[i];
    da[i] = CrossProduct(dwp[i], links[i].r) + CrossProduct(dw[i], CrossProduct(w[i], links[i].r)) +
            CrossProduct(w[i], CrossProduct(dw[i], links[i].r)) + dvp[i];
  ...
  for (i = dof; i >= 1; i--)
  {
    ...
    else
    {
      ...
      if (links[i + 1].get_joint_type() == 0)
        dn[i] += (links[i + 1].R * Q * n[i + 1] + CrossProduct(p[i + 1], links[i + 1].R * Q * f[i + 1])) * dq(i + 1);
      else
...
void mRobot_min_para::delta_torque(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &qpp,
                                   const ColumnVector &dq, const ColumnVector &dqp, const ColumnVector &dqpp,
                                   ColumnVector &ltorque, ColumnVector &dtorque)
/*!
  @brief Delta torque dynamics.
...
*/
{
  ...
  for (i = dof; i >= 1; i--)
  {
    F[i] = vp[i] * links[i].m + CrossProduct(wp[i], links[i].mc) + CrossProduct(w[i], CrossProduct(w[i], links[i].mc));
    dF[i] = dvp[i] * links[i].m + CrossProduct(dwp[i], links[i].mc) +
            CrossProduct(dw[i], CrossProduct(w[i], links[i].mc)) + CrossProduct(w[i], CrossProduct(dw[i], links[i].mc));
    N[i] = links[i].I * wp[i] + CrossProduct(w[i], links[i].I * w[i]) - CrossProduct(vp[i], links[i].mc);
    dN[i] = links[i].I * dwp[i] + CrossProduct(dw[i], links[i].I * w[i]) + CrossProduct(w[i], links[i].I * dw[i]) -
