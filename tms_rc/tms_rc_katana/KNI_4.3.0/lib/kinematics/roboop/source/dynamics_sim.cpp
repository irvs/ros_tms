...
*/
{
  ...
  {
    if ((path_select.path.p_pdot_pddot(0.0, pd, ppd, pppd) == 0) && (path_select.path_quat.quat_w(0.0, quatd, wd) == 0))
    {
      Matrix Tobj(4, 4);
      Tobj = 0;
  ...
  if (robot)
  {
    switch (control.type)
    {
      ...
      case RRA:
        ...
        {
          if (path_select.path.p_pdot_pddot(time, pd, ppd, pppd) || path_select.path_quat.quat_w(time, quatd, wd))
          {
            xd = qp & qpp;
            xd.Release();
            return xd;
