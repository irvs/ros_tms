...
*/
{
  ...
  if (converge)
  {
    ...
    // Make sure that: theta_min <= qout <= theta_max for revolute joints
    for (int i = 1; i <= dof; i++)
    {
      if (links[i].immobile)
        continue;
      if (links[i].get_joint_type() == 0)
      {
        if (links[i].get_theta_min() > qout(i))
        {
          converge = false;
        }
        if (links[i].get_theta_max() < qout(i))
        {
          converge = false;
        }
      }
    }
    //      if (converge) { // return solution out of range too, but set converge flag to false
    set_q(qPrev);
    qout.Release();
    return qout;
    /*      } else {
...
*/
{
  ...
  try
  {
    Real theta[6], diff1, diff2, tmp, angle, L = 0.0, M = 0.0, K = 0.0, H = 0.0, G = 0.0;
    ...
    angle = atan2(-1 * cos(theta[1]) * Tobj(1, 3) - sin(theta[1]) * Tobj(2, 3), -1 * Tobj(3, 3));

    L = cos(theta[1]) * Tobj(1, 4) + sin(theta[1]) * Tobj(2, 4) + links[5].d * sin(angle) - links[4].a * cos(angle);
    M = links[1].d - Tobj(3, 4) - links[5].d * cos(angle) - links[4].a * sin(angle);
    K = (L * L + M * M - links[3].a * links[3].a - links[2].a * links[2].a) / (2 * links[3].a * links[2].a);
    ...
    theta[4] =
        atan2(-1 * cos(theta[1]) * Tobj(1, 3) - sin(theta[1]) * Tobj(2, 3), -1 * Tobj(3, 3)) - theta[2] - theta[3];
...
*/
{
  ...
  try
  {
    Real theta[7], diff1, diff2, tmp, A = 0.0, B = 0.0, C = 0.0, D = 0.0, Ro = 0.0, H = 0.0, L = 0.0, M = 0.0;
    ...
    C = Tobj(1, 4) * Tobj(1, 4) + Tobj(2, 4) * Tobj(2, 4) + (Tobj(3, 4) - links[1].d) * (Tobj(3, 4) - links[1].d) -
        (links[2].d + links[3].d) * (links[2].d + links[3].d) - links[2].a * links[2].a - links[3].a * links[3].a -
        links[4].d * links[4].d;
    A = C / (2 * links[2].a);
    ...
    theta[0] = atan2(-sin(theta[1]) * Tobj(1, 3) + cos(theta[1]) * Tobj(2, 3),
                     cos(theta[2] + theta[3]) * (cos(theta[1]) * Tobj(1, 3) + sin(theta[1]) * Tobj(2, 3)) -
    ...
    theta[5] =
        atan2(cos(theta[4]) * (cos(theta[2] + theta[3]) * (cos(theta[1]) * Tobj(1, 3) + sin(theta[1]) * Tobj(2, 3)) -
                               (sin(theta[2] + theta[3]) * Tobj(3, 3))) +
                  sin(theta[4]) * (-sin(theta[1]) * Tobj(1, 3) + cos(theta[1]) * Tobj(2, 3)),
              sin(theta[2] + theta[3]) * (cos(theta[1]) * Tobj(1, 3) + sin(theta[1]) * Tobj(2, 3)) +
                  (cos(theta[2] + theta[3]) * Tobj(3, 3)));

    theta[6] =
        atan2(-sin(theta[4]) * (cos(theta[2] + theta[3]) * (cos(theta[1]) * Tobj(1, 1) + sin(theta[1]) * Tobj(2, 1)) -
                                (sin(theta[2] + theta[3]) * Tobj(3, 1))) +
                  cos(theta[4]) * (-sin(theta[1]) * Tobj(1, 1) + cos(theta[1]) * Tobj(2, 1)),
              -sin(theta[4]) * (cos(theta[2] + theta[3]) * (cos(theta[1]) * Tobj(1, 2) + sin(theta[1]) * Tobj(2, 2)) -
                                (sin(theta[2] + theta[3]) * Tobj(3, 2))) +
                  cos(theta[4]) * (-sin(theta[1]) * Tobj(1, 2) + cos(theta[1]) * Tobj(2, 2)));
...
*/
{
  ...
  try
  {
    ...
    theta[5] =
        atan2((cos(theta234) * (cos(theta[1]) * Tobj(1, 3) + sin(theta[1]) * Tobj(2, 3)) + sin(theta234) * Tobj(3, 3)),
              (sin(theta[1]) * Tobj(1, 3) - cos(theta[1]) * Tobj(2, 3)));

    theta[6] = atan2(
        (-sin(theta234) * (cos(theta[1]) * Tobj(1, 1) + sin(theta[1]) * Tobj(2, 1)) + cos(theta234) * Tobj(3, 1)),
        (-sin(theta234) * (cos(theta[1]) * Tobj(1, 2) + sin(theta[1]) * Tobj(2, 2)) + cos(theta234) * Tobj(3, 2)));

    A = cos(theta[1]) * Tobj(1, 4) + sin(theta[1]) * Tobj(2, 4) - links[1].a - links[4].a * cos(theta234);

    B = Tobj(3, 4) - links[1].d - links[4].a * sin(theta234);

    // cos(theta3)
    K = (A * A + B * B - (links[3].a * links[3].a) - (links[2].a * links[2].a)) / (2 * links[2].a * links[3].a);
...
*/
{
  ...
  try
  {
    ...
    L = cos(theta[1]) * Tobj(1, 4) + sin(theta[1]) * Tobj(2, 4) + links[5].d * sin(angle) - links[5].a * cos(angle);
    M = links[1].d - Tobj(3, 4) - links[5].d * cos(angle) - links[5].a * sin(angle);
    K = (L * L + M * M - links[4].a * links[4].a - links[3].a * links[3].a) / (2 * links[4].a * links[4].a);
...
*/
{
  ...
  try
  {
    ...
    C = Tobj(1, 4) * Tobj(1, 4) + Tobj(2, 4) * Tobj(2, 4) + (Tobj(3, 4) - links[1].d) * (Tobj(3, 4) - links[1].d) -
        (links[2].d + links[3].d) * (links[2].d + links[3].d) - links[3].a * links[3].a - links[4].a * links[4].a -
        links[4].d * links[4].d;
    A = C / (2 * links[3].a);
...
*/
{
  ...
  try
  {
    ...
    A = cos(theta[1]) * Tobj(1, 4) + sin(theta[1]) * Tobj(2, 4) - links[2].a - links[5].a * cos(theta234);

    B = Tobj(3, 4) - links[1].d - links[5].a * sin(theta234);

    // cos(theta3)
    K = (A * A + B * B - (links[4].a * links[4].a) - (links[3].a * links[3].a)) / (2 * links[3].a * links[4].a);
