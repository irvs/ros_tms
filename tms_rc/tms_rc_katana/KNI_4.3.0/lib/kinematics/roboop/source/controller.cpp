...
Impedance::Impedance(const Robot_basic &robot, const DiagonalMatrix &Mp_, const DiagonalMatrix &Dp_,
                     const DiagonalMatrix &Kp_, const DiagonalMatrix &Mo_, const DiagonalMatrix &Do_,
...
short Impedance::control(const ColumnVector &pdpp, const ColumnVector &pdp, const ColumnVector &pd,
                         const ColumnVector &wdp, const ColumnVector &wd, const Quaternion &qd, const ColumnVector &f,
                         const ColumnVector &n, const Real dt)
/*!
...
Resolved_acc::Resolved_acc(const Robot_basic &robot, const Real Kvp_, const Real Kpp_, const Real Kvo_, const Real Kpo_)
//! @brief Constructor.
{
  set_Kvp(Kvp_);
  set_Kpp(Kpp_);
...
ReturnMatrix Resolved_acc::torque_cmd(Robot_basic &robot, const ColumnVector &pdpp, const ColumnVector &pdp,
                                      const ColumnVector &pd, const ColumnVector &wdp, const ColumnVector &wd,
                                      const Quaternion &quatd, const short link_pc, const Real dt)
/*!
  @brief Output torque.
...
*/
{
  ...
  quat_v_error = quate.s() * quat.v() - quat.s() * quate.v() + x_prod_matrix(quate.v()) * quat.v();

  a.SubMatrix(1, 3, 1, 1) = pdpp + Kvp * (pdp - pp) + Kpp * (pd - p);
  a.SubMatrix(4, 6, 1, 1) = wdp + Kvo * (wd - Rot * robot.w[robot.get_dof()]) + Kpo * quat_v_error;
  qp = robot.get_qp();
  //                          (eps, lamda_max)
...
Computed_torque_method::Computed_torque_method(const Robot_basic &robot, const DiagonalMatrix &Kp,
...
ReturnMatrix Computed_torque_method::torque_cmd(Robot_basic &robot, const ColumnVector &qd, const ColumnVector &qpd,
                                                const ColumnVector &qppd)
//! @brief Output torque.
{
  if (qd.Nrows() != dof)
  {
    ColumnVector tau(dof);
    tau = 0;
...
Proportional_Derivative::Proportional_Derivative(const Robot_basic &robot, const DiagonalMatrix &Kp,
                                                 const DiagonalMatrix &Kd)
//! @brief Constructor.
{
  dof = robot.get_dof();
  ...
  q = 0;
  qp = q;
  tau = ColumnVector(dof);
  tau = 0;
  zero3 = ColumnVector(3);
  zero3 = 0;
}

ReturnMatrix Proportional_Derivative::torque_cmd(Robot_basic &robot, const ColumnVector &qd, const ColumnVector &qpd)
//! @brief Output torque.
{
  if (qd.Nrows() != dof)
  {
