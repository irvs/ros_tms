...
ReturnMatrix Robot_basic::acceleration(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &tau_cmd)
...
ReturnMatrix Robot_basic::acceleration(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &tau_cmd,
                                       const ColumnVector &Fext, const ColumnVector &Next)
/*!
...
ReturnMatrix Robot::torque(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &qpp)
/*!
...
*/
{
  ...
  for (i = dof; i >= 1; i--)
  {
    ...
    {
      f[i] = F[i] + Fext;
      n[i] = CrossProduct(p[i], f[i]) + CrossProduct(links[i].r, F[i]) + N[i] + Next;
    }
    else
    {
...
*/
{
  ...
  for (i = 1; i <= dof; i++)
  {
    ...
    if (links[i].get_joint_type() == 0)
    {
      ...
      vp[i] = CrossProduct(wp[i], p[i]) + Rt * (vp[i - 1]);
    }
    else
    {
      wp[i] = Rt * wp[i - 1];
      vp[i] = Rt * (vp[i - 1] + z0 * qpp(i)) + CrossProduct(wp[i], p[i]);
    }
  ...
  for (i = dof; i >= 1; i--)
  {
    ...
    {
      f_nv[i] = F[i];
      n_nv[i] = CrossProduct(p[i], f_nv[i]) + CrossProduct(links[i].r, F[i]) + N[i];
    }
    else
    {
      f_nv[i] = links[i + 1].R * f_nv[i + 1] + F[i];
      n_nv[i] = links[i + 1].R * n_nv[i + 1] + CrossProduct(p[i], f_nv[i]) + CrossProduct(links[i].r, F[i]) + N[i];
    }
    if (links[i].get_joint_type() == 0)
...
//! @brief Joint torque due to gravity based on Recursive Newton-Euler formulation.
{
  ...
  for (i = dof; i >= 1; i--)
  {
    ...
    {
      f[i] = F[i];
      n[i] = CrossProduct(p[i], f[i]) + CrossProduct(links[i].r, F[i]);
    }
    else
    {
      f[i] = links[i + 1].R * f[i + 1] + F[i];
      n[i] = links[i + 1].R * n[i + 1] + CrossProduct(p[i], f[i]) + CrossProduct(links[i].r, F[i]);
    }
    if (links[i].get_joint_type() == 0)
      temp = ((z0.t() * links[i].R) * n[i]);
    else
...
//! @brief Joint torque due to centrifugal and Corriolis based on Recursive Newton-Euler formulation.
{
  ...
  for (i = dof; i >= 1; i--)
  {
    ...
    if (links[i].get_joint_type() == 0)
      ...
      temp = ((z0.t() * links[i].R) * f[i]);
    ltorque(i) = temp(1, 1) + links[i].Gr * (links[i].Gr * links[i].B * qp(i) + links[i].Cf * sign(qp(i)));
  }
...
ReturnMatrix mRobot::torque(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &qpp)
...
ReturnMatrix mRobot::torque(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &qpp,
                            const ColumnVector &Fext_, const ColumnVector &Next_)
/*!
  @brief Joint torque based on Recursive Newton-Euler formulation.
...
//! @brief Joint torque. when joint velocity is 0, based on Recursive Newton-Euler formulation.
{
  ...
  for (i = 1; i <= dof; i++)
  {
    ...
    {
      wp[i] = Rt * wp[i - 1];
      vp[i] = Rt * (vp[i - 1] + CrossProduct(wp[i - 1], p[i])) + z0 * qpp(i);
    }
    a[i] = CrossProduct(wp[i], links[i].r) + vp[i];
  }
...
ReturnMatrix mRobot_min_para::torque(const ColumnVector &q, const ColumnVector &qp, const ColumnVector &qpp)
//! @brief Joint torque without contact force based on Recursive Newton-Euler formulation.
{
  ColumnVector Fext(3), Next(3);
  Fext = 0;
...
*/
{
  ...
  for (i = 1; i <= dof; i++)
  {
    ...
    {
      w[i] = Rt * w[i - 1] + z0 * qp(i);
      wp[i] = Rt * (wp[i - 1] + CrossProduct(w[i - 1], z0 * qp(i))) + z0 * qpp(i);
      vp[i] = Rt * (CrossProduct(wp[i - 1], p[i]) + CrossProduct(w[i - 1], CrossProduct(w[i - 1], p[i])) + vp[i - 1]);
    }
    else
    {
