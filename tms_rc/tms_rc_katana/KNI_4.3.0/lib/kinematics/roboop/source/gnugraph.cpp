...
char *curvetype[] = {"lines", "points", "linespoints", "impulses", "dots", "steps", "boxes"};
...
/* all command and data files are ready for the call to gnuplot */
  ...
  char *d;
  HWND hwndm, hwnd;
  if (WinExec(GNUPLOT, SW_SHOWNORMAL) <= 32)
  { /* start gnuplot */
    /* failed */
  ...
  d = c;
  while (*d != '\0')
  { /* sending the command through windows messages */
  ...
  if (data.size())
  {
    if (!nb_iterations_write)
    {
      ...
      if (stat(ptr_filename, &buf))  // File does not exist
      {
        ...
        if (outvecfile)
        {
          outvecfile << "nd_iterations " << nb_iterations_write << "        " << endl;
          outvecfile << "nb_vector " << data.size() << endl;
          for (unsigned int i = 0; i < data.size(); i++)
            outvecfile << "nb_rows " << data[i].Nrows() << "  "
                       << "nb_cols " << data[i].Ncols() << "  " << title[i] << endl;
          outvecfile << "---------------------------------\n";
        }
        else
        {
  ...
  if (invecfile)
  {
    if (!position_read)
    {
      ...
      for (int j = 0; j < nb_element; j++)
      {
        ...
        if ((nbrow != data[j].Nrows()) || (nbcol != data[j].Ncols()))
...
short Plot_file::graph(const string &title_graph, const string &label, const short x, const short y,
                 ...
                 const vector<int> &data_select)
{
  ...
  for (unsigned int i = 0; i < data_select.size(); i++)
    plotgraph.addcurve((xdata & ydata.SubMatrix(data_select[i], data_select[i], 1, ydata.Ncols())).t(), label[i],
                       enLineType);
  plotgraph.gnuplot();
...
short set_plot3d(const Matrix &xyz, const string &title_graph, const string &x_axis_title, const string &y_axis_title,
