...
bool action(tms_msg_rp::action::Request  &req,
            tms_msg_rp::action::Response &res)
{
...
class SmartpalActionPlugin : public Plugin
{
 public:
  ...
  void LoopRosTest(void) {
    ...
    for(int i=0; i< rplsrv.response.rps_path[0].rps_route.size(); i++){
        ROS_INFO("[%d]: x=%f, y=%f, th=%f", i, rplsrv.response.rps_path[0].rps_route[i].x, rplsrv.response.rps_path[0].rps_route[i].y, rplsrv.response.rps_path[0].rps_route[i].th);
    }
    } else {
 ...
 public:
  ...
  virtual bool initialize() {
    is_ros_init = false;
    menuManager().setPath("/Tools").addItem("SmartpalAction")
      ->sigTriggered().connect(bind(&SmartpalActionPlugin::OnRosInterfaceActivated, this));
