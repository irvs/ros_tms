...
public:
	...
	double getVW(){ return varw;}
	double getVU(){ return varu;}
	...
	void setX(int i, double s){ *(x+i) = s;} 
	double setVW(double vw){ 
		varw = vw;
		dzero(m,m,W);
		for(int i=0; i<m; i++) *(W+i*m+i) = varw; 
	}
	double setVU(double vu){ 
		varu = vu;
		dzero(l,l,U);
		for(int i=0; i<l; i++) *(U+i*l+i) = varu; 
	}
	void setA(double *a){
		memcpy(A,a,n*n*sizeof(double));
	}
	...
	void init(double vw = 0.1, double vu = 0.1, double t = 0.1)
	{
		...
		// 以下　等加速度運動を仮定
		deye(n,n,A);
		for(int i=n/2; i<n; i++) *(A+(i-n/2)*n+i) = DT;

		dzero(n,l,B);
		for(int i=0; i<l; i++) *(B+i*l+i)     = DT*DT/2;
		for(int i=0; i<l; i++) *(B+(i+n/2)*l+i) = DT;

		//観測は位置
		dzero(m,n,C);
		for(int i=0; i<m; i++) *(C+i*n+i) = 1.0; 
