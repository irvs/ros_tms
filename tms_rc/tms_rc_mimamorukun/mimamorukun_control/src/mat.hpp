...
inline void deye(int l, int m, void* m1)		//뭁댧뛱쀱
{
	...
	for(i=0;i<l;i++)
		for(j=0;j<m;j++)
			if(i==j) *((double *)m1+m*i+j) = 1.0;
			else *((double *)m1+m*i+j) = 0.0;
}
...
{
	*((double *)m+3*0+0) = 1.0;    *((double *)m+3*0+1) =      0.0;    *((double *)m+3*0+2) =       0.0;
	*((double *)m+3*1+0) = 0.0;    *((double *)m+3*1+1) = cos(phi);    *((double *)m+3*1+2) = -sin(phi);
	*((double *)m+3*2+0) = 0.0;    *((double *)m+3*2+1) = sin(phi);    *((double *)m+3*2+2) =  cos(phi);
}

inline void rotj(double phi, void* m)		//굺렡뢂귟궻됷?뛱쀱
{
	*((double *)m+3*0+0) =  cos(phi);    *((double *)m+3*0+1) = 0.0;    *((double *)m+3*0+2) = sin(phi);
	*((double *)m+3*1+0) =       0.0;    *((double *)m+3*1+1) = 1.0;    *((double *)m+3*1+2) =      0.0;
	*((double *)m+3*2+0) = -sin(phi);    *((double *)m+3*2+1) = 0.0;    *((double *)m+3*2+2) = cos(phi);
}

inline void rotk(double phi, void* m)		//굾렡뢂귟궻됷?뛱쀱
{
	*((double *)m+3*0+0) =  cos(phi);    *((double *)m+3*0+1) = -sin(phi);   *((double *)m+3*0+2) = 0.0;
	*((double *)m+3*1+0) =  sin(phi);    *((double *)m+3*1+1) =  cos(phi);   *((double *)m+3*1+2) = 0.0;
	*((double *)m+3*2+0) =       0.0;    *((double *)m+3*2+1) =       0.0;   *((double *)m+3*2+2) = 1.0;
}
...
inline void rot(double x,double y,double z,double phi, void* m)		//任意の軸に回転
{
	...
	x = x / r;	y = y / r;	z = z / r;
	*((double *)m+3*0+0) = (x * x * (1 - cos(phi)) + cos(phi));    *((double *)m+3*0+1) = (x * y * (1 - cos(phi)) + z * sin(phi));    *((double *)m+3*0+2) = (x * z * (1 - cos(phi)) - y * sin(phi));
	*((double *)m+3*1+0) = (x * y * (1 - cos(phi)) - z * sin(phi));    *((double *)m+3*1+1) = (y * y * (1 - cos(phi)) + cos(phi));		  *((double *)m+3*1+2) = (y * z * (1 - cos(phi)) + x * sin(phi));
	*((double *)m+3*2+0) = (x * z * (1 - cos(phi)) + y * sin(phi));    *((double *)m+3*2+1) = (y * z * (1 - cos(phi)) - x * sin(phi));    *((double *)m+3*2+2) = (z * z * (1 - cos(phi)) + cos(phi));
}
...
inline void calcOffsetWithMotion(double resOffset[3],int x,int y,int z,double pitch,double roll,double yaw,bool acceptMotionEffect)
{
	double offset[3] = {(double)x , (double)y , (double)z};
	double rotateM[3][3];
	if (acceptMotionEffect) makeSRPYrotateMatrix(rotateM,0,roll,pitch,yaw);
	else makeSRPYrotateMatrix(rotateM,0,0,0,yaw);
...
inline void nsa_to_abg(double mat[3][3], double vec[3])
{
	...
	if(sin(beta)>MINLBL)
		...
		gamma = atan2(cos(alpha)*normal[1]
	-sin(alpha)*normal[0],
		cos(alpha)*slide[1]
	-sin(alpha)*slide[0]);
	vec[0] = alpha;
...
inline void abg_to_nsa(double vec[3], double mat[3][3])
{
	...
	normal[0] = cos(alpha)*cos(beta)*cos(gamma)
		-sin(alpha)*sin(gamma);
	normal[1] = sin(alpha)*cos(beta)*cos(gamma)
		+cos(alpha)*sin(gamma);
	normal[2] =-sin(beta)*cos(gamma);

	slide[0] =-cos(alpha)*cos(beta)*sin(gamma)
		-sin(alpha)*cos(gamma);
	slide[1] =-sin(alpha)*cos(beta)*cos(gamma)
		+cos(alpha)*cos(gamma);
...
inline void djacobi_method(int n, void *a ,void *vec ,void *val ,int *found)
{
	...
	for(i=0;i<n;i++)
		for(j=0;j<n;j++)
			if(i==j) *((double *)vec+n*i+j) = 1;
			else *((double *)vec+n*i+j) = 0;
...
inline void dminv2(int m, int n, void *mt, void *mi, int getdof)
{
	...
	for(i=0;i<n;i++)
		for(j=0;j<n;j++)
			if(i==j) *((double *)d+n*i+i) = vecN[i];
			else *((double *)d+n*i+j) = 0;
					...
					if(i==j){
						if(dof < getdof){
							*((double *)sqd+n*i+i) = 1/sqrt(*((double *)d+n*i+i));
							dof=dof+1;
						} else *((double *)sqd+n*i+i) = 0;
					} else *((double *)sqd+n*i+j) = 0;
				}
