...
class MainWidget(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        # self.status_lbl.setStyleSheet('color: red; subcontrol-position:
        # center top; text-align: right;')
        self.status_lbl.setStyleSheet(
            'text-align: right; color: red; font: bold 18px;')
...
#        , "    ", self.wc_pos)
    ...
    def updateCurPos(self):
        try:
            ...
            res = srv_client(req)
            if 0 < len(res.tmsdb):
                self.wc_pos = QtCore.QPoint(
                    res.tmsdb[0].x * 1000, res.tmsdb[0].y * 1000)
                # print(self.wc_pos.x(), self.wc_pos.y()),
                wc_mark_pos = QtCore.QPoint(
                    self.wc_pos.x() * 1.0 / ROOM_SIZE.x() *
                    MAP_SIZE.x() + MAP_ORIGN.x(),
    ...
    def startMoving(self):
        ...
        if not ARGS.direct:
            ...
            req.arg = [
                -1, self.tgt_pos.x() * 0.001, self.tgt_pos.y() * 0.001, 0]
        else:
            srv_client = rospy.ServiceProxy(
                "/mkun_goal_pose", tms_msg_rc.srv.rc_robot_control)
    ...
    def updateTgtPos(self, event):
        self.tgt_pos = QtCore.QPoint(
            (event.pos().x() * 1.0 - MAP_ORIGN.x()) /
            MAP_SIZE.x() * ROOM_SIZE.x(),
...
def main():
    ...
    p.add_argument(
        "-d", "--direct", action='store_true', default=False, help='移動先座標を直接指定')
    ARGS = p.parse_args()
    # print args.direct
